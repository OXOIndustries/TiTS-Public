...
function flahneSexMenu(display:Boolean = true):void {
	...
	if(flags["FLAHNE_LIKE_OVIPOSITOR"] > 0 && !pc.isTaur()) addButton(0,"Fuck N Suck",fuckAndSuckWithFlahne);
	else addDisabledButton(0,"Locked");
	//Desk Munchies
	//Requires rahn race discussion has taken place
	if(flags["TALKED_ABOUT_FLAHNES_RACE"] > 0 && flags["FLAHNE_DESK_DISABLED"] != 1 && !pc.isTaur()) addButton(1,"Under Desk",flahneDeskMunchies);
	...
	if(pc.hasVagina() && !pc.isTaur()) addButton(2,"GetEatenOut",flahneMunchesBoxesMetalBawkses);
	else addDisabledButton(2,"Locked");
	
	//Rahn Suppository (All Sexes, or lack thereof; needs Ovi; Savin's Fault)
	if(flags["FLAHNE_LIKE_OVIPOSITOR"] > 0 && !pc.isTaur()) addButton(3,"Get Stuffed",rahnSuppository);
	else addDisabledButton(3,"Locked");
	
	//Bend her Over (Dickwielders; Savin wrote dis)
	if(pc.hasCock() && pc.cockThatFits(flahne.vaginalCapacity()) >= 0) addButton(4,"Bend Her Over",bendFlahneOver);
...
function flahneDeskMunchies():void {
	...
	//[Eat Out] [Suck&Swallow] [Play&Spray(only for semi-bastards)]
		...
		//Eat Out
		addButton(0,"Eat Out",eatFlahneOutUnderDesk);
		//Suck off
		addButton(1,"Suck Off",flahneGetsSuckedAndSwallowed);
		//Play&Spray
		if(pc.isAss()) addButton(0,"PlayNSpray",playAndSpray);
	}
}
...
function flahneGetsSuckedAndSwallowed():void {
	...
	if(pc.hasFaceFlag(MUZZLED)) output("Thanks to your muzzle, you don’t have much problems accepting a few more inches into your mouth. ");
