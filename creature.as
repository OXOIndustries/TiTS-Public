package
{
	...
	public class creature
	{
		...
		public var characterClass:int = SMUGGLER;
		...
		public var vaginas:Array;
			...
			public var girlCumType:Number = GIRLCUM;
		...
		public function skinFurScales(forceTone:Boolean = false, forceAdjective:Boolean = false, skin:Boolean = false):String {
			...
			if(forceAdjective || rand(3) == 0) {
				if(hasSkinFlag(SMOOTH)) output += "smooth";
				else if(hasSkinFlag(THICK)) output += "thick";
				else if(hasSkinFlag(STICKY)) output += "sticky";
				else if(hasSkinFlag(FLUFFY) && !skin && skinType == 1) output += "fluffy";
			}
			//25% of time, describe skin tone.
			if(forceTone || rand(4) == 0) {
				if(output != "") output += ", ";
				if(skinType == FUR && !skin) output += furColor;
				else if(skinType == SCALES && !skin) output += scaleColor;
		...
		public function face(forceAdjectives:Boolean = false):String {
			...
			if(rand(4) == 0 || forceAdjectives) {
				if(faceType == FELINE) output += "feline";
				if(faceType == EQUINE) output += "long";
				if(faceType == BOVINE) output += "bovine";
				if(hasFaceFlag(ANGULAR)) output += "angular";
				if(faceType == LIZAN || faceType == DRACONIC) output += "reptilian";
			}
			//Space if adjective'd
			if(output != "") output += " ";
			
			//Add Noun
			if(hasFaceFlag(MUZZLED) && rand(2) == 0) output += "muzzle";
			else if(rand(2) == 0 && hasFaceFlag(MUZZLED) && (faceType == LIZAN || faceType == DRACONIC)) 
				output += "snout";
			else output += "face";
		...
		public function isNaga():Boolean {
			if(legCount == 1 && legType == NAGA) return true;
			return false;
		}
		...
		public function isDrider():Boolean {
			return (legCount >= 4 && (legType == DRIDER || legType == ARACHNID));
		}
		public function isGoo():Boolean {
			if(legType == GOOEY) return true;
			return false;
		}
		public function legs(forceType:Boolean = false, forceAdjective:Boolean = false):String {
			var select:Number = 0;
			//EDGE CASSES
			if(legType == SNAKE) return "coils";
			else if(legType == GOOEY && hasLegFlag(AMORPHOUS)) {
				select = rand(5);
		...
		public function leg(forceType:Boolean = false, forceAdjective:Boolean = false):String {
			...
			//8 - goo shit
			if(legType == GOOEY && hasLegFlag(AMORPHOUS)) {
			...
			if(rand(4) == 0 || forceType) {
				...
				else if(legType == ARACHNID) {
					if(select <= 3) output += "chitinous";
					else if(select <= 5) output += "armored";
					else if(select <= 7) output += "insect-like";
					else output += "carapace-covered";
				}
				else if(legType == DRIDER) {
			...
			if (rand(3) == 0 || forceAdjective) {
				if(legCount > 2 && rand(2) == 0) {
					output += "numerous";
				}
				else if(hasLegFlag(DIGITIGRADE)) {
					output += "digitigrade";
				}
				else if(hasLegFlag(DIGITIGRADE)) {
					output += "plantigrade";
				}
				else if(hasLegFlag(SCALED)) {
					output += "scaled";
				}
				else if(hasLegFlag(FURRED)) {
					select = rand(10);
					if(select <= 3) output += "furry";
					else if(select <= 6) output += "fuzzy";
					else output += "fur-covered";
				}
				else if(hasLegFlag(TENDRIL)) {
					output += "wiggling";
				}
			}
			//NOUN IT UP BITCHES!
		...
		public function feet(forceType:Boolean = false,forceAdjective:Boolean = false):String {
			...
			//Noun
			if(output != "") output += " ";
			if(hasLegFlag(HOOVES)) output += "hooves";
			else if(hasLegFlag(PAWS) && rand(2) == 0) output += "paws";
			else if(hasLegFlag(AMORPHOUS) && legType == GOO) output += "cilia";
			else if(hasLegFlag(HEELS) && rand(2) == 0) output += "high-heels";
			else output += "feet";
			return output;
		}
		public function foot(forceType:Boolean = false,forceAdjective:Boolean = false):String {
			...
			//Noun
			if(output != "") output += " ";
			if(hasLegFlag(HOOVES)) output += "hoof";
			else if(hasLegFlag(PAWS) && rand(2) == 0) output += "paw";
			else if(hasLegFlag(AMORPHOUS) && legType == GOO) output += "undercarriage";
			else if(hasLegFlag(HEELS) && rand(2) == 0) output += "high-heel";
			else output += "foot";
			return output;
		}
		public function toes():String {
			...
			var output:String = "";
			//Noun
			if(hasLegFlag(HOOVES)) output += "hoof-tips";
			else if(hasLegFlag(PAWS) && rand(2) == 0) output += "fuzzy toes";
			else if(hasLegFlag(AMORPHOUS) && legType == GOO) output += "cilia";
			else if(hasLegFlag(HEELS) && rand(2) == 0) output += "toes";
			else output += "toes";
			return output;
		}
		public function knees():String {
			...
			var output:String = "";
			//Noun
			if(hasLegFlag(AMORPHOUS) && legType == GOO) output += "cilia";
			else output += "knees";
			return output;
		}
		...
		public function knockUp(hole:int,type:int = 0,incubation:int = 0,beat:int = 100,arg:int = 0):void {
			...
			if(type != 1 && type != 5 && type != 10) {
				if(hasTailFlag(OVIPOSITOR) && (tailType == ARACHNID || tailType == DRIDER || tailType == BEE)) {
		...
		public function canOvipositBee():Boolean {
			if(eggs >= 10 && hasTailFlag(OVIPOSITOR) && tailType == BEE) return true;
			return false;
		}
			...
			while(counter > 0) {
				...
				if(type == "area") {
					...
					if(cocks[counter].cLength <= fits) {
						...
						if(index >= 0) {
							//See if the newcomer beats the saved small guy
							if(cocks[counter].cLength > cocks[index].cLength)
								index = counter;							
						}
						//Store the index of fitting dick
						else index = counter;
		...
		public function hasLipples():Boolean {
			...
			while(counter > 0) {
				counter--;
				if(breastRows[counter].nippleType == LIPPLES) index = counter;
			}
		...
		public function hasNippleCocks():Boolean {
			...
			while(counter > 0) {
				counter--;
				if(breastRows[counter].nippleType == DICK) return true;
			}
			return false;
		}
		...
		//Change cock type
		public function shiftCock(slot:int = 0,type:int = EQUINE):void {
			...
			if(type == CANINE) {
				...
				cocks[0].addFlag(KNOTTED);
			}
			if(type == EQUINE) {
				cocks[slot].knotMultiplier = 1;
				cocks[0].addFlag(BLUNT);
				cocks[0].addFlag(FLARED);
			}			
		}
		//PC can fly?
		...
		public function ballsDescript(forceCount:Boolean = false,forceSize:Boolean = false,forceSingular:Boolean = false):String {
			...
			var rando:Number = 0;
			var desc:String = "";
			if(hasStatusEffect("Uniball") < 0 && (rand(4) == 0 || forceCount) && !forceSingular) {
				if(balls == 1) {
					rando = rand(4);
			...
			//Uniball
			if(hasStatusEffect("Uniball") >= 0 && rand(3) == 0) {
				if(descripted > 0) desc += ", ";
				rando == rand(4);
		...
		public function nippleDescript(rowNum:Number = 0,plural:Boolean = false):String {
			...
			if(rand(4) == 0 && breastRows[rowNum].nippleType != FUCKABLE && breastRows[rowNum].nippleType != FLAT && breastRows[rowNum].nippleType != INVERTED) {
				//TINAHHHH
			...
			if(rand(3) == 0 && descripted < 2) {
				...
				if(breastRows[rowNum].nippleType == FUCKABLE) {
				...
				//Lipples
				else if(breastRows[rowNum].nippleType == LIPPLES) {
					if(descripted > 0) description += ", ";
					if(isLactating()) {
						rando = rand(5);
				...
				//Dicknipples
				else if(breastRows[rowNum].nippleType == DICK) {
					if(descripted > 0) description += ", ";
					rando = rand(3);
				...
				//Flat
				else if(breastRows[rowNum].nippleType == FLAT) {
					if(descripted > 0) description += ", ";
					rando = rand(3);
				...
				//Inverted
				else if(breastRows[rowNum].nippleType == INVERTED) {
					if(descripted > 0) description += ", ";
			...
			else if(rand(3) == 0 && descripted < 2 && breastRows[rowNum].nippleType != FLAT && breastRows[rowNum].nippleType != INVERTED && breastRows[rowNum].nippleType != LIPPLES && breastRows[rowNum].nippleType != FUCKABLE) {
				if(lust() > 50 && lust() < 75) {
					if(descripted > 0) description += ", ";
					rando = rand(5);
		...
		public function hairDescript(forceLength:Boolean = false, forceColor:Boolean = false):String {
			...
			//Mane special stuff.
			if(skinType == 1 && hairLength > 3 && rand(2) == 0) {
				//Oddball shit
			...
			//Not manes
				...
				if(descripted > 0) descript += " ";
				if(hairType == 4 && rand(2) == 0) descript += "tentacle-hair";
				else if(hairType == 1 && rand(2) == 0) descript += "plumage";
				else descript += "hair";
			}
			return descript;
		}
		...
		public function clitDescript(pussy:Number = 0):String {
			...
			if(rand(3) == 0 && descripted < 2) {
				...
				if(skinType == FUR && hasVaginaType(VULPINE) && rand(3) <= 1) {
					if(descripted > 0) descript += ", ";
					descript += "foxy ";
					descripted++;
				}
			...
			if(hasVagina()) {
				if(vaginas[0].clitPierced > 0 && descripted < 2 && rand(3) < 2) {
					if(descripted > 0) descript += ", ";
					descript += "pierced";
					descripted++;
				}
			...
			if(skinType == FUR && hasVaginaType(CANINE) && rand(2) == 0) descript += "bitch-";
			//Horse descriptors - 50%
			else if(skinType == FUR && hasVaginaType(EQUINE) && rand(3) <= 0) descript += "mare-";
			randt = rand(10);
		...
		public function tailVaginaDescript(forceAdjectives:Boolean = false,adjectives:Boolean = true):String {
			...
			if(descripted < 2 && skinType == 3 && ((adjectives && rand(3) == 0) || forceAdjectives)) {
				if(descripted > 0) vag += ", ";
		...
		public function vaginaNoun(type:int = 0, simple:Boolean = false):String {
			...
			else if(type == SIREN || type == ANEMONE) {
				if(!simple) {
					temp = rand(16);
		...
		public function vaginasDescript():String {
			...
			if(vaginas.length > 1) return pluralize(vaginaDescript(rand(vaginas.length)));
		...
		public function cockNoun(type:Number, simple:Boolean = true, tail:Boolean = false):String {
			...
			else if(type == CANINE || type == VULPINE) {
				if(!simple) {
					rando = rand(11);
					if(rando <= 0 && type == CANINE) descript+="canine ";
		...
		public function cockAdjective(cockNum:Number = -1) {
			...
			//Length 1/3 chance
				...
				if(cocks[cockNum].cLength < 3) {
					...
					rando = rand(3);
					if(rando == 0) {
						if(cocks[cockNum].cType == EQUINE) descript = "pony-sized";
						else descript = "long";
					}
					else if(rando == 1) {
						if(cocks[cockNum].cType == EQUINE) descript = "colt-like";
					...
					if(rando == 0) descript = "huge";
						...
						if(cocks[cockNum].cType == CANINE) descript = "mastiff-like";
					...
					rando = rand(7);
					if(cocks[cockNum].cType == 4 && rand(4) == 0) descript = "coiled ";
					else {
		...
		public function statCockAdjective(l:int,w:int,type:int = 0) {
			...
			//Length 1/3 chance
				...
				if(l < 3) {
					...
					rando = rand(3);
					if(rando == 0) {
						if(type == EQUINE) descript = "pony-sized";
						else descript = "long";
					}
					else if(rando == 1) {
						if(type == EQUINE) descript = "colt-like";
						else descript = "lengthy";
					}
					else descript = "sizable";
				}
					...
					if(rando == 0) descript = "huge";
						...
						if(type == CANINE) descript = "mastiff-like";
						else descript = "cucumber-length";
					}
					else descript = "foot-long";
				}
					...
					rando = rand(7);
					if(type == 4 && rand(4) == 0) descript = "coiled ";
					else {
						if(rando == 0) descript = "towering";
						else if(rando == 1) descript = "freakish";
						else if(rando == 2) descript = "monstrous";
		...
		public function nippleCockAdjective(plural:Boolean = false) {
			...
			//Length 1/3 chance
				...
				if(l < 3) {
					...
					rando = rand(3);
					if(rando == 0) {
						if(dickNippleType == EQUINE) descript = "pony-sized";
						else descript = "long";
					}
					else if(rando == 1) {
						if(dickNippleType == EQUINE) descript = "colt-like";
						else descript = "lengthy";
					}
					else descript = "sizable";
				}
					...
					if(rando == 0) descript = "huge";
						...
						if(dickNippleType == CANINE) descript = "mastiff-like";
						else descript = "cucumber-length";
					}
					else descript = "foot-long";
				}
					...
					rando = rand(7);
					if(dickNippleType == 4 && rand(4) == 0) descript = "coiled ";
					else {
						if(rando == 0) descript = "towering";
						else if(rando == 1) descript = "freakish";
						else if(rando == 2) descript = "monstrous";
		...
		public function nippleCockDescript():String {
			...
			if(rand(10) <= 6) {
				...
				if(rand(2) == 0) descript += ", " + cockNoun(dickNippleType,false);
				//Otherwise normal
				else descript += " " + cockNoun(dickNippleType, false);
			}
			//These guys get a bonus adjective 70% of the time.
			else {
				if(rand(10) <= 6) descript += cockNoun(dickNippleType, false);
		...
		public function nippleCocksDescript(appearance:Boolean = false):String {
			...
			//These guys get a bonus adjective 70% of the time.
				...
				if(rand(10) <= 6 && !appearance) descript += cockNoun(dickNippleType, false);
				else descript += cockNoun(dickNippleType);
			}
		...
		public function cockDescript(cockNum:Number = 0):String {
			...
			if(cockNum != 99) {
				...
				if(rand(10) <= 6) {
					...
					//50% of the time add supplimental cock adjective with the noun.
					if(rand(2) == 0) descript += ", " + cockNoun(cocks[cockNum].cType,false);
					//Otherwise normal
					else descript += " " + cockNoun(cocks[cockNum].cType, false);
				}
				//These guys get a bonus adjective 70% of the time.
				else {
					if(rand(10) <= 6) descript += cockNoun(cocks[cockNum].cType, false);
			...
			//Boring descriptions. Mostly the same but kinda lame, actually.
				...
				//70% of the time add a descriptor
				if(rand(10) <= 6) descript += cockAdjective(-1) + " " + cockNoun(0);
		...
		public function tailCockDescript():String {
			...
			if(rand(10) <= 6) {
				...
				//50% of the time add supplimental cock adjective with the noun.
				if(rand(2) == 0) descript += ", " + cockNoun(tailGenitalArg,false,true);
				//Otherwise normal
				else descript += " " + cockNoun(tailGenitalArg, false,true);
			}
			//These guys get a bonus adjective 70% of the time.
			else {
				if(rand(10) <= 6) descript += cockNoun(tailGenitalArg, false, true);
		...
		public function cockHead(cockNum:Number = 0):String {
			...
			else if(type == NAGA || type == LIZAN || type == SNAKE) {
				temp = rand(6);
			...
			else if(type == ANEMONE || type == SIREN) {
				temp = rand(6);
