...
public function carrieBlowjobIntro():void
{
	...
	if(flags["CARRIE_BLOWJOBBED"] == undefined)
	{
		...
		output("\n\n<i>“Yeah,”</i> you grin. You can practically feel the pheromones pouring off of you, and you" + (pc.hasPerk("Pheromone Cloud") ? "r heady tunnel vision allows you to" : " can actually") + " see the effect they are having on the auburn-headed cow - the way her lips slowly part, her pupils dilating, her smile growing softer.");
	...
	else
	{
		...
		if(pc.hasPerk("Pheromone Cloud")) output(" This is one of the most wonderful things about the Treatment, you’ve found; you don’t have to say much to communicate with other Treated. And sometimes, you don’t have to say anything at all.");
...
public function carrieBlowjobsGo():void
{
	...
	if(flags["CARRIE_BLOWJOBBED"] == undefined)
	{
		...
		if(pc.hasPerk("Pheromone Cloud")) output(" You hazily wonder why you’re spending so much time and energy searching the stars when everything you could ever want is right here: a rampant bull’s true home.");
		...
		if(pc.cumQ() < 25) 
		{
			...
			if(pc.hasPerk("Pheromone Cloud")) output(" You grin down at her absurdly, feeling nothing but shining fondness and warmth for the auburn cock-polisher.");
		...
		if(pc.hasPerk("Pheromone Cloud")) output(" of cooperation");
		...
		if(pc.cocks[x].cLength() < 12 && pc.cumQ() < 25) output(" <i>“I don’t mind that y’all don’t cum like Moby Dick. A working gal always needs a snack, y’know?”</i>");
	...
	else if(flags["CORA_QUEUED"] == 1 && (pc.cocks[x].cLength() >= 12 || doubleDickEnough))
	{
		...
		if(!doubleDickEnough)
		{
			...
			if(pc.cumQ() >= 1500) output(" - things are going to get very messy, after all - ");
	...
	else
	{
		...
		if(pc.cumQ() < 500) 
		{
			...
			output("\n\n<i>“Mmm,”</i> she says happily, licking her lips. <i>“All for me. Deserve a reward for all the hard work I do with the milking machine, don’cha think?”</i>");
			if(pc.hasPerk("Pheromone Cloud")) output(" You grin down at her absurdly, feeling nothing but shining fondness and warmth for your auburn cock-polisher.");
			else output(" You grin down at the auburn cock-polisher, contentment throbbing through you.");
		}
		else if(pc.cumQ() < 1500)
		{
		...
		else
		{
			...
			output("\n\nShe laughs, half in disgusted exasperation and half in squealing delight, as with flexing grunts you paint her from top to bottom in musky slut-feed, paying particular attention to her plush boobs. She, and much of the nearby surroundings, look like they’ve been hit by a [pc.cum] bomb by the time you finally finish. You stand over her, breathing heavily, semi-tumescent cock dangling, gently oozing the kind of cum an unmodded human would ejaculate in a single orgasm. A plump, eager set of lips wicks your throbbing head and slit quite clean.");
			output("\n\n<i>“Wow. Ok,”</i> she says at last, scooching back and wiping [pc.cum] out of her eyes. <i>“Gonna have to schedule some shower time afterwards if I do this with you again, ain’t I?”</i>");
			if(pc.hasPerk("Pheromone Cloud")) output(" You grin down at her absurdly. Heavy need satiated, you feel nothing but shining fondness and warmth for the auburn cock-polisher.");
			else output(" Heavy need satiated, you grin down at the auburn cock-polisher, contentment throbbing through you.");
		}
...
public function carrieSoothingShowerGo(response:String = "intro"):void
{
	...
	switch(response)
	{
		case "intro":
			...
			output("\n\n<i>“I think you’ve given us enough for now, big [pc.guy],”</i> Carrie sighs. <i>“" + (pc.hasPerk("Pheromone Cloud") ? "Bull s" : "S") + "perm, it’s... not exactly in short supply around here, y’know? I had to dr- throw away some of th’ last batch.”</i>");
		...
		case "shower intro":
			...
			{
				if(pc.hasPerk("Pheromone Cloud")) output("You don’t have to say anything; all you have to do is go across, curl each arm around a warm, giggly cow-girl, breathe in their sweet, milky scent and give their plump butts a good, healthy squeeze. The Treatment makes everything so straightforward.");
		...
		case "shower fivesome p1":
			...
			if(pc.hasPerk("Pheromone Cloud")) output(" bull");
