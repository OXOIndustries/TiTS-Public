...
import classes.Characters.VarmintPet;
...
{
	if (chars["VARMINTPET"].short == "varmint") return false;
	return true;
}
...
public function varmintPetName(article:String):String
{
	if (!varmintRenamend())
	{
		if (article == "") return "varmint";
		return article + " varmint";
	}
...
public function varmintPetHeader():void
{
	...
	showName("\n"+ chars["VARMINTPET"].short.toUpperCase());
}
...
public function varmintOnShipBonus(btnSlot:int = 0):String
{
	...
	{
		bonusText += "\n\nYour pet varmint "+ (!varmintRenamend() ? "" : chars["VARMINTPET"].short + " ") +"is";
		...
		addButton(btnSlot, (!varmintRenamend() ? "Varmint" : chars["VARMINTPET"].short), approachPetVarmint, true, (!varmintRenamend() ? "Varmint Companion" : chars["VARMINTPET"].short), btnText);
	}
	return bonusText;
}
...
public function approachPetVarmint(introText:Boolean = false):void
{
	...
	if(introText)
	{
		if(rand(2) == 0)
		{
			...
			else output("\n\n<i>“Hey there, "+ (!varmintRenamend() ? "runt" : chars["VARMINTPET"].short) +"!”</i> you tease.");
	...
	if (varmintRenamend()) addDisabledButton(5, "Name it", "Name it", "To late to change the name, "+ chars["VARMINTPET"].short +" already got used to it.");
	else if(!pc.hasStatusEffect("Varmint Leashed")) addDisabledButton(5, "Name it", "Name it", "You need to leash the varmint before you can give it a name.");
	else addButton(5, "Name it", nameTheVarmint, undefined, "Name it", "Give your varmint a name.");
...
public function varmintNamingConventions(retFunc:Function = null):void
{
	...
	output("<i>“I think I am going to call you "+ chars["VARMINTPET"].short +" from now on, what do you think about that?”</i> you say, smiling at the big blue varmint next to you.");
	...
	output("\n\n<b>Your varmint is now named "+ chars["VARMINTPET"].short +"!</b>");
...
public function doVarmintPlayTime(response:String = "none"):void
{
	...
	{
		output("You go over to where "+ (!varmintRenamend() ? "the varmint’s" : chars["VARMINTPET"].short+ " is") +" sitting, thumping its massive tail on your deck. It hoots quietly as you approach, swishing its tail a little faster. It doesn’t stop you as you hook its collar and leash around its tree-trunk neck, though its spikes rise a bit as you lock it in place. The great big beasty takes a big, huffing breath as you activate its leash and stands up, sauntering over to stand at your side. Looks like it’s ready to go! ");
	...
	{
		output("You crouch down and unclip your leash from the great big blue varmint’s neck, giving it a pat on the haunch and sending it on its way. "+ (!varmintRenamend() ? "It" : chars["VARMINTPET"].short) +" makes a happy hooting sound and bounds down the corridor as fast as it can, ready to burn off some excess energy.");
