	...
	if(showID == "Personal" || showID == "All")
	{
		...
		if(pc.hasHorns())
		{
			...
			else
			{
				...
				else output2(" " + pc.horns + ", " + prettifyLength(pc.hornLength) + ", " + GLOBAL.TYPE_NAMES[pc.hornType]);
			}
		}
		// Body
		output2("\n<b><u>Body</u></b>");
		...
		if(pc.tailCount > 0)
		{
			...
			output2(" " + pc.tailCount + " " + GLOBAL.TYPE_NAMES[pc.tailType]);
		}
		...
		if(pc.breastRows.length > 0 || pc.hasNipples())
		{
			...
			if(pc.hasNipples())
			{
				...
				if(pc.canLactate()) output2(", Lactation Active");
		...
		if(pc.bellyRating() > 0 || pc.isPregnant())
		{
			...
			if(pc.isPregnant())
			{
				...
				{
					if(pc.hasVagina(x) && pc.pregnancyData[x].pregnancyType != "")
					{
						if(x != 3)
						{
							if(pc.pregnancyData.length == 2) output2("\n<b><u>Womb</u></b>");
...
public function questLogMenu(currentFunc:Function):Boolean
{
	...
	if(flags["UVETO_UNLOCKED"] != undefined)
	{
		...
		else addGhostButton(4, "Uveto", currentFunc, "Uveto");
	}
	// New Texas
	if(flags["NEW_TEXAS_COORDINATES_GAINED"] != undefined)
	{
...
public function displayQuestLog(showID:String = "All"):void
{
	...
	if(showID != "Other" || showID == "All")
	{
		...
		// Uveto
		if(flags["UVETO_UNLOCKED"] != undefined && (showID == "Uveto" || showID == "All"))
		{
			output2("\n<b><u>Uveto VII</u></b>");
			output2("\n<b>* Status:</b>");
...
public function displayEncounterLog(showID:String = "All"):void
{
	clearOutput2();
	clearGhostMenu();
	addGhostButton(14, "Back", statisticsScreen);
	...
	if(showID != "Other" || showID == "All")
	{
		...
		if(showID == "New Texas" || showID == "All")
		{
			...
			if(flags["MET_YAMMI"] != undefined)
			{
				...
				// Reaha special
				if(flags["REAHA_ICE_CREAM_DAYS"] != undefined) output2("\n<b>* Reaha, Days Since Last Had Ice Cream With: </b>" + flags["REAHA_ICE_CREAM_DAYS"]);
			...
			if(flags["BRYNN_AUTOENTER"] != undefined || flags["MET_GIANNA"] != undefined || flags["MET_MILLIE"] != undefined || flags["MET_YANCY"] != undefined || flags["GOBBLES_SEXYTIMES_STARTED"] != undefined || StatTracking.getStat("milkers/breast milker uses") > 0 || StatTracking.getStat("milkers/prostate milker uses") > 0)
			{
				...
				if(flags["MET_GIANNA"] != undefined)
				{
					...
					if(flags["BEEN_IN_GIANNA_BODYMOD_MENU"] != undefined)
					{
						...
						output2("\n<b>* Gianna, Body, Posterior Size: </b>" + prettifyLength(chars["GIANNA"].buttRating()));
					...
					if(flags["GIANNA_BODY_TALK_RESULT"] != undefined)
					{
						...
						{
							if(chars["GIANNA"].cockVirgin) output2("Virgin ");
						...
						if(chars["GIANNA"].hasVagina())
						{
							...
							if(chars["GIANNA"].vaginalVirgin) output2("Virgin ");
						...
						if(chars["GIANNA"].analVirgin) output2("Virgin ");
						output2("Asshole");
					}
					// Sex stuff
		...
		if(showID == "Mhen'ga" || showID == "All")
		{
			...
			if(flags["MET_PENNY"] != undefined)
			{
				...
				if(flags["SEXED_PENNY"] != undefined)
				{
					...
					{
						if(chars["PENNY"].cockVirgin) output2("Virgin ");
					...
					if(chars["PENNY"].hasVagina())
					{
						...
						if(chars["PENNY"].vaginalVirgin) output2("Virgin ");
					...
					if(chars["PENNY"].analVirgin) output2("Virgin ");
					output2("Asshole");
		...
		if(showID == "Tarkus" || showID == "All")
		{
			...
			{
				output2("\n<b><u>...?!</u></b>");
				if(flags["BESS_IN_RUBBLE"] != undefined) output2("\n<b>* [bess.name]:</b>");
				...
				if(flags["BESS_DISABLED"] != undefined)
				{
					...
					if(flags["BESS_DUMPED_INTO_SPAAAAAAACE"] != undefined) output2(", Dumped into space");
				}
				else output2(" Active");
				if(flags["BESS_CUMDUMP"] != undefined) output2(", Your personal cumdump");
				...
				if(!(chars["BESS"].meleeWeapon is EmptySlot)) output2("\n<b>* [bess.name], Weapon: </b>" + StringUtil.toTitleCase(chars["BESS"].meleeWeapon.shortName));
				// Affection/Relationship
				if(flags["BESS_AFFECTION"] != undefined) output2("\n<b>* [bess.name], Affection: </b>" + flags["BESS_AFFECTION"]);
				...
				if(9999 == 0)
				{
					output2("\n<b>* [bess.name], Body, Tone: </b>" + prettifyLength(chars["BESS"].tone + "/100"));
					output2("\n<b>* [bess.name], Body, Thickness: </b>" + prettifyLength(chars["BESS"].thickness + "/100"));
					...
					{
						output2("\n<b>* [bess.name], Body, Belly Size: </b>" + prettifyLength(chars["BESS"].bellyRating()));
						output2("\n<b>* [bess.name], Body, Hip Size: </b>" + prettifyLength(chars["BESS"].hipRating()));
						output2("\n<b>* [bess.name], Body, Posterior Size: </b>" + prettifyLength(chars["BESS"].buttRating()));
					}
					...
					if(chars["BESS"].canLactate()) output2(", Lactation Active");
					...
					{
						if(chars["BESS"].cockVirgin) output2("Virgin ");
						output2(GLOBAL.TYPE_NAMES[chars["BESS"].cocks[0].cType] + " cock");
					...
					{
						if(chars["BESS"].hasCock()) output2(", ");
						if(chars["BESS"].vaginalVirgin) output2("Virgin ");
						output2(GLOBAL.TYPE_NAMES[chars["BESS"].vaginas[0].type] + " vagina");
					...
					if(chars["BESS"].analVirgin) output2("Virgin ");
					output2("Asshole");
				}
				// Sex-related
			...
			if(flags["MET_EMBRY"] != undefined || flags["MET_LILIANA"] != undefined)
			{
				...
				if(flags["MET_EMBRY"] != undefined)
				{
					...
					if(flags["SEXED_EMBRY"] != undefined)
					{
						...
						{
							if(chars["EMBRY"].cockVirgin) output2("Virgin ");
							output2(GLOBAL.TYPE_NAMES[chars["EMBRY"].cocks[0].cType] + " cock");
						...
						if(chars["EMBRY"].hasVagina())
						{
							...
							if(chars["EMBRY"].vaginalVirgin) output2("Virgin ");
							output2(GLOBAL.TYPE_NAMES[chars["EMBRY"].vaginas[0].type] + " vagina");
						...
						if(chars["EMBRY"].analVirgin) output2("Virgin ");
						output2("Asshole");
	...
	if(showID == "Other" || showID == "All")
	{
		...
		if(flags["RESCUE KIRO FROM BLUEBALLS"] == 1 && flags["KIRO_BAR_MET"] != undefined)
		{
			...
			if(flags["KIRO_DRINKING_CONTEST_RESULTS"] != undefined)
			{
				...
				if(flags["KIRO_DRINKING_CONTEST_RESULTS"] == 0) output2(", Cheated");
			...
			if(flags["KIRO_FUCKED_DURING_RESCUE"] != undefined || flags["KIRO_DEEPTHROATED_YOU"] != undefined || flags["KIRO_BALLJOBBED_YOU"] != undefined || flags["KIRO_X_SAEN_HAPPENED"] != undefined || flags["PUMPED_KIRO_PUSSY"] != undefined)
			{
				...
				{
					if(chars["KIRO"].cockVirgin) output2("Virgin ");
					output2(GLOBAL.TYPE_NAMES[chars["KIRO"].cocks[0].cType] + " cock");
				...
				if(chars["KIRO"].hasVagina())
				{
					...
					if(chars["KIRO"].vaginalVirgin) output2("Virgin ");
					output2(GLOBAL.TYPE_NAMES[chars["KIRO"].vaginas[0].type] + " vagina");
				...
				if(chars["KIRO"].analVirgin) output2("Virgin ");
				output2("Asshole");
		...
		if(flags["FALL OF THE PHOENIX STATUS"] >= 1 || flags["SAEN MET AT THE BAR"] != undefined)
		{
			...
			if(flags["SAENDRA TIMES SEXED"] != undefined)
			{
				...
				{
					if(chars["SAENDRA"].cockVirgin) output2("Virgin ");
					output2(GLOBAL.TYPE_NAMES[chars["SAENDRA"].cocks[0].cType] + " cock");
				...
				if(chars["SAENDRA"].hasVagina())
				{
					...
					if(chars["SAENDRA"].vaginalVirgin) output2("Virgin ");
					output2(GLOBAL.TYPE_NAMES[chars["SAENDRA"].vaginas[0].type] + " vagina");
				...
				if(chars["SAENDRA"].analVirgin) output2("Virgin ");
				output2("Asshole");
