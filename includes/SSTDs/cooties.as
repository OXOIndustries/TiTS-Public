...
public function cootiesProcGoooo(deltaT:uint, maxEffectLength:uint, doOut:Boolean, target:Creature, effect:StorageClass):void
{
	...
	if(effect.value4 < 3 && effect.minutesLeft <= 0)
	{
		...
		//swell up
		else if(pc.puffiestVaginalPuffiness() < 1)
		{
			textBuff += "\n\n[pc.Walking] gets a whole lot harder - not because something goes wrong with your legs but because <b>your vaginal lips are plump and swollen</b>. It’s impossible to travel more than a few steps without noticing them rubbing against one another in a distractingly bothersome way!\n\nAt least <b>the Cooties have run their course</b>. You don’t need to worry about them transforming you any further.";
			for(var i:int = 0; i < pc.totalVaginas(); i++)
			{
				if(!pc.vaginalPuffiness(i) < 2) pc.inflateVagina(i); 
			}
		}
