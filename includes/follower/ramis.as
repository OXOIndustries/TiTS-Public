...
public function ramisTalkWork(disable:int):void
{
	...
	switch(flags["RAMIS_DRINKS_DRUNK"])
	{
		...
		case 2:
			...
			output("\n\n<i>“Took me a while to figure out the two were linked. Merc companies worken with peeps like the Black Void, runnen protection rackets, everyone gets a cut. You don’t get in on that shite until you’ve been worken with one crew or another for a while. I suppose I got too distracted to put two ‘n two together at the time.”</i> She smiles more warmly. <i>“Cuz you get to spend downtime on those distant stations ‘n mining platforms. And my word, are the worken boyos ‘n girlees happy to see anyone new. Oh, we had some amazen parties back then, drunker then I’ve ever been!");
			output("\n\n<i>“Spacewalks whilst half-cut, chucken conc grenades into zero grav minen shafts, tryen out the weirdest temporary TF combos we could think of... we were so lucky we didn’t kill ourselves,”</i> the kaithrit reminisces happily, leaning back on her chair. ”And, of course, I had the pick of the boyos. I’d have a fit lad on Saturday, a sissy on Sunday, back on board ‘n back to port on Monday, never hear from either again. What a life.”</i> She sips her drink. <i>“If the pay wasn’t so shite, and it wasn’t so bloody boren the rest of the time, I’d probably still be doin’ that.”</i>");
			break;
		case 3:
...
public function ramisMakeMeLastLongerYesLikeThat():void
{
	...
	if (!looksFemaleToRamis() && !pc.isTaur() && ramisOnTop() && pc.lust() > 30)
	{
...
public function ramisSleepButton():void
{
	if (flags["RAMIS_SEXED_SHIP"] == undefined) return;
	if (flags["CREWMEMBER_SLEEP_WITH"] != "RAMIS")
		addButton(4, "Sleep With", ramisSleepInvite, undefined, "Sleep With Ramis", "Invite her to sleep in your bed.");
	else
		addButton(4, "Kick Out", ramisSleepInvite, undefined, "Kick Out", "Tell her you’d rather sleep alone for now.");
}
...
public function ramisEnoughCatnapping():void
{
	...
	if (ramisOnTop())
	{
		...
		if (pc.biggestCockLength() < 6)
		{
			output("\n\nA minute later your wrists are being pressed firmly into the mattress, 120 kilograms of warm, brawny kaithrit is sat on your crotch, and you’re watching Ramis use your strainingly erect [pc.cockNoun] with slow, assertive thrusts of her hips. Your prick is overwhelmed in a cave of wet, shifting pressure, and you can’t stop moans and gasps being pushed out of your mouth, little stars exploding in your head.");
