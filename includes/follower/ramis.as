...
RAMIS_SEX_DISABLED: Ramis has told you she doesn't wanna shag (as you are).
RAMIS_PARTIED: Times gone drinking to Tavros as crewmember.
RAMIS_PARTIED_SEXED: Times it ended in sex
RAMIS_PARTIED_TRAPS
RAMIS_PARTIED_THRAGGEN

RAMIS_STRETCHED: Ever did Ramis with a dick bigger than her capacity?
...
stop being lazy and add real buttons to ramis's role switch scenenes <- no. am lazy.
...
make pcs feel bad about leaving tavros while follower!ramis is drinking. use status effect "partying ramis"
soreness wonky. actually look into how it works.
confirm tallnesses
stretching ramis doesn't change her capacity. ask nonesuch about this.
*/
...
public function ramisCrewApproach():void
{
    ...
    if (flags["RAMIS_SEXED_SHIP"] != undefined && !looksFemaleToRamis())
    {
...
public function ramisTalkBackground(disable:int):void
{
    ...
    if (flags["RAMIS_DRINKS_DRUNK"] < 2) output(" But… y’know.” Ramis taps a claw on the table surface, looking at her glass. “It is difficult when you’re a sprog, if nobody cares. The orphanage’n field staff <i>cared</i>, but they <i>cared</i> in the same way you care about a parcel you’re deliveren, y’know? You’re an item they’re processen through the system. And, once I got big, I had to be the tough one for all the smaller kids. I never… had anyone. I didn’t really know that, think about it as… somethen missen about me, until I met a few people with kids.”\n\nShe sniffs snottily, before hitting the glass back again.");
    else output("”");
...
public function ramisMakeMeLastLongerYesLikeThat():void
{
    ...
    if (!looksFemaleToRamis() && ramisOnTop() && pc.lust() > 30)
    {
...
public function ramisMakeMeStronger():void
{
    ...
    output("\n\n<i>“You done?”</i> grins Ramis. She’s on her back, medicine ball in hand. <i>“You start maken the real gains once you’re a bit knackered, you know… ");
...
public function ramisPushMahStrengthToTheLimit():void
{
    ...
    if (!looksFemaleToRamis() && !ramisOnTop() && pc.lust() > 30)
    {
        ...
        //Also random value
        if (pc.cumQ() < 1500) output(" her pussy is stuffed with your cum.");
...
public function ramisPlsSexMeh():void
{
    ...
    {
        output("\n\nRamis’s ears wilt.");
        output("\n\n“Aww,” she grouses. “You’re serious, aren’t you? I finally find a guy who’s a real man’n in the bedroom, ‘n then of course he turns round and asks for me to be in charge. Alright, fine.” She sighs, and narrows her eyes at you mischievously. “If that’s the way you want it to be, I’m goin’ to treat you bad. So bad you’ll want to put me in my place sooner rather then later.”");
    }
    else
    {
        output("“You want me to treat you bad?” Ramis grins. She prowls across, and you gasp as she grasps your [pc.butt], gives it a hard squeeze. “Well alright captain, if you insist. I have been thinken that you need someone to make you a bit less high’n mighty, you know…”");
    }
    ...
    else
    {
        ...
        else
        {
            if (flags["RAMIS_SEXED_SHIP"] == undefined)
            {
                ...
                //9999 tallnesses
                if (pc.tallness < 6*12 + 10) output("face");
                else if (pc.tallness < 7*12 + 6) output("[pc.chest]");
...
public function ramisCatsAndDoggystyle(continued:Boolean = false):void
{
    ...
    var tallBoyo:Boolean = pc.tallness > 6*12 + 5;
    ...
    if (tooBig)
    {
        output("You almost immediately have to withdraw your finger from her clit, the formidable girth of your cock stretching her entrance out, making it impossible to get at it. That’s fine, though -");
    ...
    {
        output("You keep fingering her, flicking away relentlessly at her clit, as you dig yourself further and further in, revelling in the dizzying tightness and wetness of it, sawing backwards and forwards a few times just so you can feel the way her walls tug at your withdrawing [pc.cockNoun]. Her tails lash at your face, uncontained excitement that finds voice as a husky yowl when your " + (pc.balls > 0 ? "[pc.balls]" : "thighs") + " bump against her inner thighs, your prick fully clothed in pussy pussy.");
    ...
    if (pc.tallness > 6*12 + 5) output(" by clutching her boob for support, teasing at a small nipple between your fingers to intensify her ear-ringing squeals.");
    ...
    if (pc.cumQ() > 9999) output(" your [pc.cockBiggest] is achingly dry, [pc.cumVisc] fluid oozing out around it from her thoroughly fucked hole.");
...
public function ramisAndTheWondrousHolodick():void
{
    ...
    if (flags["RAMIS_SEXED_STRAP"] == undefined)
    {
        ...
        if (pc.buttChange(150)) output("\n\n");

        output("<i>“Unf. Oh! Sorry petal, was that too fast?”</i> Ramis’s tone shifts from wickedness to concern. The pressure immediately withdraws, and an embarrassed hand squeezes your shoulder. <i>“I’m sorry. I just sort of did it how I’d want a boyo to do me, rough like…”</i>");
        ...
        output("\n\n[pc.eachCock] throbs and twinges, unable to come to orgasm, but as taut, thick, amazonian muscle continues to beat against your rear [pc.cum] begins to " + (pc.cumQ() < 9999 ? "leak slowly" : "drool") + " from your dilated [pc.cockHead], your stimulated by the beat of that hard-light rod against your buried, oh-so sensitive " + (pc.isFemboy() ? "boi" : "boy") + " buzzer.");
    ...
    else
    {
        ...
        else if (flags["RAMIS_SEXED_STRAP"] >= 2 && pc.biggestCockLength() < 6)
        {
            ...
            if (flags["RAMIS_PARTIED_THRAGGEN"] != undefined) output("She looks at you with a slightly wilder expression on her face, and you blush even harder, realizing you’re both thinking about the same thing. <i>“You should wear it when we’re with the big lad. Mmm... I’d love to hear that thing jingling away whilst he’s shagging me and your pretty lips are wrapped around his bollocks.”</i>\n\n");
...
public function ramisFucksABoiWithHisStraponPartIII():void
{
    ...
    if (pc.balls > 0 && pc.ballSize() < 9999)
    {
    ...
    if (pc.cumQ() > 9999) output(" The sheets are dappled [pc.cumColor] with what’s been mercilessly milked out of you.");
...
public function ramisCorridorMolesting():void
{
    ...
    if (pc.tallness > 9999) output(" face");
    ...
    output("\n\nShe looks");
    if (pc.tallness < 9999) output(" down");
    ...
    {
    //9999 {wat}
        output("\n\n<i>“Why did you make it so fucken big?”</i> she wonders. She has to really work to cover the entirety of your massive cock, masting well out of your underwear. Muscles bulge in her arm as she thrusts her palm up and down its veiny underside. <i>“This thing’s no use to me. Just a big, ugly trophy on my {big / little}, {ugly / pretty} boy.”</i>");
...
public function ramisBathingCats(from:String = "none"):void
{
    ...
    else
    {
        ...
        //9999 ask nonesuch about condition
...
public function ramisBottomShowerEnd():void
{
    ...
    if (!ramisWouldBottom() /* 9999 */) output(" Perhaps you cud look into getten a willy bigger'n then a girl's finger, so you ken do this for someone else sometime, hmm?");
    output(" Keep your hands still now...\"</i>");

    output("\n\nGasp after moan after gasp is pushing out of your lungs into the humid air as Ramis milks your prostate with an insistent beckoning motion of her finger, driving over that tender bulb within your [pc.ass], all the while her tight grip flicks up and down your [pc.cock]. No bitch boi in the galaxy could last long against such treatment, and you are swiftly driven to a coruscating orgasm, swelling up in the kaithrit's hand and " + (pc.cumQ() > 9999 ? "squirting" : "pasting") + " [pc.cum] across the shower wall between your quivering hands. She ruthlessly works you, pressuring your clenching boy buzzer and kneading your jolting cock until you are flexing dryly");
...
public function ramisLetsGoBowling():void
{
    ...
    output("\n\n<i>“Aw, you’re the best bloody captain!” She gives you a light, playful punch and returns to her make-up table. <i>“You go fix yourselve up. I’ll be ready in five minutes.”</i>");
    
    //Time forward 1 hour, move to square outside ship
    output("\n\nAn hour later the two of you are striding towards the hangar elevator that will bear you to the Merchant Deck, a choking cloud of " + ramisfmt("perfume", "perfume and aftershave", "perfume") /*9999*/+ " left in your wake.");
...
public function ramisTheresATrapForYouAndMe():void
{
    ...
    if (flags["RAMIS_PARTIED_TRAPS"] == undefined)
    {
        ...
        output("\n\n<i>“For sure,” grins Kroy, getting up. He’s just about tall enough for Ramis to support her boobs on his head. <i>“Vanesse won’t come unless I come. And you want him to come. He’s freaky as <i>hell</i> once you coax him out of his cocoon.”</i>");
...
public function ramisFurtherBoiDrilling(shagType:String):void
{
    ...
    if (shagType == "cock")
    {
        ...
        output("<i>“Yeah,”</i> groans Ramis, watching all of this with leonine lust writ large on her face, breasts bouncing up and down. <i>“Give it to him good. YES!”</i>");
        output("\n\nYou think she actually cums herself, thick thighs tightening around Kroy’s waspy waist, just watching you spear your [pc.cock] into Vanesse’s ass-pussy, the saeri emitting one quiet ‘oh’ as you pressure his sphincter and then a much louder, shocked ‘Ohh!”</i> when you spread it wide, gloriously enveloping your tender hardness in his warm, tight innards. Innards that clench and shiver delightfully as you begin to saw yourself into them, intent on turning that pert, round bum into a mold of your cock, its only purpose to pleasure you.");
    ...
    else
    {
        ...
        if (pc.holeChange(0, ramisBoiCockVol)) output("\n\n");
        
        output("You flick at your [pc.clit], pleasuring yourself as you grit your teeth and begin to firmly fuck him, squishing his tiny penis as hard as you can");
...
public function ramisAftermathOfBoimageddon():void
{
    ...
    if (pc.libido() < 50)
    {
        ...
        if (pc.tallness > 6*12 + 5 /*9999*/) output(" (he needs to clamber onto the sink to do it)");
    ...
    else if (pc.hasCock())
    {
        ...
        if (pc.tallness > 6*12 + 5 /*9999*/) output(" (he needs to climb onto the sink to do it)");
        output(".\n\n<i>“You two were sooo good. I love peeps who can show Vanesse how much great sex you can score if you embrace being a boy bitch. He’s too shy, y’know?” A lush moan emanates from the shower, and Kroy giggles as he leaves. <i>“Find us again, won’t you?”</i>");
    }
    else
    {
        output("\n\nWhat a silly question.");
...
//Space Orc
public var ohrskunsDickVol:int = 200;
...
public function ramisOrcAndGirlees():void
{
    ...
    if (pc.tallness > 4*12 + 4) output(", getting on your knees as you come.");
    ...
    if (pc.hasLowerGarment()) output(" Your [pc.lowerGarments] are similarly ripped away with two impatient yanks.");
    ...
    if (pc.holeChange(0, ohrskunsDickVol)) output("\n\n");
    ...
    if (tight) output(" Your captain is a really tight fuck. Did you know that?”</i>");
    else output(" Your captain can take someone the size of me like a trained whore. Did you know that?”</i>");
...
public function ramisTheOrcIsStillHarderThanNamingFunctions():void
{
    ...
    output("\n\n<i>“You’re such a good sport, captain, I’m so glad you’re here,” she slurs, feline eyes focusing upon you. <i>“Sharen is what bein’ bezzies is all about!” She raises her hand. You can only hi-five her, laughing together at the complete ridiculousness of it.");
...
public function ramisOrcAndTrapbois():void
{
    ...
    if (pc.buttChange(ohrskunsDickVol)) output("\n\n");
...
public function ramisTrapsAndTheirNaturalPredator():void
{
    ...
    output("\n\n<i>“Oh yeah?” the kaithrit leers, shifting on the quivering bed, tracing the line of her big, supple breasts. <i>“Like to see that, if you’ve got the guts to match your gob.”</i>");
...
public function ramisOrcTablesAndTurns():void
{
    ...
    if (pc.biggestCockLength() < 6)
    {
        ...
        output("\n\n<i>“Sounds like puss is enjoying herself,”</i> Ohrskun laughs. The slap he lands on Ramis’s ass reverberates right through to you. <i>“Are you, puss? You want some more?”</i> <i>“Yes!”</i> cries your gunnery officer deliriously. <i>“Keep doen it like that, you fucken machine! Yesssss!”</i>");
    ...
    else
    {
        ...
        output("\n\n<i>“Did you enjoy that, big boy?”</i> you ask him teasingly, watching him as he repositions Ramis, gets her on her hands and knees, and then starts doing her doggy style. " + StringUtil.capitalize(pc.cumColor()) + " juices ooze down his pistoning thighs.");
