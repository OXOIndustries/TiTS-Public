...
public function loreleiRandomEncounter():Boolean
{
	...
	else if (pc.reflexes() < 25) loreleisAmbushGotFoiled();
	else if (flags["SUBMITTED_TO_LORELEI"] != 3) loreleiIntroducesHerselfToSlowPCs();
...
public function loreleiMakesAnOfferYouProbablyDontWantToRefuse():void
{
	...
	output("\n\nSuddenly, the needle pressing into you withdraws, and she twirls it between her fingers skilfully and playfully as she continues. <i>“I know all about your little adventure to find your daddy’s beacons, so I knew you were here for it. The effort you went through to keep it a secret was commendable, but I</i> do <i>so love a challenge: once you and your cousin logged in with the harbormaster at Tarkus");
...
public function loreleiTellsYouWhereSheGotHerSuperSuit():void
{
	...
	if (silly) output(" <i>“I’ve taken my share of mudbaths before, but</i> tar? <i>You’d catch me dead and cold before I’d ever put muck like that on my body!”</i>");
	else output(" <i>“Who could even make up something so silly? As if anyone would think something like a ‘symbiote’ made by some God could ever exist in reality!”</i>");
...
public function loreleiEngagesInADifferentTypeOfIllicitAct():void
{
	...
	if (isLoreleisBitch()) output(" <i>“I appreciate the compliment, darling, but we’re past the pleasantries.”</i>");
...
public function loreleiHasStolenYourGenitaliaOhNo(genIdx:int, vag:Boolean):void
{
	...
	if (tooBig)
	{
		...
		output("\n\n<i>“But I’ll be damned if I’ll let that stop us,”</i> she continues, then leans forward to lift her lower body off you. With her left hand, she fishes for your dick, grabbing it by the [pc.base <i>\" + genIdx + \"</i>] and shifting it forward, sliding it between your hot, wet bodies until it’s up against her stomach.");
	...
	else if (tooBig) output(" grinding your cock into the yourself, eager to know what it feels like to straddle your massive log as it unloads gout after gout of cum onto your chest.");
...
public function loreleiShouldntTalkWithHerMouthFull(genIdx:int, vag:Boolean):void
{
	...
	output("\n\nLoreley beckons her finger towards you. <i>“" + (isLoreleisBitch() ? "It is your turn to satisfy your mistress" : "Be a good lover, [pc.name]") + ",”</i> she says.");
...
public function loreleisGreatestTreasureRevealed(apologizing:Boolean):void
{
	...
	else output(" Lorelei is naked:Her eyes normally stay on yours, her lips pursed at the sight of you working so diligently up her body, but occasionally, she distracts herself with the glitter of the gems in the walls.");
	output("\n\nYour keep your right hand against her stretched leg, gently drawing your fingernails along her skin, feeling how smooth it is, while your left becomes more adventurous, reaching");
...
public function loreleisFutaSexFinallyBegins():void
{
	...
	if (pc.cuntThatFits(2*lorelei.biggestCockVolume()) < 0 || 2*lorelei.biggestCockVolume() > pc.analCapacity()) addDisabledButton(1, "Get Fucked", "Get Fucked", "Lorelei’s too big for you to take. <b>If you want to take a monster like this into yourself, maybe you should look into increasing your capacity.</b>");
	else addButton(1, "Get Fucked", vaginaRouter, [loreleiHasAGoAtStealingYourVirginity, lorelei.biggestCockVolume(), (pc.hasVagina() ? 0 : -1), 0], "Get Fucked", (lorelei.cLength() < 24 ? "Lorelei’s grown a cock, just for you. It wouldn’t be right to ask her to go through the trouble and then <i>not</i> give it the attention it needs…" : "You can’t help but shiver at the sight of this <i>beast</i> between Lorelei’s legs. You <i>need</i> to feel it inside you…!"));
...
public function loreleiAreYouSureArtTastesLikeCum():void
{
	...
	output(" with each downward suck, and you take in more with each new, bold bob down her shaft. Lorelei twitches under your skilful work on her body, and she can’t help but put a hand on your hand and clench her legs in pleasure, locking you to her body.");
...
public function loreleiHasAGoAtStealingYourVirginity(holeIdx:int):void
{
	...
	if (pc.hasCock())
	{
		output("[pc.cock] is standing at attention between you two, erect as could be, your own [pc.preCum] flinging back and forth with the metronome-like swaying of your meat as Lorelei pounds into you again and again");
