...
public function ratsBoyStealsCockIntoHisButt(cockId:int, rounds:int):void
{
	...
	{
		output(", your");
		if (pc.hasStatusEffect("Blue Balls") || pc.cumQ() >= 500 || pc.ballSize() >= 16) output(" packed and");
...
public function ratsEnoughRatFuckingForNow(rounds:int):void
{
	...
	switch (rounds)
	{
		...
		case 6:
			...
			if (ratsPCIsGood()) output(" <i>\"You gotta meet our boss as soon as possible. When you get upstairs you're gonna stay with me in my bed for a day! I'll be thinkin' about " + (pc.hasCock() ? "your dick" : "you") + " until until you come 'n ravage me real proper, 'kay?\"</i>");
...
public function ratsRatsRatsStackedStackedStacked():void
{
	...
	if (pc.balls > 0) output(" The mini-climaxes either forcibly suck or gently coax the " + (pc.ballSize() < 8 ? "wobbling" : "tremendously weighty") + (pc.balls > 1 ? "nuts" : "nut") + " in your ballsack to offer up greater loads of pre and seed. They're so tight" + (pc.ballSize() < 8 ? "" : " and heavy") + " that the batch you're brewing seems like expulsion'll cause a permanent deleterious effect on your sanity when you cum."); 
