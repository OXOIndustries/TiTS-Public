...
public function boredJumperPregEncounterGoWithRestRvsCowgirlNext(arg:Array):void
{
	...
	if (pc.hasCock()) 
	{
		...
		if (pc.cumQ() >= 2000) output(" puddles");
		else if (pc.cumQ() >= 1000) output(" waves");
		else output(" ropes");
...
public function boredJumperPregEncounterGoWithRestSneakBBNext2(jumperType:int=0):void
{
	...
	if (pc.vaginas[hole].hasFlag(GLOBAL.TYPE_SIREN) || pc.vaginas[hole].hasFlag(GLOBAL.TYPE_ANEMONE)) output(", her intruding organ being stung by every tendril it disturbs until she's as wet as your overaroused passage");
	output(".");
...
public function boredJumperPregEncounterGoWithRestSneakBBNext3(arg:Array):void
{
	...
	if (pc.vaginas[hole].hasFlag(GLOBAL.TYPE_SIREN) || pc.vaginas[hole].hasFlag(GLOBAL.TYPE_ANEMONE)) output(" A shake, and a tiny laugh when her overwhelmingly erect member is stung by your lust-bearing cilia.");
	
	output(" <i>“Reeaallly?”</i> The Jumper pulls back slightly");
	if (pc.vaginas[hole].hasFlag(GLOBAL.TYPE_SIREN) || pc.vaginas[hole].hasFlag(GLOBAL.TYPE_ANEMONE)) output(", wincing,");
