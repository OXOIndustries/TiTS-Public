...
public function willowSexMenu():void
{
	...
	if (flags["WILLOW_HAS_GRAV_CUFFS"] == 1||pc.hasItemByClass(GravCuffs))
	{
		...
		else if (flags["WILLOW_BUTT_SEXED"] ==undefined) addDisabledButton(2,"DblPen","DplPen","Willow can't be an anal virgin.");
		else if (flags["WILLOW_AFFECTION"] < 20) addDisabledButton(2,"DblPen","DplPen","Willow hasn't gained enough affection for you.");
		else if (!pc.hasCock())addDisabledButton(2,"DblPen","DplPen","You must have at least one front cock.");
		else if (pc.hasTailCock()&&pc.hasParasiteTail())addDisabledButton(2,"DblPen","DplPen","You can't have a parasitic tailcock.");
		else if (!pc.hasCocks())addDisabledButton(2,"DblPen","DplPen","Get another cock or get a tailcock.");
		...
		else if ((flags["WILLOW_VAG_SEXED"]<3)||(flags["WILLOW_VAG_SEXED"]==undefined)) addDisabledButton(3,"DblStuffVag","DblStuffVag","Willow's pussy isn't used to your cock yet.");
		else if (flags["WILLOW_AFFECTION"] < 40) addDisabledButton(3,"DblStuffVag","DblStuffVag","Willow hasn't gained enough affection for you.");
		else if (!pc.hasCock())addDisabledButton(3,"DblStuffVag","DblStuffVag","You must have at least one front cock.");
		else if (pc.hasTailCock()&&pc.hasParasiteTail())addDisabledButton(3,"DblStuffVag","DblStuffVag","You can't have a parasitic tailcock.");
		else if (!pc.hasCocks())addDisabledButton(3,"DblStuffVag","DblStuffVag","Get another cock or get a tailcock.");
	}
	else
	{
		...
		else if (flags["WILLOW_BUTT_SEXED"] ==undefined) addDisabledButton(2,"DblPen","DblPen","Willow can't be an anal virgin.");
		else if (flags["WILLOW_AFFECTION"] < 20) addDisabledButton(2,"DblPen","DblPen","Willow hasn't gained enough affection for you.");
		else if (!pc.hasCock())addDisabledButton(2,"DblPen","DblPen","You must have at least one front cock.");
		else if (pc.hasTailCock()&&pc.hasParasiteTail())addDisabledButton(2,"DblPen","DblPen","You can't have a parasitic tailcock.");
		else if (!pc.hasCocks())addDisabledButton(2,"DblPen","DblPen","Get another cock or get a tailcock.");
	...
	else if ((flags["WILLOW_DBLPEN_VAG"]<3)||(flags["WILLOW_DBLPEN_VAG"]==undefined)) addDisabledButton(4,"TrplStuffVag","TrplStuffVag","Willow's pussy still needs to get used to your two cocks.");
	else if (flags["WILLOW_AFFECTION"] < 70) addDisabledButton(4,"TrplStuffVag","TrplStuffVag","Willow hasn't gained enough affection for you.");
	else if (!pc.hasCock())addDisabledButton(4,"TrplStuffVag","TrplStuffVag","You must have at least one front cock.");
	else if (pc.hasTailCock()&&pc.hasParasiteTail())addDisabledButton(4,"TrplStuffVag","TrplStuffVag","You can't have a parasitic tailcock.");
	else if (!pc.hasCocks())addDisabledButton(4,"TrplStuffVag","TrplStuffVag","Get more cocks or tailcocks.");
...
{	
	...
	if (flags["WILLOW_HAS_GRAV_CUFFS"]==undefined && sceneChoice == (1||5||9||11)) 
	{
	...
	if (sceneChoice==0)
	{
		...
		if (pc.hasCocks()) output("Your other cock slides over her butt, rubbing between the soft cheeks of her ass and against her twitching asshole.");
		...
		if (pc.hasCock()) output("sensitive flesh.");
		...
		if (pc.hasCock())
		{
			...
			if (pc.hasKnot())output("Your knot swells up in preparation and you brutally fuck the knot into her, Willow screaming in pain and pleasure.");
			output("Your dick spurts ropes and ropes of [pc.cumColor] cum into her womb"); 
			if (pc.hasCocks()) output(" and over her back");
			output(", filling it up so much that Willow simply can’t hold all of it in, ");
			if (pc.hasKnot())output("but your swollen knot ensures that it stays there.");
	...
	else if (sceneChoice==2)
	{
		...
		if (pc.hasCocks()) output("Your other "+ cock2 +" slides up through the crack of her ass, the soft flesh serving to provide you with a buttjob.");
		...
		if (pc.hasCock())
		{
			...
			if(pc.hasKnot())output("your knot swelling, ");
			output ("ready to pack her full. After a few hard slams, ");
			if (pc.hasKnot())output("you roughly fuck the knot into her, making her scream out with pain and pleasure at the sudden stretching. ");
			else output ("you bury yourself deep in her and cum, spraying rope after rope of cum into her. ");
			output ("Your other "+ cock2 +" sprays its load over her back. ")
			output ("There’s so much, ");
			if (pc.hasKnot())output("but your knot ensures that nothing flows back out, keeping her pussy filled with cum.");
		...
		{
			output("Your ");
			if (pc.hasCocks()) output("[pc.cocks] stiffen");
	...
	else if (sceneChoice==3)
	{
		...
		if (pc.hasCocks()) output(" Your other "+ cock2 +" rubs over her clit, stimulating her even further.");
		...
		if (pc.hasCock())
		{
			...
			if (pc.hasKnot())output("ramming the knot in");
	...
	else if (sceneChoice==4)
	{
		...
		if (pc.hasCock()) output("Your [pc.cocks] start to stiffen merely from the thought of pounding this tight ass. ");
		...
		output(" for its cum");
		if (!pc.hasCock()) output(", which, unfortunately, you don’t have");
		...
		if (pc.hasKnot())output("fucking your swollen knots through her tight sphincter and earning another loud scream of pure bliss, ");
		...
		if (pc.hasKnot())output("your knot packing it in tight.");
		...
		if (pc.hasCock()) output("mixing with the cum on your [pc.lowerBody].");
		else
		{
		...
		if (pc.hasCock()) output("and rubs her swollen belly. <i>“Mm… So full… Thank you, [pc.master]…”</i> She murmurs");
	...
	else if (sceneChoice==6)
	{
		...
		if (pc.hasCock())output("and fill it with your seed!");
		...
		if (pc.hasCock())
		{
			...
			if (pc.hasKnot())output(" spearing your knot into her, stretching her wide and");
		...
		if (pc.hasCock())output(",”</i> she says as she rubs her belly. <i>“So full and warm…”</i> She murmurs, rubbing her head back against you.");
		else output(".”</i>");
	...
	else if (sceneChoice==7)
	{
		...
		if (pc.hasKnot())output("your knot starting to swell, ");
		...
		if (pc.hasCock())
		{
			...
			if (pc.hasKnot())output("burying your knot in as well, earning a loud scream of pleasure and ");
	...
	else if (sceneChoice==10)
	{
		...
		if (pc.hasCocks())output("b-both of those[pc.cocks]");
	...
	else if (sceneChoice==11)
	{
		...
		if (pc.hasCocks())output("other " + cock2 + "");
	...
	else if (sceneChoice==12)
	{
		...
		if (pc.hasCocks())output("[pc.biggestCock] and "+ cock2 +"");
		else output("[pc.cock] and [pc.tailCock]");
...
public function willowWearClothes(sceneChoice:int):void
{
	...
	if (sceneChoice == 0)
	{
		...
		//Wear Clothes Menu
		if (flags["WILLOW_MAID_OUTFIT"] == 0) addButton(0,"MaidO.",willowWearClothes,1,"MaidO.","Have Willow wear the Maid Outfit.");
		else if (flags["WILLOW_MAID_OUTFIT"] == (1||2)) addDisabledButton(0,"MaidO.","MaidO.","Willow is already wearing the Maid Outfit.");
		else addDisabledButton(0,"MaidO.","MaidO.","You haven't given her a Maid Outfit.");
				
		if (flags["WILLOW_LEATHER_OUTFIT"] == 0) addButton(1,"L.Harness",willowWearClothes,2,"L.Harness","Have Willow wear the Leather Harness.");
		else if (flags["WILLOW_LEATHER_OUTFIT"] == 1) addDisabledButton(1,"L.Harness","L.Harness","Willow is already wearing the Leather Harness.");
		else addDisabledButton(1,"L.Harness","L.Harness","You haven't given her a Leather Harness.");
		
		if (flags["WILLOW_LATEX_OUTFIT"] == 0) addButton(2,"LtxBdy",willowWearClothes,3,"LtxBdy","Have Willow wear the Latex Outfit.");
		else if (flags["WILLOW_LATEX_OUTFIT"] == 1) addDisabledButton(2,"LtxBdy","LtxBdy","Willow is already wearing the Latex Outfit.");
		else addDisabledButton(2,"LtxBdy","LtxBdy","You haven't given her a Latex Outfit.");
		
		if (flags["WILLOW_PIRATE_OUTFIT"] == 0) addButton(3,"S.Pirate O.",willowWearClothes,4,"S.Pirate O.","Have Willow wear the Space Pirate Outfit.");
		else if (flags["WILLOW_PIRATE_OUTFIT"] == 1) addDisabledButton(3,"S.Pirate O.","S.Pirate O.","Willow is already wearing the Space Pirate Outfit.");
		else addDisabledButton(3,"S.Pirate O.","S.Pirate O.","You haven't given her a Space Pirate Outfit.");
		
		if (flags["WILLOW_CHEERLEADER_OUTFIT"] == 0) addButton(4,"Chr.Ldr.",willowWearClothes,5,"Chr.Ldr.","Have Willow wear the Cheerleader Uniform.");
		else if (flags["WILLOW_CHEERLEADER_OUTFIT"] == 1) addDisabledButton(4,"Chr.Ldr.","Chr.Ldr.","Willow is already wearing the Cheerleader Uniform.");
		else addDisabledButton(4,"Chr.Ldr.","Chr.Ldr.","You haven't given her a Cheerleader Uniform.");
		
		if (flags["WILLOW_BUNNY_OUTFIT"] == 0) addButton(5,"Bunny O.",willowWearClothes,6,"Bunny O.","Have Willow wear the Bunny Outfit.");
		else if (flags["WILLOW_BUNNY_OUTFIT"] == 1) addDisabledButton(5,"Bunny O.","Bunny O.","Willow is already wearing the Bunny Outfit.");
		else addDisabledButton(5,"Bunny O.","Bunny O.","You haven't given her a Bunny Outfit.");
		
		if (flags["WILLOW_SCHOOL_OUTFIT"] == 0) addButton(6,"S.Girl O.",willowWearClothes,7,"S.Girl O.","Have Willow wear the Schoolgirl Outfit.");
		else if (flags["WILLOW_SCHOOL_OUTFIT"] == 1) addDisabledButton(6,"S.Girl O.","S.Girl O.","Willow is already wearing the Schoolgirl Outfit.");
		else addDisabledButton(6,"S.Girl O.","S.Girl O.","You haven't given her a Schoolgirl Outfit.");
	...
	else
	{
		...
		//Flags reset
		if (sceneChoice!=(8||9))flags["WILLOW_MAID_OUTFIT"] = 0;
