...
public function loseToKorgiGirlRouter():void
{
	...
	else if(pc.hasCock()) loseToKorgonneFemaleIfHasCock();
...
public function loseToFemKorgonneNoDick():void
{
	...
	if(korgiTranslate()) output("<i>“You explore planet first. Now you explore korgonne.”</i> ");
...
public function korgiNoDickPart2Loss():void
{
	...
	if(korgiTranslate()) output("<i>“Inside,”</i> she instructs, impatiently. <i>“Korgonne joy inside body!”</i> ");
...
public function defeatDaKorgiShit():void
{
	...
	//[Tit Fuck]
	//Needs cock
	if(pc.hasCock()) addButton(1,"Tit Fuck",titfuckKorgonneWin,undefined,"Tit Fuck","A tightly bound chest like hers would be perfect for people with dicks.");
	else addDisabledButton(1,"Tit Fuck","Tit Fuck","You need a penis to tit-fuck her.");

	//[Ride Her Tongue]
	if(pc.hasVagina()) addButton(2,"Ride Tongue",rideTheKorgiTongue,undefined,"Ride Tongue","That long tongue of hers would probably feel great if you had a pussy!");
	else addDisabledButton(2,"Ride Tongue","Ride Tongue","You need a vagina in order to ride her tongue.");

	//[Let Her Lead]
	//Needs fittable dick
	if(pc.hasCock() && pc.cockThatFits(enemy.analCapacity()) >= 0) addButton(3,"Let Her Lead",letDatKorgiLeadWinsex,undefined,"Let Her Lead","You’ll need a dick if you want her to ride you.");
	else addDisabledButton(3,"Let Her Lead","Let Her Lead","You need a dick that will fit in her ass for this.");
...
public function sitOnKorgiForTaurs():void
{
	...
	else if(korgiTranslate()) output(" <i>“Is no time for rest,”</i> she whines.");
...
public function taurSitOnKorgi2():void
{
	...
	output(" Gasping, she licks harder, her tongue encircling the fleshy folds of your backside. Growing more confident with each circuit, the girl beneath you traces her licking path teasingly across the tender ingress of your [pc.asshole]. Leaning back to press the full weight of your [pc.ass] against her face, your [pc.chest] rises with the sharp sensitivity of your saliva-glossed posterior.");
...
public function letDatKorgiLeadWinsex():void
{
	...
	if(!korgiTranslate()) output("making fervid oaths whose meanings are all too clear, despite the fruitlessness of the individual words.");
	//translated: 
	...
	if(!korgiTranslate()) output("<i>“Fuck alien,”</i> she says, with a frisky, approving growl.");
...
public function rideTheKorgiTongue():void
{
	...
	if(korgiTranslate()) output("<i>“Strong mouth,”</i> she praises, one expert to another. ");
...
public function titfuckKorgonneWin():void
{
	...
	if(korgiTranslate()) output("<i>“But- but alien taste good!”</i> she whines. ");
...
public function titFuckKorgiWinPart2(x:int):void
{
	...
	if(pc.cocks[x].cLength < 12) output(StringUtil.capitalize(num2Text(Math.round(pc.cocks[x].cLength()))));
	else output("Twelve");
	...
	output("\n\nWhen she’s finally clean, ");
	if(!pc.isAss()) output("you give her freshly-plumped belly an affectionate pat and ");
...
public function pumpKorgiPuss():void
{
	...
	if(korgiTranslate()) output("<i>“Alien bold. You claim prize?”</i> she asks. ");
