...
{
	showBust(lundBustString(nude));
	showName("\nLUND");
	author("Wsan");
}
public function lundBustString(nude:Boolean = false):String
{
	return ("LUND" + (nude ? "_NUDE":""));
}
...
public function lundSexMenu():void
{
	...
	addButton(2,"Ride Him",vaginaRouter,[rideLundWheeee,lund.cockVolume(0),1,0],"Ride Him","Take your pleasure from him yourself.");
...
public function moreCookiesPleaseLund3():void
{
	...
	if(pc.hasVagina()) 
	{
		...
		pc.cuntChange(0,lund.cockVolume(0));
	}
	...
	processTime(10);
	if(pc.hasVagina()) pc.loadInCunt(lund,0);
	else pc.loadInAss(lund); 
	pc.orgasm();
	//[=Again=][=Enough=]
...
public function lundSubmissionThing():void
{
	...
	var sceneSelect:Number = 0;
	if(flags["LUND_ADVANCED_SUBMISSION"] != undefined) sceneSelect = flags["LUND_ADVANCED_SUBMISSION"];
	if(sceneSelect > 3) sceneSelect = 1 + rand(3);

	if(sceneSelect == 3) 
	{
		lundSubmissionNumber4();
		return;
	}
	else if(sceneSelect == 2)
	{
		lundSubmissionNumber3();
		return;
	}
	//First time.
	...
	clearMenu();
	addButton(0,"Next",lundSpecialSubmission2,sceneSelect);
}

public function lundSpecialSubmission2(sceneSelect:Number):void
{
	clearOutput();
	showLund(true);
	author("Wsan");
...
//Lund Submission #3 by Wsan
//[Submission]
//REQUIRES GENITALS
public function lundSubmissionNumber3():void
{
	clearOutput();
	showName("LUND\n& SAVI");
	...
	output("\n\n<i>“Here,”</i> he says, indicating you should join him.");

	output("\n\n<i>“Master...”</i> you begin, lowering your voice as you obey" + (!pc.isCrotchExposed() && !pc.isAssExposed() ? ", stripping out of your clothes":"") + ". <i>“Who is she?”</i>");

	output("\n\nThere’s a korgonne woman kneeling on the bed looking at you with shame writ large on her face. Her breasts, ample but not overly large on her modest frame, rise and fall rapidly with her breath as her gaze flicks between you and Lund. Though she’s completely naked, she’s not unadorned - you can see the last few inches of a savicite dildo sticking out of her pussy, wet and dripping onto the sheets. On closer inspection the gag she has held in her mouth is actually her own panties, soaked through with juices and saliva.");

	output("\n\n<i>“Savi,”</i> Lund replies, drawing the attention of the short woman. <i>“Now come. Lie under " + (korgiTranslate() ? "me":"master") + ".”</i>");

	output("\n\nHe takes his loincloth off, revealing his delicious-looking treat of a cock, and suddenly you don’t really care about the girl on the bed. If she’s here to worship your master too, that’s fine as long as he lets you play. Kneeling behind Savi, Lund raises his hips so that you can put your face underneath his cock. You get a couple of seconds to admire the view of his big, hanging nuts before he brings them down on you, sitting on your face with a pleased grunt.");

	output("\n\n<i>“Now lick,”</i> he commands you.");

	output("\n\nYou’re only too happy to oblige. You can feel him shifting above you, his muscular thighs on either side of your head, and raise your hands to place them around his hips just to get your bearings. Then you’re pressing yourself upwards with a desperation incommunicable by words, moaning into his sack while your tongue presses insistently against his hot, heavy balls, licking him as hard as you can. His slight shudder of pleasure above you is magnified many times over in your own body, your");
	if(pc.isHerm()) output(" [pc.cocks] straining in the air while your [pc.vaginas] squeeze" + (pc.hasVaginas() ? "":"s") + " sympathetically");
	else if(pc.hasCock()) output(" [pc.cocks] dripping and straining in the air, precum flowing freely down your shaft" + (pc.hasCocks() ? "s":""));
	else output(" [pc.vaginas] squeezing tight in sympathetic pleasure, warm arousal dripping onto the bed");
	output(".");
	output("\n\n<i>“Nnnfh! Hnnn,”</i> you moan, loud enough for the two of them to hear it. You don’t even care - let her be jealous that you get to service your master. You’d be content to stay down here all day and night, taking deep breaths from your nose squished against Lund’s balls. <i>“Nnngh! Hhhh!”</i>");
	output("\n\nThis feels so good; squirming on the bed, rubbing your [pc.thighs] together as you serve your powerful, domineering master as he begins to rock above you, you think you might actually cum without being touched. His musk surrounds you, permeating your thoughts and wrapping itself around your brain until you’re naught but a slave beneath him, bound not by ropes but by your own mind. The orgasm is inevitable.");
	output("\n\nUnable to help yourself, you excitedly hump the air even as you try to subdue your instincts, ");
	if(pc.isHerm()) 
	{
		output(" cumming all over your [pc.stomach] while your puss");
		if(!pc.hasVaginas()) output("y contracts and " + (pc.isSquirter() ? "squirts":"drips"));
		else output("ies contract and " + (pc.isSquirter() ? "squirt":"drip"));
	}
	else if(pc.hasCock()) output(" coating the [pc.skinFurScales] of your [pc.stomach] in [pc.cum], cock" + (pc.hasCocks() ? "s":"") + "pulsing every few seconds");
	else output((pc.isSquirter() ? "squirting":"dripping") + " all over Lund’s bed and down your [pc.thighs]");
	output(". You can hear your master grunting, no doubt impressed that you’re now so much of his slut that you can cum just giving him oral service.");
	output("\n\nThe orgasm doesn’t slow you down at all. If anything it completely renews your vigor, and you throw yourself back into your task with haste. As intimately familiar as you are with the salient points of Lund’s anatomy, you can tell he’s close to cumming. His balls are swelling with churning seed, growing warmer by the minute, and the way he grunts above you is getting more and more strained. Finally, with a greedy slurp between his tightening cumtanks, you hear him groan in release.");
	output("\n\nYou’re treated to the delightful sensation of his balls positively throbbing against your [pc.tongue], each load being pumped out while his powerful thighs flex and tense on either side of your head. It feels like bliss to be allowed to do this, to <i>feel</i> your master’s orgasm so physically. You promise yourself to remember this feeling, what it’s like to have Lund’s nuts in your mouth while he cums gigantic loads from his cock.");
	output("\n\nYou orgasm again before he’s even finished, ");
	if(pc.isHerm()) output("your limp cock" + (pc.hasCocks() ? "s":"") + " weakly spurting jizz across your tummy while your puss" + (pc.hasVaginas() ? "ies lazily contract":"y lazily contracts"))
	else if(pc.hasCock()) output("limp cock" + (pc.hasCocks() ? "s":"") + " spurting and dribbling jizz into the pooling seed already slathered across your tummy");
	else output("your [pc.clits] throbbing as your " + (pc.hasKnees() ? "knees":"[pc.legOrLegs]") + " draw" + (!pc.hasKnees() && pc.legCount == 1 ? "s":"") + " inwards, wetness sluicing into the bed");
	output(". When he’s finally finished, you feel him let out a sigh of satisfaction and hear a soft exclamation from the woman he brought to his room, Savi. Truth be told, you’d forgotten she was there.");

	output("\n\nLifting himself off you, Lund lets you get a breath of fresh air for the first time in a while. You hardly notice or care, gaze still lingering on his balls as you turn over and get up. Savi’s sprawled out in front of him, ass still raised in the air and her pussy gaped and dripping. It’s pretty clear what they were up to while you were down there, but you can’t find it in yourself to care much. Draping yourself over Lund’s shoulder, you press your [pc.chest] against his back.");

	output("\n\n<i>“Have you still got more for me, master?”</i> you whisper.");

	output("\n\n<i>“Heh,”</i> Lund grunts amusedly. He points to his still proudly erect cock, standing straight up and slathered in a mixture of Savi’s juices and his own. <i>“Clean it.”</i>");

	output("\n\nMoving to his side and bending at the hip, you " + (pc.hasHair() && pc.hairLength > 5 ? "pull your [pc.hair] to the side and ":"") + "moan as he allows you to grace your tongue with the head of his stiff cock. You’re careful not to overstimulate it in the wake of his orgasm, serving him with the love and care your master deserves. At some point during your gentle blowjob, Savi rouses herself and turns back around to watch. You give her a sly smile before sliding Lund’s entire cock into your throat, planting a kiss on the base of his swollen knot as he shivers in pleasure.");

	output("\n\n<i>“See, Savi?”</i> Lund says, grinning as he puts a hand on your head. <i>“This is why " + (korgiTranslate() ? "I keep a pet. [pc.heShe] will do anything to please me. I could fuck [pc.hisHer] asshole all day and [pc.heShe] would thank me for the favor":"Lund keep pet. Lund could fuck pet ass all day and pet would be thankful") + ".”</i>");
	output("\n\n<i>“" + (korgiTranslate() ? "I... I":"Savi... Savi") + " could do that too,”</i> she says falteringly, looking between Lund’s domineering grin and your slow, sensual deepthroat blowjob. <i>“" + (korgiTranslate() ? "I":"Savi") + " could...”</i>");
	output("\n\nLund sighs, but it’s you who replies, not him.");
	output("\n\n<i>“Come here,”</i> you say lazily. She still seems hesitant, so you plant your hand on the back of her head and bring her down to your level. <i>“Kiss his knot. He likes that,”</i> you instruct her.");
	output("\n\nDespite her being what must be a few years younger than you and Lund, you can tell Savi has the hots for him. You’re content to let her share, teach her how to best serve your master and ultimately debase herself for his pleasure, but first things first: she has to take the opportunity for herself. To use that slender, feminine form for Lund’s benefit. To allow herself to bend to his will.");
	output("\n\nSliding up and down Lund’s cock, you watch as she extends her tongue to tenatively taste and feel his big, strong knot. It’s a feeling you know well, one you’ve experienced many times over. Leaning forward, she closes her eyes and shakily plants her lips on the bulging flesh, planting a small kiss on his nethers.");
	output("\n\n<i>“Nnn...”</i> Lund sighs, putting his free hand on Savi’s head as you take yours away. <i>“Good.”</i>");
	output("\n\nSomewhat encouraged, Savi begins nervously plying Lund’s knot with kisses, gradually growing wetter and sloppier until she’s almost mimicking you, your [pc.lips] smeared with saliva and precum. You can feel Lund getting closer and closer to orgasm, his breathing getting more labored, and redouble your efforts to make him cum. Tapping Savi’s forehead to get her out of the way you all but impale yourself on Lund’s cock, wrapping your lips around his fat, throbbing knot.");
	output("\n\nShe gasps in shock as your eyes roll back in pleasure, putting her fingers over her mouth while she watches the impressive bulge in your throat pulsate. Lund groans in pleasure, holding you down as you let him take his fill of your oral cockhole. It’s what you do best, after all. Glancing sideways after recovering a bit of yourself, you can see Savi is gawking open-mouthed at your feat of submission.");
	output("\n\n<i>That’s right</i>, you feel like telling her. <i>Can you compete with this?</i>");
	output("\n\nWhen at last Lund has ejaculated enough to fill your stomach several times over he lifts his hands away and you slowly rise from his lap, careful not to let anything spill. Assured that everything he left inside you is safely on its way to or already warming your stomach, you bend down once more to give him one last gentle squeeze into your mouth, earning the thick, creamy remainder of his loads.");
	output("\n\nRising and turning to Savi, you push her down on the bed, overpowering the tiny korgonne girl and smiling as she gasps in surprise once more. Then you’re upon her, spreading her lips with your tongue and giving her a proper taste of her master-to-be. She resists at first, trying to turn her face away, but you don’t let her. Gradually, she stops and even leans into you slightly, though she’s clearly too inexperienced to know much of what to do.");
	output("\n\nWhen you pull back, Savi heaves a shuddering breath, looking at you with a mixed expression. A small trail of Lund’s seed runs from the corner of her lips and without thinking much of it, you reach out and brush it back up into her mouth with your finger. Lund chuckles behind you as she dazedly swallows it down.");
	output("\n\n<i>“" + (korgiTranslate() ? "A pet that makes more pets":"Pet beget pet") + "? Fun.”</i>");
	output("\n\n<i>“Ah, sorry, master,”</i> you say, turning and apologizing. <i>“I shouldn’t have done that on my own...”</i>");
	output("\n\n<i>“" + (korgiTranslate() ? "Don’t worry about it":"Pet not worry") + ",”</i> he says, shrugging. <i>“" + (korgiTranslate() ? "I don’t mind more pets":"More pet, more fun") + ".”</i>");
	output("\n\n<i>“That... that" + (korgiTranslate() ? " was":"") + "...”</i> Savi mumbles.");
	output("\n\n<i>“Amazing?”</i> you suggest, running your hand along her cheek. She’s actually very pretty, now that you look at her. No doubt she’s a popular girl among the korgonnes of the hold. Too bad for them she fell in with you and Lund...");
	output("\n\n<i>“That " + (korgiTranslate() ? "was... my":"Savi") + " first kiss!”</i> she says, looking at you wide-eyed.");
	output("\n\nYou raise your eyebrows in surprise.");
	output("\n\n<i>“" + (korgiTranslate() ? "I think you’ll find your first kiss was a little earlier":"Savi kiss Lund already") + ",”</i> Lund snickers, pointing at his dick.");
	output("\n\n<i>“Ugh, " + (korgiTranslate() ? "you":"Lund") + " know what " + (korgiTranslate() ? "I":"Savi") + " mean, jeez,”</i> Savi grumbles.");
	output("\n\n<i>“Would you like to try it again?”</i> you offer, hand slinking around Lund’s still-hard cock. Void, you don’t expect you’ll ever know the true extent of his stamina...");
	output("\n\n<i>“" + (korgiTranslate() ? "I":"Savi") + "-...”</i> Savi begins, looking between you, Lund, and his menacingly large cock. For a second, you think she’ll actually agree. <i>“N-no, " + (korgiTranslate() ? "I":"Savi") + " can’t,”</i> she says, shaking her head firmly enough that her hair cascades across her front. <i>“" + (korgiTranslate() ? "I have to see my sisters. They’ll be wondering where I am":"Savi have to find sisters. Not know where Savi is") + ".”</i>");
	output("\n\nWith that, Savi collects her clothes - slipping her panties on with a slightly despairing moan when she remembers that not only is her pussy still dripping with Lund’s spunk, but her panties are stained with saliva - and leaves the two of you alone. You call out a goodbye after her, leaning back into Lund and snuggling into his chest.");
	output("\n\n<i>“That was fun, master,”</i> you murmur.");
	output("\n\n<i>“Hmm,”</i> he hums in agreement. <i>“" + (korgiTranslate() ? "Doubt she’ll be back, though":"Lund not think Savi come back") + ".”</i>");
	if(pc.mf("","f") == "f") output("\n\n<i>“Hoho, master,”</i> you laugh into his strong chest, running your hands through his fur. <i>“Don’t take a young girl’s love </i>too<i> lightly.”</i>");
	else output("\n\n<i>“Dunno about that one,”</i> you say, running your hands through his chest fur. <i>“She seems awfully attached to you, master.”</i>");
	output("\n\n<i>“Eh,”</i> he grunts, shrugging disinterestedly. <i>“" + (korgiTranslate() ? "She’s gone now. Entertain me":"Savi gone now. Pet entertain") + ".”</i>");
	output("\n\n<i>“Yes, master,”</i> you whisper, rising to your knees and bending over at the hips, exposing your [pc.asshole].");
	output("\n\nHe doesn’t hesitate and you quickly find yourself borne to the bed, screaming in ecstasy as your master pounds your [pc.ass] hard enough for you to cum. He doesn’t stop there, either, fucking you into the mattress from above while your");
	if(pc.hasCock()) output(" limp cock" + (pc.hasCocks() ? "s spurt":" spurts") + " cum " + (pc.legCount > 1 ? "between your [pc.legs]":"below"));
...
//[Submission]
public function lundSubmissionNumber4():void
{
	clearOutput();
	showLund(true);
	author("Wsan");
	...
	output("\n\n<i>“Pet...”</i> he grunts, idly rubbing your back. It’s a nice, comforting feeling. <i>“Can " + (korgiTranslate() ? "you":"pet") + " cook?”</i>");
	output("\n\n<i>“Huh?”</i> you say, taken offguard by his question. He delivers a smart slap to your [pc.butt], eliciting a light gasp from your throat.");
	output("\n\n<i>“Can " + (korgiTranslate() ? "you":"pet") + " cook?”</i> he repeats.");
	output("\n\n<i>“Y-yes, I can cook, master,”</i> you stammer as he looks " + (pc.tallness < 64 ? "down at you":"at you") + ". <i>“I might not be the best chef, but I can at least make you a meal!”</i>");
	output("\n\n<i>“Good,”</i> he nods, stepping away from the wall and bringing you with him. <i>“" + (korgiTranslate() ? "You’re going to cook me dinner":"Pet cook Lund dinner") + ".”</i>");
	output("\n\n<i>“Oh... okay!”</i> you say, happy to go with the flow. Of course, there’s no reason you wouldn’t serve your master no matter what he asks. <i>“Show me the kitchen and hand me an apron, master,”</i> you say with a grin.");
	output("\n\n<i>“Hah, good pet,”</i> Lund says, tugging you along. <i>“" + (korgiTranslate() ? "I’ll take you to the kitchen":"Lund take pet to kitchen") + ".”</i>");
	output("\n\nHe tugs you along until you catch up, walking side by side with your master. It feels a little thrilling to pass through the hold with your hand in his, almost like he’s parading you around but not quite as literally as the times he’s shown you off before. Your eyebrows arch in surprise when you realize he’s taken you to a place you’ve visited before under very different circumstances - the dining room.");
	output("\n\nIt makes sense that the kitchens would be adjacent but it doesn’t stop you from thinking of the time he brought you here on the end of his cock. Unaware of the thoughts running through your head and making your cheeks flush with color, Lund pushes you into a room where a bunch of korgonne of mixed gender are fussing over pots and contained fires in what look like impressively constructed furnaces purposed for cooking.");
	output("\n\n<i>“Here,”</i> he says, pointing at a relatively undisturbed pile of meat. <i>“Go cook.”</i>");
	output("\n\nWith that he leaves you bewildered in the room and goes to take a seat in the dining room, sitting next to people who greet him casually. You suppose they must be his hunting friends. Looking around you, you spot an unused pot on a bench; that’ll do. It’s lucky the korgonne are a very small race, because the way you’re being jostled about by the busy, furry little creatures you’d probably be on the floor if they were any bigger. They pay no heed to the alien in their midst, focused wholly on their tasks. An industrious people indeed.");
	output("\n\nYou make your way to the meat and take up a knife, hastily chopping it up into slices and sprinkling salt on it along with a smidge of pepper. They have a relatively abundant store of salt of all different kinds here, but you’ll settle for rock salt. Throwing some meat in the pot, you step away for a second and think what you can do to make his meal extra special. Maybe you’ll whip up some gravy.");
	output("\n\nThankfully, butter and flour both appear to be present in the kitchen. It’s a little selfish, but you can quickly fry up some gravy in a second disused pot - now you’re beginning to realize why the korgonnes are running about so frantically. It’s actually a fairly well-oiled process, though it looks like chaos to an uninformed eye. Turning the meat over, you get to watch them scurrying to and fro with food in hand. You have your master’s meal ready in only a few minutes - whatever this meat is, it cooks quite fast over the flame. There’s a lot of juices for the gravy, too.");
	output("\n\nStepping out of the kitchen" + (pc.tallness > 72 ? " and taking care not to step on any wayward cooking korgs":"") + ", you bring Lund his meal and lean over his shoulder, setting it down gently on the table before him. You’re quite proud of what you’ve managed to accomplish with the limited means available.");
	output("\n\n<i>“Seasoned meat with gravy,”</i> you say to Lund, smiling hopefully. <i>“Enjoy, master.”</i>");
	output("\n\n<i>“Oh, fancy,”</i> one of Lund’s friends says, smiling. <i>“" + (korgiTranslate() ? "I have half a mind to take you for myself":"Almost want to take slave for self") + ".”</i>");
	output("\n\n<i>“" + (korgiTranslate() ? "You couldn’t give her what [pc.heShe] needs, Haken":"Haken not give pet what pet needs") + ",”</i> Lund snickers, shaking his head. <i>“" + (korgiTranslate() ? "Come here, pet. Why don’t you get your own dinner from under my loincloth":"Pet come closer. Can pet get own dinner from under Lund loincloth") + "?”</i>");
	output("\n\n<i>“Oh, master,”</i> you sigh erotically, leaning in to kiss him on the cheek before he begins to eat. <i>“You </i>spoil<i> me...”</i>");
	output("\n\nStepping over the bench he’s seated on and sitting in his lap for a moment just to cherish the experience, you give him a grin before sliding downwards under the table, leaving him to his meal. His friends are laughing, jeering and sloshing about their alcohol, but all you can think about is how fucking hot it is that you’re going to blow him in front of all of his friends. You’ve come so far since being embarrassed by master having you in public. Now you’re more than happy to flaunt it, your ");
	if(pc.isHerm()) output("[pc.cocks] already stiffly erect while your [pc.vaginas] drip" + (pc.hasVaginas() ? "":"s") + " with the desire of sating your master");
	else if(pc.hasCock()) output("[pc.cocks] stiffly erect at the prospect of swallowing down your master’s seed");
	else output("[pc.vaginas] dripping with heated desire, [pc.clits] stiff and lips inflamed at the thought of getting a stomachful of master’s seed");
	output(". You can hardly wait.");

	output("\n\nYou’re not shy about performing at all, taking his hefty balls in hand and shoving your head beneath his loincloth to give them long, laborious licks while his fat doggycock springs from its sheath. You let it tent the brown fabric of his sole item of clothing for a while, working between his spread legs to lick, suck, and generally slather his bulging balls in warm saliva. You leave them wet and almost dripping so enthusiastic are you,");
	output("\n\nBy the time you make it up to his big, stiff cock, you’ve completely zoned out the rest of the dining hall. As far as you’re concerned the only two people in this world right now are you and your master, and he deserves the best. Sucking his pointed tip between your [pc.lips], you don’t hesitate to sink down the base of his impressive prick. You effortlessly get all ten inches of its splendor buried in your throat, enjoying being unhindered by his knot.");
	output("\n\nRather than go full force right from the start, you take a much more measured approach. It would be shameful conduct for a slave to embarrass [pc.hisHer] master by forcing them to have a colossal, sudden orgasm in front of his friends. Fondling the balls you lavished with so much attention just prior, you slowly glide up and down his canine cock while you rub and gently squeeze.");
	output("\n\nYou can hear him grunt approvingly over the din of the hall while he eats, devouring the meal you made for him while you dutifully suck him off. You’ve never felt so content - isn’t this what life is <i>about</i>? The idea of continuing your journey is starting to lose its lustre when you have all you want right here, sitting on a bench with his legs spread for you. You’ve started to think of the hold as your home. You keep coming back to it, looking for your master and for the satisfaction of serving him.");
	output("\n\nThe way he makes you feel is undeniable; as his slave and fucktoy it’s far from your place to tell him as much, but what you feel for Lund is as close to love as you can get. Not to mention a healthy mix of desire, lust, worship and servility. Even now, you can feel happiness blooming inside your chest as your master’s nuts begin to tighten, hugging his groin while the seconds before he cums count down.");
	output("\n\nYou time it perfectly, your [pc.lips] reaching his groin right as a thick rope of warm seed splatters against the back of your throat, slipping down into your stomach. He doesn’t even need to hold you there, your body so well-trained you can hold yourself in place as his knot begins to rapidly swell. Hot, sticky cum gushes into your tummy as Lund gently nudges you with a knee, indicating you shouldn’t get stuck on his knot.");
	output("\n\nYou slowly make your way back up his impressive length, sucking him as hard as you can. Each long, hard suck rewards you with another extended spray of wet jizz that slides down your throat, lining your insides. By the time you’re finished and take a long, satisfied inhale after a job well done, you’ve been repainted from the inside out. Daintily wiping your mouth with your fingers and ensuring there’s nothing left, you pat your master’s loincloth and reemerge from under the table to stand behind him with a smile.");
	output("\n\nThough a couple of them are wearing knowing smiles, most of Lund’s friends are too involved with their meals and drinks to care. You obediently wait for him to finish up, watching him with adoration. Finally, he nods farewell to his friends and gestures for you to follow. You head back to his room, closing the door behind yourself as he seats himself on the bed. You stay standing, waiting before him to see if he plans to order you to do something. Instead, he leans back and nods at you.");
	output("\n\n<i>“" + (korgiTranslate() ? "How do you feel about life in the hold, pet":"How pet feel about life in hold") + "?”</i> Lund asks. <i>“Be honest.”</i>");
	output("\n\n<i>“It’s a little more primitive than what I’m used to, master,”</i> you say, smiling at him. <i>“But I enjoy it. I think the korgonne exemplify what Uveto is about. You in particular, of course, master. I had fun with the cooking and the meal,”</i> you continue, drawing closer and flushing with desire. <i>“I like it here a lot, master...”</i>");
	output("\n\n<i>“Good. But " + (korgiTranslate() ? "I":"Lund") + " know what " + (korgiTranslate() ? "you":"pet") + " like best,”</i> he says, grinning. <i>“Why " + (korgiTranslate() ? "don’t you":"doesn’t pet") + " take " + (korgiTranslate() ? "my":"Lund") + " loincloth off and sit in " + (korgiTranslate() ? "my":"Lund") + " lap.”</i>");
	...
	output("\n\n<i>“Heh,”</i> Lund grunts in amusement, settling back on his pillows with an arm around you. <i>“" + (korgiTranslate() ? "Rest here for the night, pet":"Pet rest here for night") + ".”</i>");
...
public function beLundsCookslutEpilogue():void
{
	clearOutput();
	showLund(true);
	author("Wsan");
	...
	output("\n\n<i>“Yes, master, thank you,”</i> you murmur, still half-asleep" + (pc.hasHair() && pc.hairLength > 3 ? " with your [pc.hair] strewn about":"") + ". You give him a smile. <i>“Would you like some service before you go?”</i>");
	output("\n\nStanding before you, Lund leaves his hand on your head while you eagerly bob on his dick, gulping it down and sliding it back up your throat until he gives you his first orgasm of the day. It’s thick, sticky, and tastes like your master. You obediently swallow all of it down, not leaving even a single drop.");
	output("\n\nSighing in satisfaction, he refastens his loincloth and nods.");
	output("\n\n<i>“" + (korgiTranslate() ? "Be seeing you, pet":"Lund see you later, pet") + ",”</i> your master says, ducking out of the door and closing it behind him.");
	output("\n\nLooking around the room, sitting in your master’s bed after a night of passion and a meal at his hands, you’re filled with unease. You’re going to leave <i>this</i> behind for your journey? You could spend the rest of your life here in complete happiness, sticking it out with your master. Every day would be bliss, spreading your legs and opening your mouth for his personal use and then serving him in all of the ways a slave can. Just looking around, you can see his room is slightly dusty...");
...
public function lundFight():void
{
	...
	CombatManager.setHostileActors(lund);
...
//Victory scene
public function lundVictory():void
{
	showLund();
	output("<i>“Cheh!”</i> Lund spits, stumbling backwards. The wind begins to howl through his fur as he kneels, taking a powder from a pouch. ");
...
//First
public function initialLundBitchening():void
{
	showLund(true);
	...
	{
		output("\n\nLying on the ground exhausted and bruised all you can muster is a tired groan before Lund sits heavily on your chest, momentarily costing you your breath. He looks down at you disinterestedly and gives you one hard slap across the cheek that leaves");
		if (pc.hasFur()) output(" the skin beneath your fur");
		else if (pc.hasScales()) output(" your scaled skin");
		else output(" your skin");
		output(" stinging in the wake of it, the noise ringing in your ears while he begins to methodically strip you down. Having gotten the message across, the korgonne hunter lifts his loincloth to the side to present his big, throbbing dogdick.");
	}

	output("\n\n<i>“Suck,”</i> he growls, pressing it to your face.");
	output("\n\nFuck, it’s a <i>lot</i> bigger than you would have expected for such a compact species. No wonder he struts about all arrogant and demanding... but you’re at his mercy now, and the only way out of this situation is to suck him off until he’s content.");
	if (!hpLoss)
	{
		output(" Hopefully after you make him blow his load he’ll help you with yours, though even without that assurance the savicite powder is driving you just crazy enough for it not to matter. You’d be thrilled just to feel his knotty cock throbbing on your [pc.tongue].");
		output("\n\nLeaning upwards you take the pointed tip of his erect penis between your [pc.lips], taking the briefest of seconds to taste the droplet of warm, salty precum hanging there. God, that’s good... you wouldn’t mind feeling this smeared all over the back of your throat. Luckily for you, that seems to be where he wants it to be - leaning forward and putting his hands to either side of your head, sinking his hips lower.");
	}
	else
	{
		output(" He’d better not try and knot your face, the fucker. You don’t want to be out here sucking his seed down for twenty minutes.");
		output("\n\nYou reluctantly lean upwards and take the pointed tip of his erect penis between your [pc.lips], trying your damnedest to ignore the warm, salty drop of precum that washes across your [pc.tongue]. The taste seems to linger there, almost as if you’ve been <i>marked</i> by the defilement, but you have no time to dwell on the issue. He leans forward, putting his hands to either side of your head and sinking his hips.");
	}
	if (!pc.canDeepthroat())
	{
		output(" You almost gag as the ten inch canine prick pushes firmly into your throat, just barely repressing the reflex before he starts slowly fucking your face. <i>Now</i> you gag, choking and spitting all over his rock hard cock as he thrusts himself balls-deep, groaning in pleasure.");
		output("\n\nIt takes some time before Lund fucks your reflexes into submission, overwhelming your body with such deep penetration that your entire throat is nothing more than his warm, spit-lubricated cocksleeve. The only noises you make are wet, gurgling moans as he drills into you, saliva dribbling down your chin in rivulets.");
	}
	else
	{
		output(" You’re such a gifted cocksucker that you have no trouble at all taking him in, your throat spreading wide around all ten inches of his canine prick and undulating around it. So gifted are you, in fact, that Lund immediately takes notice and seeks to capitalize. Groaning in pleasure, he puts one hand behind your head and shoves you into his furred crotch, grinding his nuts all over your chin while precum drips down into your stomach.");
		if (hpLoss) output(" Even as unwilling as you are, you’re forced to admit this feels fucking amazing...");
		else output(" It’s everything you could have asked for.");
	}

	output("\n\n<i>“Fuck,”</i> Lund curses above you, flexing as he starts pounding you even harder.");
	if (!korgiTranslate()) output(" <i>“Alien throat good sheath!”</i>");
	else output("<i>“Your throat is perfect for cock!”</i>");
	...
	output("\n\nHe seems perfectly content to stay like that, draining his nuts down your held-open throat.");
	if (!hpLoss)
	{
		output(" Warm contentment washes over you,");
		if (pc.isHerm()) output(" both your [pc.cocks] and [pc.pussies] drooling and twitching.");
		else if (pc.hasCock()) output(" your [pc.cocks] twitching and drooling precum.");
		else if (pc.hasVagina()) output(" your [pc.pussies] drooling in need.");
		else output(" your asshole involuntarily twitching with need.");
		output(" It feels just a bit like your own orgasm is near but it stays firmly out of reach, eluding your grasp as Lund’s knot pulses on your tongue.");
	}
	else output(" Fuck it... it’s better than being beaten down some more. Were it not for the uncomfortably gigantic knot lodged in your mouth, this wouldn’t even be so bad; at least your insides are warm.");
	output(" It’s only several minutes later that Lund finally moves, sighing in satisfaction as he tugs his knot free and lifts himself off you.");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Alien look much better now,”</i>");
	else output("<i>“That’s a nice look on you, offworlder,”</i>");
	output(" he says with a cruel smirk, and you look down to find your stomach is so full of his sperm that it’s slightly swollen with liquid weight. Groaning, you let your head fall back against the ground.");
	if (!hpLoss) output(" Will you find no release?");
	else output(" This damn korgonne is just doing whatever he pleases to you.");

	output("\n\nLund rolls you over onto your front with a thump and you cough, soupy cum leaking from the corners of your mouth. You don’t even need to look back to know what he’s doing now, his hand firm upon your [pc.ass] as he spreads your cheeks apart to reveal your [pc.asshole].");
	if (!hpLoss)
	{
		output(" Oh fuck, <i>finally</i>! You’re so close to orgasm that you try to raise your ass for him before he slaps your butt the way he did your face, pressing you back down into the ground.");

		output("\n\n<i>“</i>Lund<i> fuck alien,”</i> he growls threateningly. <i>“Not other way.”</i>");
	}
	else output(" Fuck! He’s clearly not done with you just yet, his pale blue cock still hard and upright, twitching in need. Now that you think about it, you couldn’t feel him going soft in your mouth at all.");
	if (pc.isHerm()) output(" Your [pc.cocks] stiffen and your [pc.pussies] drip");
	else if (pc.hasCock()) output(" Your own [pc.cocks] grow stiff against the ground");
	else if (pc.hasVagina()) output(" Your [pc.pussies] twitch and drool");
	else output(" Your asshole flexes involuntarily");
	output(" as you think of what’s going to happen next. <i>Just how virile </i>is<i> this korgonne?</i>");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Lund hope alien stretchy,”</i>");
	else output("<i>“I hope you’re stretchy,”</i>");
	output(" Lund grunts. ");
	if (!korgiTranslate()) output("<i>“Going to need it.”</i>");
	else output("<i>“Gonna need the help.”</i>");
	...
	else
	{
		...
		if (pc.analVirgin)
		{
			output("\n\nFuck..! Lund pushing into your asshole and stretching you wide is painful");
			if (hpLoss) output(", and the fat load of hot precum he leaves inside you as lubrication only makes it sting your ego all the more. He’s making you his fucking bitch, his rough hands spreading your cheeks apart so he can slide deeper inside your asshole, and though it hurts you’re forced to shamefully admit there’s a part of you that’s really enjoying the way his thick dick is rubbing against your prostate.");
			else output("\n\n no matter how much savicite you’re dusted up with, though the fat load of hot precum he leaves inside you is a nice consolation. It serves to assauge the sting and doubles as lubrication, and soon you find yourself moaning in his hands, revelling in the way his thick, dominant dick rubs and strokes your prostate.");

			output("\n\n<i>“Tight fuck,”</i> he grunts. ");
			if (!korgiTranslate()) output("<i>“Alien make good bitch.”</i>");
			else output("<i>“You’re gonna make a great bitch.”</i>");
		}
		//pc not virgin:
		else
		{
			output("\n\nYour grunt gives way to a moan as Lund slips inside you, spreading your asshole around his engorged prick.");
			if (hpLoss) output(" Precum spurts from his tip into your insides, warming you and providing all the lubrication he’s going to need to make a mess of your guts. You’re his bitch for the moment, and though the prospect stings your ego you find yourself moaning regardless.");
			else output(" Fuck, he is <i>good</i>... The way his big, thick dogcock rubs against your prostate is divine, translucent precum drooling from your [pc.cockHeadBiggest] in response to every stroke. You moan as he shifts his hands, spreading your asscheeks wide to get deeper inside you.}");
			output("\n\n<i>“Nice,”</i> he grunts. ");
			if (!korgiTranslate()) output("<i>“Alien good fuck.”</i>");
			else output("<i>“You’re a good buttslut.”</i>");
		}
	}
	var wasVirgin:Boolean = false;
	if (x == -1 && pc.analVirgin) wasVirgin == true;
	else if (pc.hasHymen(x)) wasVirgin == true;
	pc.holeChange(x,lund.cockVolume(0));

	output("\n\n");
	if (hpLoss) output("No matter how you feel about Lund, you have to admit within a few seconds of him crushing your will to resist: he’s <i>really</i> good at fucking.");
	else output("You’ll be Lund’s slut if he wants as long as he keeps it feeling this good.");
	output(" His priority is clearly <i>not</i> your pleasure but all the same you’re clenching up on him and grunting, the arrogant korgonne’s dick hitting you in all the right spots. The longer it goes the more you feel control slipping away like grains of sand through your hands, leaving you");
	if (hpLoss) output(" grasping at nothing but the ground.");
	else output(" almost insensate with pleasure.");

	output("\n\n<i>“Nnnrrrgh,”</i> Lund growls, leaning in further and fucking you more aggressively, his hands finding your [pc.hips]. <i>“Cum!”</i>");
	...
	output("\n\nIt’s several minutes before he tires of taking advantage of you, dismounting and slapping you across both asscheeks hard enough to make you wince in pain after everything you’ve gone through. You’re sore and sensitive, your");
	if (wasVirgin) output(" formerly virgin");
	else output(" abused");
	if (x >= 0) output(" slit");
	else output(" ring");
	output(" left drooling virile spunk as Lund departs without a word, leaving you facedown with only the warmth inside you as company.");

	if (hpLoss)
	{
		output("\n\n<i>“Fuuuck,”</i> you groan, rolling over onto your back and immediately regretting the decision as spunk gushes down your [pc.thighs]. <i>“Ngh.”</i>");
...
//Second
public function becomingLundsBitch():void
{
	showLund(true);
	var hpLoss:Boolean = false;
	if (pc.HP() <= 1) hpLoss = true;
	...
	output("\n\nThen he’s on top of you, stripping off your [pc.gear] and leaving it to the side while he spreads your cheeks, inspecting his prize. ");
	if (pc.hasCock() && !pc.isHerm())
	{
		output("You feel his furred hand wrap around your [pc.cockBiggest] and squeeze, rubbing you just enough to make a string of precum drool from your [pc.cockHeadBiggest] before pulling away.");
		output("\n\n");

		if (pc.biggestCockLength() < 8)
		{
			if (!korgiTranslate()) output("<i>“Alien small,”</i> Lund chuckles, patting your butt as if consoling you. <i>“Is ok. Not need to be big to be bitch.”</i>");
			else output("<i>“You’re definitely on the smaller side but that’s okay,”</i> Lund chuckles, patting your butt as if consoling you. <i>“You don’t need to be hung to be my bitch.”</i>");
		}
		else if (pc.biggestCockLength() < 14)
		{
			if (!korgiTranslate()) output("<i>“Alien think big dick make big strong? Idiot,”</i> Lund chuckles, lightly slapping your butt. <i>“Big strong come from heart, not dick. Not that alien know.”</i>");
			else output("<i>“Do you think having a bigger dick makes you stronger? Idiot,”</i> Lund chuckles, lightly slapping your butt. <i>“Being strong comes from within. Not that I’d expect you to know, bitch.”</i>");
		}
		else
		{
			if (!korgiTranslate()) output("<i>“Oh, alien must think bigger better,”</i> Lund sighs, patting your butt pityingly. <i>“So simple and dumb, alien. At least alien look real good when cumming.”</i>");
			else output("<i>“Oh, you must think bigger is better,”</i> Lund sighs, patting your butt pityingly. <i>“What a primitive mindset. At least you’re going to look really pretty when I make you cum.”</i>");
		}
	}
	else if (pc.hasVagina() && !pc.isHerm())
	{
		output("You feel his furred fingers run down your inflamed pussy lips and can’t help but moan like a whore when he sinks them into your slickened fuckhole, pressing the tips to your walls and exploring your depths until he pulls free.");
		output("\n\n");

		if (pc.looseness(x) <= 2)
		{
			if (!korgiTranslate()) output("<i>“Alien so tight,”</i> Lund comments, rubbing your butt. <i>“Good work. Try same next time.”</i>");
			else output("<i>“You’re so tight,”</i> Lund comments, rubbing your butt. <i>“All the better. Try and keep it that way for next time, huh?”</i>}");
		}
		else if(pc.looseness(x) <= 4)
		{
			if (!korgiTranslate()) output("<i>“Alien not going to have problem taking knot,”</i> Lund comments, rubbing your butt. <i>“Good slut.”</i>");
			else output("<i>“You’re not gonna have any problems taking the knot,”</i> Lund comments, rubbing your butt. <i>“What a good slut you are.”</i>");
		}
		else
		{
			if (!korgiTranslate()) output("<i>“Alien loose whore,”</i> Lund sighs, slapping your butt. <i>“Not worry. Lund get you used to regular dick again.”</i>");
			else output("<i>“You have a whore’s pussy,”</i> Lund sighs, slapping your butt. <i>“Don’t worry, I’ll get you used to stretching around my cock again.”</i>");
		}
	}
	else if (pc.isHerm())
	{
		output("You feel his furred hand wrap around your [pc.cockBiggest] and squeeze before his fingers slide up your inflamed pussy lips and sink into your slickened fuckhole, making you moan like a whore while you drool precum from your [pc.cockHeadBiggest].");
		output("\n\n");
		if (!korgiTranslate()) output("<i>“Alien gonna cum twice at once?”</i> Lund wonders, pulling his fingers free. <i>“Cute bitch.”</i>");
		else output("<i>“Are you going to cum twice at once?”</i> Lund wonders, pulling his fingers free. <i>“That’s cute for a bitch.”</i>");
	}
	else
	{
		output("You feel two of his furred fingers prod at your asshole before pushing inwards, spreading you around his digits and forcing you to moan like a whore when he gives you a few experimental pumps.");
		output("\n\n");
		if (!korgiTranslate()) output("<i>“Alien strange, but good for bitch,”</i> Lund comments, pulling his fingers free and patting your butt. <i>“No need to cum.”</i>");
		else output("<i>“You’re a strange one, but that works fine for being a bitch,”</i> Lund comments, pulling his fingers free and patting your butt. <i>“You don’t need to cum, after all.”</i>");
	}

	output("\n\nThen you hear him shifting his loincloth and before you can even brace yourself he has his hands around your [pc.hips], grinding his fat erect cock against your [pc.ass] and letting you feel just how hard the cock that’s about to fuck you is. He hardly gives you a moment to consider it. His pointed tip is sliding into your [pc.vagOrAss " + x + "] the next moment, and you’re tensing up around him before you can even think to relax.");

	output("\n\n<i>“Good,”</i> he grunts, squeezing you tight around the waist. ");
	if (!korgiTranslate()) output("<i>“Alien please master.”</i>");
	else output("<i>“Please your master.”</i>");
	...
	output("\n\n<i>“Ohhh, stars! N-nnngghh!”</i> you cry out, your");
	if (pc.isHerm()) output(" squirting slit clamping down on him and caressing his pounding cock while you spurt [pc.cum] all up your [pc.stomach], covering you in your own spunk while he ravages you.");
	else if (pc.hasVagina()) output(" squirting slit clamping down on him and caressing his pounding cock while you cum around him.");
	else if (pc.hasCock()) output(" asshole clamping down on him and caressing his pounding cock while your cock" + (pc.hasCocks() ? "s":"") + " spray [pc.cum] up your [pc.stomach], covering you in your own spunk while he ravages you.");
	else output(" asshole clamping down on him and caressing his pounding cock while he ravages you.");
	output(" <i>“Gghhgh, fuck!”</i>");

	output("\n\n<i>“Slut,”</i> Lund pants, laughing in your face. ");
	if (!korgiTranslate()) output("<i>“Alien can’t help it, huh?”</i>");
	else output("<i>“You just can’t help yourself, can you?”</i>");

	output("\n\nJaw clenched in frustration, you ride out what has to be one of the most enjoyable orgasms of your life even as you fight to keep your eyes from rolling back in your head. How the fuck does he <i>do</i> this to you? Is it the powder, or is it this fat fucking cock lodged deep inside your");
	if (x >= 0) output(" squelching cunt");
	else output(" quivering asshole");
	output("? It’s impossible to differentiate the two.");

	output("\n\nAll you know is that the more he forces himself upon you the more you’re growing to enjoy the sensation of his big, hard dick pumping in and out of you,");
	if (x >= 0) output(" shining with the juices he’s fucking out of you");
	else output(" stroking your" + (pc.hasCock() ? " prostate":" insides") + " with a rhythm that has your [pc.tongue] lolling desirously from your mouth.");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Feel cock?”</i>");
	else output("<i>“Feel my dick throb?”</i> Lund whispers, leaning over to whisper in your ear.");
	if (!korgiTranslate()) output("<i>“Alien beg or no cum.”</i>");
	else output("<i>“Beg me, or I won’t cum inside you.”</i>");
	...
	output("\n\n<i>“Please cum in my " + (x >= 0 ? "pussy":"ass") + ",”</i> you moan, balling your hands into fists. <i>“Please, I need you deep inside!”</i>");

	output("\n\nWith renewed vigor Lund grabs your shoulders and pushes you into the ground, grunting as he begins to fuck you even harder. Muted screams of happiness work their way out of your [pc.lips] as you’re relentlessly pounded, your " + (x >= 0 ? "cunt":"insides") + " giving way beneath his assault. When he begins to grunt and groan you instinctively pull him in closer and wrap your [pc.thighs] around him, ensuring he’ll be balls-deep in you when he orgasms.");

	output("\n\nYou let out a delighted scream as warmth blossoms deep inside you, jetting from the end of his cock, and then another higher-pitched scream as his bulging knot pops into your stretched-wide " + (x >= 0 ? "slit":"asshole") + ". He’s <i>really</i> inside you now, and he wants you to know it. Lund gruffly pulls you upwards and begins grinding himself against you, fucking you in his lap while he blows his load into your " + (x >= 0 ? "[pc.womb]":"guts") + ".");

	output("\n\nStars, he’s a stud - just gyrating on his beautiful knot has you cumming your brains out, groaning like an animal in heat while he plows you from below, threatening to pull his knot free and stretching you out to an obscene degree. You’re bounced up and down in his arms, loosely hugging his chest and panting while");
	if (pc.hasBreasts()) output(" your [pc.breasts] jiggle enticingly");
	else output(" he finishes up with you");
	output(". The moment he’s done he lets out a groan of satisfaction and pushes you over, pulling himself free of your gaping " + (x >= 0 ? "cunny":"asshole") + " with a slick <i>thuck</i>.");

	output("\n\n<i>“Oh!”</i> Fffuck,”</i> you moan,");
	if (x >= 0) output(" pressing your hand to your cum-dripping fuckhole");
	else (" clenching your cheeks");
	output(" and wincing. <i>“Nnnmm...”</i>");
...
//Third
public function youreLundsBitch():void
{
	showLund(true);
	pc.changeLust(50);
	...
	output("\n\n<i>“Oohh, good,”</i> he mutters. ");
	if (!korgiTranslate()) output("<i>“Alien smart now.”</i>");
	else output("<i>“You’re getting smarter.”</i>");

	output("\n\nThough your thoughts are obscured by the lust pounding in your chest, you recognize the compliment for what it is - demeaning. Yet that doesn’t stop your");
	if (pc.isHerm())
	{
		output(" [pc.cocks] from growing harder, your [pc.pussies] dripping and clenching");
		if (!pc.isCrotchExposed()) output(" in your [pc.crotchCoverUnder]");
		output(" in remembrance of the last time he took you.");
	}
	else if (pc.hasCock())
	{
		output(" [pc.cocks] from growing harder");
		if (!pc.isCrotchExposed()) output(" in your [pc.crotchCoverUnder]");
		output(" and your asshole from instinctively clenching in remembrance of the last time he took you.");
	}
	else if (pc.hasVagina()) output(" [pc.pussies] from growing wetter, clenching in remembrance of the last time he took you.");
	else output(" asshole from instinctively clenching in remembrance of the last time he took you.");
	output(" If anything, it just makes you hornier. The shame must be showing on your face, because Lund lets out a chuckle.");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Alien enjoy being Lund slut?”</i>");
	else output("<i>“Oh, you enjoy being my slut?”</i>");
	output(" he asks. ");
	if (!korgiTranslate()) output("<i>“Know what next?”</i>");
	else output("<i>“Do you know what comes next, then?”</i>");
	...
	output("\n\n<i>“Yes, yes, good,”</i> Lund murmurs,");
	if (pc.hasHair()) output(" running a hand through your [pc.hair]");
	else output(" running a hand over your scalp");
	output(". <i>“Good bitch.”</i>");

	output("\n\nThat stings more than losing the fight, your eyes narrowing as you direct your intense dislike up at Lund’s grinning countenance while you spread your lips around the arrogant fuck’s bulging balls and suck like a whore. He might have you beaten now, but you’re far from conquered - and besides, the arrangement is mutually beneficial. He gets to blow a thick, massive load into your stomach or your" + (x >= 0 ? " cunt":" asshole") + " and you get the satisfaction of a job well done, not to mention your own orgasms. In fact, if you think about it, you’ve been cumming more and harder than he has.");

	output("\n\n<i>“’Mm noh your bih,”</i> you mumble around his balls, tickling them with your tongue as you kiss the underside of his impressive shaft. <i>“Nnnmm...”</i>");
	...
	output("\n\n<i>“Nngh!”</i> Lund grunts, pulling you into his groin.");
	...
	output("\n\nAnd yet for some reason you can’t fathom, every spurt of warm seed down the back of your throat brings you closer, and closer, until you realize this is really going to happen. Unable to make a noise, your vocal chords thrum against Lund’s throbbing cumvein as your [pc.hips] begin to buck,");

	if (pc.isHerm())
	{
		output(" your puss" + (pc.hasVaginas() ? "ies":"y") + " " + (pc.isSquirter() ? " squirting":" dripping") + " [pc.girlCum] down your inner thighs while your painfully stiff cock" + (pc.hasCocks() ? "s":"") + " spew [pc.cum]");
		if (!pc.isCrotchExposed()) output(" into your [pc.crotchCoverUnder] and");
		output(" all over your [pc.legOrLegs]");
	}
	else if (pc.hasCock())
	{
		output(" your painfully stiff cock" + (pc.hasCocks() ? "s":"") + " spraying [pc.cum]");
		if (!pc.isCrotchExposed()) output(" into your [pc.crotchCoverUnder] and");
		output(" down your [pc.thigh]");
	}
	else if (pc.hasVagina())
	{
		output(" your puss" + (pc.hasVaginas() ? "ies":"y") + " " + (pc.isSquirter() ? " squirting":" dripping") + " [pc.girlCum]");
		if (!pc.isCrotchExposed()) output(" into your [pc.crotchCoverUnder] and");
		output(" down your [pc.thigh]");
	}
	else output(" your [pc.asshole] clenching as you just about orgasm, your lower half grinding as if impaled on an invisible cock, trying to cum and never quite reaching release");
	output(". Dazed and confused, you hardly have any idea what’s happening to you until Lund reaches down and taps your [pc.nose].");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Alien be here a while,”</i>");
	else output("<i>“You’re gonna be here for a while,”</i>");
	output(" he says, grinding his fingers against your nostrils. It’s only now you realize they’re covered in savicite powder, little piles of the stuff that make clouds of dust as he stuffs you full of it. ");
	if (!korgiTranslate()) output("<i>“Enjoy.”</i>");
	else output("<i>“Have fun.”</i>");

	output("\n\nYour eyes roll up when immense, thunderous bliss begins to roll through you, radiating from your groin outwards as you start to cum non-stop, fucking the air while you swallow Lund’s cum. Vision fading as the sensations run wild through your body, the last thing you see before you pass out is Lund grinning as he holds you upright to ensure you can keep swallowing.");
...
public function stuckonLundsDickWithYourMouth():void
{
	...
	if (pc.hasCock()) output(" - or maybe that’s yours, it’s hard to tell");
	output(". Stars, he certainly could have been a little gentler.");
	...
	output("\n\nThe savicite must still be affecting you.");
	if (pc.hasCock()) output(" Your dick" + (pc.hasCocks() ? "s are":" is") + " still standing upright, and a shiver runs down your spine when you place a weary hand on " + (pc.hasCocks() ? "one":"it") + ". Just once...");
	else if (pc.hasVagina()) output(" Your puss" + (pc.hasVaginas() ? "ies are":"y is") + " still hungry for some attention, and a shiver runs down your spine when you place your fingertips on [pc.oneClit]. You’ll just do it once...");
	else (" You lift one leg up, exposing your asshole and reaching downwards with a couple of fingers. Just a few minutes...");
...
public function submitToLundBadEnd():void
{
	clearOutput();
	showLund(true);
	var vag:Boolean = pc.hasVagina();

	output("You sink to your knees, dropping your weapons. You’re tired... so tired. Though the journey started as an entertaining adventure, by now it’s little more than an endless burden, dragging you down into the abyss. The fire in your eyes dies down to little more than embers as Lund trudges towards you, grimacing.");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Alien weak,”</i>");
	else output("<i>“You’re weak,”</i>");
	output(" he says, taking ahold of your" + (pc.hasHair() ? " [pc.hair]":" head") + "");
	output(". <i>“Here.”</i>");
	...
	output("\n\n");
	if (!korgiTranslate()) output("<i>“Alien bad adventurer,”</i>");
	else output("<i>“You were a shitty adventurer,”</i>");
	output(" Lund sighs, gently scratching your scalp. ");
	if (!korgiTranslate()) output("<i>“But gonna be good" + (vag ? " breeder":" bitch") + ".”</i>");
	else output("<i>“But you’re going to make a great" + (vag ? " breeding bitch":" cockslut") + ".”</i>");

	output("\n\nHis words sink into your pliant mind, implanting themselves there and occupying your fatigued consciousness.");
	if (!korgiTranslate()) output("" + (vag ? " Breeder...":"Bitch...") + "");
	else output("" + (vag ? " Breeding bitch...":"Cockslut...") + "");
	output(" that’s so much less complicated than ‘inheritor’. A life of open non-resistance, of giving in, of accepting your place in the universe. You part your [pc.lips].");

	output("\n\nLund murmurs in approval as you begin to blow him, getting him hard with your mouth before he repositions you and begins to fuck your face. His cock is enormous for a guy standing a few inches over five feet,");
	if (pc.canDeepthroat()) output(" though thanks to your incredible ability to deepthroat you have no trouble letting him plow your throat as hard as he can. When he grunts and blows his thick, pumping load into your stomach, you’re met with a slight sense of achievement as you gaze up at his face. Maybe you <i>can</i> be good at something.");
	else output(" ten inches of throat-troubling canine meat that has you gagging and coughing spit all over his balls while he plows your throat. Your only saving grace is that when he grunts and cums hard, his bulging dick is too deep for you to even reject it. At least his jizz is warm...");

	output("\n\n<i>“Good [pc.boyGirl],”</i> Lund mutters, holding you against his groin. ");
	if (!korgiTranslate()) output("<i>“Come. Back to home.”</i>");
	else output("<i>“Let’s go home.”</i>");
	...
	output("\n\n<i>“Is this... a collar?”</i> you ask, groping at the binding. It feels smooth and warm, though you can’t see it. You can only guess what it’s made of.");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Is,”</i>");
	else output("<i>“It is,”</i>");
	output(" he says, grinning. <i>“Now come.”</i>");

	//[Next]
	processTime(15);
	pc.loadInMouth(lund);
	clearMenu();
	addButton(0,"Next",submitToLundBadEndCont);
}
public function submitToLundBadEndCont():void
{
	clearOutput();
	showLund(true);
	var x:int = -1;
	...
	output("You half-expect Lund to take you on the way back, but he doesn’t. Instead you plod behind him for what must be miles and only halt when he draws to a stop, pulling a bag of pink powder from his belt. You instinctively jerk back when he goes to smear it across your [pc.lips] but stop yourself, eyes fluttering as he rubs it into you.");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“Alien going be good?”</i>");
	else output("<i>“Are you going to be good?”</i>");
	output(" Lund asks, smiling at you.");

	output("\n\n<i>“I will,”</i> you mutter, heat rising to your cheeks. This must be savicite - you can tell what this is going to do you already");
	if (pc.isHerm()) output(" if the hardness of your [pc.cocks] or the growing arousal in your [pc.pussies] are");
	else if (pc.hasCock()) output(" if the hardness of your [pc.cocks] is");
	else if (pc.hasVagina()) output(" if the growing arousal in your [pc.pussies] is");
	else output("if the growing warmth deep inside your body is");
	output(" any indication. He’s going to make you perform...");

	output("\n\nBy the time you reach the Hold proper, though, you’ve decided you probably won’t mind. Every step you take makes you hyper-aware of");
	if (pc.isHerm())
	{
		output(" your lower lips rubbing together, wet with dew, and the sensation is");
		if (pc.hasBalls()) output(" every bit as frustrating as your [pc.balls] tightening like you’re on the verge of cumming");
		else output(" <i>supremely</i> frustrating");
	}
	else if (pc.hasCock())
	{
		output(" your growing erection" + (pc.hasCocks() ? "s":"") + ",");
		if (pc.hasBalls()) output(" your [pc.balls] tightening as if you’re on the verge of cumming");
		else output(" your need for release strengthening by the second");
	}
	else if (pc.hasVagina()) output(" your lower lips rubbing together, wet with dew, and the sensation is <i>supremely</i> frustrating");
	else output(" the need for some kind of release growing stronger by the second, your [pc.asshole] clenching instinctively");
	output(". You bite your lip to distract yourself and try not to whimper, your [pc.hips] shaking a little as you follow Lund inside.");
	...
	output("\n\nYou sink to your knees with desperation in your eyes, but even that soon dies as you realize just how much you need this.");
	if (pc.hasCock()) output(" Your [pc.cocksNounIsAre] so fucking hard " + (pc.hasCocks() ? "it hurts":"they hurt") + "");
	if (pc.isHerm()) output(", y")
	else if (pc.hasVagina()) output(" Y");
	if (pc.hasVagina()) output("our [pc.pussiesLightIsAre] so wet " + (pc.hasVaginas() ? "they’re":"it’s") + " dripping down your [pc.thighs]");
	else output(" The flame deep within isn’t going out");
	output(", and at this point you’d do pretty much anything to raise the chance of this heat inside you being assuaged. " + (pc.exhibitionism() >= 33 ? "Especially":"Even") + " debasing yourself in front of an eager, ever-growing audience.");

	output("\n\nLund pulls his loincloth aside and looks back at you, smirking. ");
	if (!korgiTranslate()) output("<i>“Lick, bitch.”</i>");
	else output("<i>“Get in there, bitch.”</i>");

	output("\n\nIf this is what it’s going to take to get off, so be it. Grabbing his muscular thighs and leaning in, you close your eyes and let your [pc.tongue] loll out.");
	//why does hasLongTongue() count if you have just a prehensile tongue?
	if (pc.hasTongueFlag(GLOBAL.FLAG_LONG)) output(" The surrounding korgonnes gasp in surprise at the sheer length of it, unfurling from your mouth to caress Lund’s asshole as if you were his lover and not his pet.");
	else if (pc.hasTongueFlag(GLOBAL.FLAG_PREHENSILE)) output(" A murmur goes up from the surrounding korgonnes at your preternatural control over it, wiggling from your mouth to caress Lund’s asshole as if you were his lover and not his pet.");
	else output(" The korgonnes watch eagerly as you lean in and run your tongue along Lund’s asshole as delicately as you can, like you’re his lover rather than his pet.");
	output(" Watching an exotic alien perform certainly seems like a novelty around here, and in your heart of hearts you already know this is just going to be the first of many, many shows for an audience.");
	...
	output("\n\n<i>“Okay,”</i> you hear distantly, as though through water. ");
	if (!korgiTranslate()) output("<i>“Alien good bitch, turn around.”</i>");
	else output("<i>“You’ve been a good bitch; now turn around.”</i>");

	output("\n\n<i>“Oh, god, thank you,”</i> you moan, pulling your tongue back and wiping your mouth off. <i>“Thank you...”</i>");

	output("\n\n<i>“Shhh,”</i> Lund hushes you, twirling his finger in a circle. ");
	if (!korgiTranslate()) output("<i>“Show ass.”</i>");
	else output("<i>“Present yourself.”</i>");

	output("\n\nYou’re so eager to comply you throw yourself to the floor and raise your [pc.ass] up in the air as high as you can, holding it out for Lund - and the audience - to gaze upon. Your" + (vag ? " dripping slit":" [pc.asshole]") + " is practically winking with the need to be fucked full by his giant canine cock, and when Lund puts his hands around your [pc.hips] you almost scream in desire. Fuck, you wish he’d just <i>put it in!</i>");
	...
	output("\n\nYou cry out into the Hold and feel yourself involuntarily tighten around him, his dominion of your body overcoming your mind and forcing you to an explosive orgasm. To the crowd it just looks like you came hard when you were spanked, like a common slut, and as your eyes roll upwards and you grit your teeth in desperate pleasure, you can’t tell if they’re wrong.");
	if (pc.isHerm()) output(" [pc.Cum] flies from the end of your cock while your stuffed-full cunt spasms around his thick cock, clenching and stroking while you try not to pass out from the pleasure");
	else if (pc.hasCock()) output(" [pc.Cum] flies from the end of your cock, splattering several feet in front of you while Lund slams your ass, practically expelling it from your throbbing [pc.cockHeadBiggest]");
	else if (pc.hasVagina()) output(" Your stuffed-full cunt spasms around his thick cock, clenching and stroking while you try not to pass out from the pleasure");
	else output(" It’s not like you have the genitals to reflect it, but everyone can see you arching your back and biting your lip like an absolute whore");
	output(".");
	...
	output("\n\n");
	if (!korgiTranslate()) output("<i>“Take it,”</i>");
	else output("<i>“Now take it,”</i>");
	output(" he snarls, hilting himself as you feel hot, thick cum spew from the end of his dick and his knot begin to swell to an obscene size. <i>“Drink.”</i>");
	...
	pc.loadInMouth(lund);
	if (vag) pc.loadInCunt(lund,x);
	else pc.loadInAss(lund);
	for (var i:int = 0; i < 20; i++) { pc.orgasm(); }
	clearMenu();
	addButton(0,"Next",submitToLundBadEndFinal);
}
public function submitToLundBadEndFinal():void
{
	clearOutput();
	showLund();
	var x:int = -1;
	...
	output("You begin every morning the same way - tied up at the foot of Lund’s bed, you’re awoken by the sensation of his enormous dogcock entering your slickened fuckhole, kept just loose enough to accept him thanks to your multiple daily fuckings.");

	output("\n\n<i>“Oohhh, goooddd,”</i> you moan, dragged from the clutches of sleep and held in your master’s powerful arms as he pumps you from behind. <i>“Uuuhhh! Nnnhhh! Nnn!”</i>");

	output("\n\nHe doesn’t even have to try to make you cum, it just happens.");
	if (pc.isHerm()) output(" Your cock" + (pc.hasCocks() ? "s spurt":" spurts") + " until you have nothing left to give, your vaginal walls getting the massage of your lifetime courtesy of a thick knotted cock");
	else if (pc.hasCock()) output(" Your cock" + (pc.hasCocks() ? "s spurt":" spurts") + " until you have nothing left to give, the frenetic grinding against your prostate leaving you dripping like a faucet even in Lund’s absence");
	else if (pc.hasVagina()) output(" Your puss" + (pc.hasVaginas() ? "ies drip and squirt":"y drips and squirts") + " with every thrust, your vaginal walls getting the massage of your lifetime courtesy of a thick knotted cock");
	else output(" Such is his hold over you that you can’t even remember what life was like without him - going back to being unable to orgasm like this would be the death of you");
	output(". Masturbation is pointless - there’s nothing it can offer you in comparison to your korgonne master.");

	output("\n\n<i>“Yes, yes, yes, yes!”</i> you cry, Lund fucking you so hard your voice shakes. <i>“Oh! Oh!”</i>");
	output("\n\nWith a growl in your ear he cums, sliding himself balls-deep and letting you feel his knot inflate in your stretched, dripping fuckhole before he pulls out and leaves you gaping, your abused, reddened " + (vag ? "cunny":"asshole") + " winking in his wake. A trail of seed runs down your [pc.thighs] as he stretches out, yawning before getting to his feet.");

	output("\n\n");
	if (!korgiTranslate()) output("<i>“I go hunt,”</i>");
	else output("<i>“I’m going to hunt”</i>");
	output(" he says, nodding at your food bowl. ");
	if (!korgiTranslate()) output("<i>“Back soon.”</i>");
	else output("<i>“I’ll be back soon”</i>.");
