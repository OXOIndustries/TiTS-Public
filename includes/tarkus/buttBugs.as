...
public function combatSandWormVictory():void
{
	...
	var swGender:int = rand(2);
	var swfVariant:int = -1;
	
	output("Although the freshly defeated worm sticks halfway out of the ground, it would seem that it is not dead yet. The frontal opening sputters out fresh goo, while what’s left of the body twitches and pulsates as if trying to deposit something. Sure enough, a much more solid object slides through the opening like some sort of strange birthing hole. It flops onto the slimy green sand, giving you time to clearly make out its " + (swGender == 0 ? "oddly segmented, butt plug-shaped form" : "strangely phallic form, attached to a bulbous base with six spindly legs") + ".");
	if(swGender == 0)
	{
	...
	var tEventCall:Function = (function(c_swGender:int, c_swfVariant:int):Function
	{
		return function():void
		{
			...
			else
			{
				...
				if(c_swGender == 0)
				{
					...
					addButton(14, "Leave", combatSandWormVictoryLeave, c_swGender);
				}
				else
				{
...
public function combatSandWormVictoryLeave(swGender:int = 0):void
{
	clearOutput();
	showSandworm();
	author("Preacher");
	
	output("Deciding against doing anything with it for now, you let the " + (swGender == 0 ? "female" : "male") + " be.");
	output("\n\n");
...
public function sexButtBugBionahole(swfVariant:int = 0):void
{
	...
	output(" You align her with your [pc.oneCock] an inch or two above the head, just to build some suspense for yourself. It pays off as the female bug’s oozing juices flow down to cover your [pc.cock " + cIdx + "] with just enough fluids to allow the soft breeze of the wastes to give your dick a slight chill. The tendrils wriggle around in the air above your cock" + (pc.cocks.length == 1 ? "" : "s") + ", before they reach down towards your [pc.cocks].");
...
public function buttBugVolume(swfVariant:int = 0):Number
{
	var vol:Number = 0;
	switch(swfVariant) 
	{
		case 0: vol = 200; break;
		case 1: vol = 300; break;
		case 2: vol = 500; break;
	}
	return vol;
}
public function sexButtBugOfferButt(swfVariant:int = 0):void
{
	...
	// v3: 
	// v4: 
	pc.createStatusEffect("Butt Bug Female", -1, 0, 0, 0, true, "", "", false, 0);
	...
	if(pc.analVirgin)
	{
		...
		pc.buttChange(buttBugVolume(newBug), false);
	}
	else pc.buttChange(buttBugVolume(newBug));
...
public function sexButtBugUse():void
{
	...
	var buttBugM:StorageClass = pc.getStatusEffect("Butt Bug Male");
...
[{if pc.isNude = false: A thought strikes you: what about your gear? You can’t effectively screw a dick with all your stuff on, even if it is a cock with legs. Deciding to take the risk, you set him down and keep your hands close just in case he decides to bolt it. It turns out that he can’t run away anyway, let alone move from where you’ve put him. He corkscrews in the sand with his cock’s head in the middle of the circle like some perverted clock.

Taking the opportunity before he goes soft, you swiftly move to drop you [pc.gear]. The desert wind blowing across your nude form is refreshingly cool and doesn’t even move your [pc.hair] with its delicate breeze. Back to the bug in the sand you pick him up once more and bring him up towards your mouth whilst lowering your form as you lean against the metallic chunk for support. It’ll be a snowy day on tarkus before you let something go into you dry.}]
...
The most the male can do at the moment is allow the female to have her fun, and for now you are in a similar predicament. At least you have the power to make it even more pleasurable for yourself. You [{if pc.firstBreastRowSize > 1: grab a breast}{if pc.firstBreastRowSize < 1: tweak a nipple}] with one hand as you prepare for some self pleasuring.

Moving your other hand you [{if has cock but no pussy: grasp {one of }your cock{s} while stroking it to a made up beat in your head. Occasionally you use a thumb to rub your cock’s glans in a circular fashion before moving back to stroking yourself with more and more pre lube as time goes on.}{if has pussy but no cock: rub at your [pc.pussies], slipping in an entire digit at times to further tease yourself. Your thumb inevitably grazes your{one of your} clit{s} with your continued self-stimulation, each making you moan while they send a delightful jolt of pleasure through you.}{if has cock and pussy: decide to not have either single set of genitalia be the focus of pleasure this time. Grabbing your{one of your} cock{s} with only your thumb and index finger you use your three other digits to stimulate your womanly folds. Shortly pumping at the [pc.base " + cIdx0 + "] of your dick leaves you enough room to manipulate the digits in your cunt{s}. [{if has balls: Your scrotum is used to rub your clit{s}, as you can’t quite reach them{it} directly, their texture making for a uniquely sensual experience}{if doesn’t have balls: Using the remainder of your digits, you slip them in halfway repeatedly until you’ve found yourself a nice rhythm to your jerk and finger combo}].}{if has no genitals: instinctively rub at your bare groin, realizing you have no genitals to speak of you keep going. These rubs somehow gives you a small measure of residual happiness that spreads through your body.}] All the while, you [{if pc.firstBreastRowSize > 1: massage your [pc.oneBreast], digits sinking into your boobflesh as [{if pc.lactation = true: [pc.milk] flows out, streaming down your body and into the dry sand below.}{if pc.lactation = false: occasionally tweak a hardened [pc.nipple] for some small manner of extra pleasure.}]]}{if pc.firstBreastRowSize < 1: tweak a hardened [pc.nipple], the sensitivity of it building as you feel more and more pleasure from the act.}]
...
Being overstimulated like this, you can’t help but increase your pace as well, sweat already dripping down your form from the exertion. [{if has cock but no pussy: Your hand bolts up and down your [pc.oneCock] as the other moves away from your chest to [{if has balls: fondle your swinging balls}{if doesn’t have balls: slip a digit into your parasite-hosting anus along with the male’s tentacle covered shaft}].}{if has pussy but no cock: You can’t handle the teasing anymore. You need more, and the hand on your [pc.chest] won’t suffice. You send it to vigorously finger your folds while the other handles clit duty, and a stream of [pc.girlCum] forms between your legs.}{if has cock and pussy: What were you thinking when you tried to pleasure both your sexes with only one hand? It’s clearly inefficient. Reaching back with your chest-occupied hand, you finger your [pc.pussies] from behind while the other jerks your [pc.oneCock]. It is like an online vid switching to a higher quality while you watch it, utter satisfaction. Exploiting your physiology as best you can you ride out the wave of satisfactory pleasure hitting your mind.}{if has no genitals: Your continued sensual rubs of your bare groin has turned your waves of happiness into those of heat and need. Lusting for more, the hand that caresses your upper body moves to join its twin as you feel hotter with every circular motion.}]

The male’s tentacle-covered cock-body suddenly throbs. Its veins bulge out and you can feel the first of many warm spurts of green cum filling up your backside. A few last powerful thrusts into your parasite occupied anus sends you over the brink as well. Fast-paced exclamations and profanities of pleasure escape you as your [{if has cock but no pussy: [{if has balls: balls go into overdrive}{if doesn’t have balls: prostate clenches hard}] while pumping your [pc.cum] through your shaft{s} to meet the desert sand. [pc.Cocks] still half hard, you collapse with breath ragged, utterly spent into sand.}{if has pussy but no cock: vaginal walls clamp down around your digits while streams of [pc.girlCum] leak out of you. Bringing your hand up to lick yourself clean you breathe a sigh of exertion before sinking downwards to rest.}{if has cock and pussy: herm physiology lets you experience two orgasmic sources at once. Cock{s} [{if has balls: emptying your balls}{if doesn’t have balls: relieving your prostate}] as they{it} release your [pc.cum] into the world. Meanwhile, your [pc.pussies] spastically clenches around your digits and gush forth [pc.girlCum] until you fall onto the sand.}{if has no genitals: your hot fever of need briefly lights up your nerves and explodes inside you. Your mind slowly ebbs into a realm of satisfaction and relief while you cool off and rest for the time being.}]
...
{if hasFemaleInside = false: As you check to make sure you’ve aimed right, you see that the phallic bug looks like he can’t wait to get inside you. However, he still seems hesitant. When his head pokes at your pucker, it pushes the bug to action. His legs extend to press his slickened head past your anal ring. You gasp as it pops into you [{if has balls:, his shaft grinding against the back of your [pc.balls] as}]. You [{pc.analVirgin = true: take your first cock up the ass as far as it will go. </b>Your anal virginity has been taken</b>.}{if pc.analLooseness = 1 or 2: slowly slide him in until your ass hits his base. You enjoy the feeling of him filling you up. This isn’t your first visitor through the backdoor after all.}{if pc.analLooseness = 3 or 4: take his cock like a pro. Your [pc.asshole] slurps it up without a single second of discomfort.}] It’s an interesting feeling to be stuffed with his veiny cock, each throb resonating through your anal walls.

He’s getting restless sitting there with his dick up your bum, and squirms this way and that as he tries to get stimulation in any way possible. You tell him to stop fidgeting in a firm voice, but he either isn’t listening, or doesn’t understand. With your ass hovering above the ground, it’ll be easy to descend to the ground and fully sit on him. Maybe that’ll remind him who’s in charge here. Telling him again to stop, you relax your [pc.legs] until you’re basically sitting in the sand, with your ass cheeks smothering him. With his wide base pressed up against your [pc.asshole] it provides a nice barrier to stop his entire form from entering you. With your weight upon him, his attempts to move have stopped. Maybe now he’ll let you do your own thing.

However, as you lift off of him, he springs to action again, and pistons his legs as he smacks into your ass. [{if has cock but no pussy: Your [pc.cocks] throb each time he hilts into you, and he [{if has balls: rubs your balls as he goes.}{if doesn’t have balls: bumps your prostate on the way in and out.}] Your [pc.cocks] drip [pc.cumColor] precum from the stimulation while the liquid flows down your frenulum.}{if has pussy but no cock: His efforts are hardly right for stimulating one such as yourself, but at the very least his enthusiasm makes up for his lack of skill. However, you need just a little more to truly satisfy yourself. You move one of your hands to rub at your [pc.cunts].}{if has cock and pussy: Both your masculine and feminine organs tingle with residual stimulation as he pounds your ass and [{if has balls: frots against the backside of your [pc.balls].}{if doesn’t have balls: makes droplets of his makeshift lube splatter against your lower labia.}].}{if has no genitals: Strange tingles move through your form while the bug pistons into your backside, more pleasurable than uncomfortable. It feels as if a ten-armed massage expert is having his way with your nerves, and you reflect on the idea that having no genitals doesn’t mean that you have to miss out.}] It seems the little dick hasn’t learned a thing, as three times you sit on him only to go back up with a twelve inch piston in your ass.
...
Your breath hastens as you gradually pick up the pace, losing the gyrations and really kicking your [pc.legs] into high gear. [{if has cock but no pussy: [pc.Cocks] bouncing [{if has balls: and [pc.balls] jiggling}{if doesn’t have balls: and prostate clenching}], even more [pc.cumColor] pre drips from your urethra{s}.}{if has pussy but no cock: Your hand quickens in your ministrations on your labial lips as your thumb polishes your pearl{s}, occasionally slipping inside for a quick frig.}{if has cock and pussy: Each bounce makes your dick{s} [{if has balls: and [pc.balls]}] do cyclical slaps between your stomach and your womanhood, stimulating you as trickles of [pc.girlCumColor] flow in between your legs and [pc.cumColor] pre flings from your urethra{s}.}{if has no genitals: The tingles inside you build up, adding to those from before.}] The sounds of drawn out moans and your ass’s squishy slapping against his base fill the air, lewd enough to arouse anyone within earshot.
...
You cum when his base hits your anal ring. [{if has cock but no pussy: [pc.Cocks] twitching as [{if has balls: your [pc.balls] churn,}{if doesn’t have balls: your spasming prostate clenches,}] the frenum of your cock{s} bulge{s} with your load pumping through your masculinity. [pc.CumColor] bursts from your urethra as you empty your [pc.balls] into the desert air, making a [{if pc.cumVolume < 100: tiny puddle}{if pc.cumVolume < 1000 && pc.cumVolume > 10000: relatively large puddle}{if pc.cumVolume > 10000: huge lake}] of [pc.cum] before draining away in the sand.}{if has pussy but no cock: [pc.Pussies] spasming around your digits, your hips buck with the [{if pc.vaginalWetness = 1: trickling river}{if pc.vaginalWetness = 2 or if pc.vaginalWetness = 3 or if pc.vaginalWetness = 4: occasional squirts}{if pc.vaginalWetness = 5: explosive burst}] of [pc.girlCum], rapidly wetting the ground below you.}{if has cock and pussy: Both sets of your genitals go off at the same time, letting you bask in the euphoria of dual sexed orgasms. Your [pc.pussies] wet{s} the ground below with [{if pc.vaginalWetness = 1: small streams}{if pc.vaginalWetness = 2 or if pc.vaginalWetness = 3 or if pc.vaginalWetness = 4: infrequent squirts}{if pc.vaginalWetness = 5: volumous sprays}] of [pc.girlcum] while your [{if has balls: [pc.balls] draw tight against you}{if doesn’t have balls: overstimulated prostate spastically twitches}] as they{it} pump{s} your [{if pc.cumVolume < 100: small}{if pc.cumVolume < 1000 && pc.cumVolume > 10000: adequate}{if pc.cumVolume > 10000: tremendous}] load of [pc.cum] a small distance away from you, and makes an appropriately sized puddle of cum.}{if has no genitals: Your entire nervous system lights up in a flash of pleasure, leaving your mouth gaping as you’re left motionless for the few seconds your brain struggles to process what’s going on. Slowly you regain environmental awareness and feel tired but satisfied in a way that can only be described as a weird form of happiness.}] The aftermath of your climax still rolling through you, you collapse exhausted into the sand. The insect’s dick pops out soon after as a couple of last squirts fly into the air before he too falls to the grainy earth.
]

[{if hasFemaleInside = true: Having had its fill, the parasite inside you gathers the leftover dripping seed from your cheeks with its tendrils before closing back up. Undulating pressures roll through your anus as the female prepares her freshly fertilized young for incubation, soon to be laid in your gut. Gathering your gear, you look forward to carrying them until they’re ready for the world.}{if hasFemaleInside = false: The male’s seed drips from your [pc.asshole], creating a nice gooey feeling between your asscheeks, and filling the void in your slightly gaping ass for now. You wonder if things would have turned out different had you had a female of the species in you. Maybe next time.}]

[{if hasMaleBug = true: Well that was fun. Hopefully your own little guy won’t be too upset that you kinda cheated on him. You leave the bug there to recover, and prepare to hit the road.}{if hasMaleBug = false: Whew, what a ride! It would be nice if you could have one of these critters with you to fuck whenever you feel like. It’s not like he’d complain. In fact, you could store him in one of your item pods. Do you do so?
[display buttons [capture] and [leave]]
}]
...
[capture]: //Self explanatory, button appears after using the male. The thing it is stored in is my own headcanon for how steele stores shit, with a couple of adaptable inventory storage pods. Then again it might become normal canon if added to game, who knows...
ButtonTIp: Capture the male you’ve just fucked into submission.
Scene:
Thinking you could have some use for the little bugger if you need to relieve yourself again, you walk up to him with an item container in hand and capture in mind. It’s only fair, since he attacked you and all. It seems the fucking really did a number on the little guy, as he’s still as limp as any other post-orgasm cock you’ve seen. Taking advantage of the situation, you grab him by the shaft and lift him up with barely any effort, dropping him into the container.

The lid closes automatically, sealing with a hiss as the small electronic label on it flashes from a green “Open” to a red “Secured” in a matter of seconds. It scans the slowly reviving male butt bug inside of it, and determines the object to be alive and organic before the self-modification system kicks in and transforms itself. It visibly transforms itself in order to keep the organic creatures inside it alive, complete with atmosphere generators, air filters, and even a low grade replicator to give it water and sustenance pellets from appropriate particles in the surrounding environment.
...
Button Tip: Use the bug you captured earlier.
Scene:
As you ponder how you’re going to satisfy yourself this time, small taps against glass hit your auditory senses. Their source seems to emanate from the item pod where your captured male butt bug is stored. A spindly insectoid foot is making the noise, and his phallic body seems half erect at the prospect of intercourse. He knows you too well, and is always at the ready for a bout of fun. A few keystrokes later, and with a hiss the container switches from “secure” to “open”.
...
{if pc has vagina: How shall you take him?
[display buttons [ass] & [vag]]
Else: go straight to ass}

[[display if vag is chosen]
These bugs may be designed to fuck asses, but that doesn’t mean they can’t be used in other ways. The nice thing about them is that they can’t fertilize anything other than their own females, meaning there’s no need for a condom. [{if hasFemaleInside = true: Your anal-lodged parasite might not like this, but it is what you want. And besides,}] {y}Your [pc.pussies] has{have} been craving for some action. [{if pc.vaginalVirginity = true: Although as a virgin, you’re not quite sure what that action is going to be like.}] You lower your form until your vaginal lips are about a foot away from the erect insect. [{if pc has cocks: Your own [pc.cocks] {is}are also hard at the prospect of this encounter.}] One of your hands sneaks back to give the bug a little help in keeping himself upright. He’s definitely a bit too endowed for his own good. His human-like cockhead lies pressed at {one of }your womanly entrance{s} before you sink his phallus through your fleshy veil. [{if pc.vaginalVirginity = true: Your hymen is torn, and you wince at the stinging sensation coming from within you. Pretty soon however, the pain goes away and is completely replaced by a sense of fulfillment and pleasure. <b>Your vaginal virginity has been taken.</b>}] [{if pc has cocks: The insertion has caused some [pc.cumColor] pre to leak from the tips of your cock{s}, and it now drips along the underside of your shaft{s}.}]
...
While he goes about stimulating your vaginal walls, you set out to stimulate parts that he cannot. The hand that was on him now moves to rub at your clit{s} while the other clasps [{if pc has cock: {one of }your cock{s}. You passionately stroke your shaft as more and more pre is coaxed out with every pass, creating a convenient lube in no time at all.}{if pc does not have cock: one of your [pc.nipples]. [{if firstBreastRowSize = 0: Your flat chest makes your nipples the prime receivers of your manual ministrations, and as hard as they are now they’re so delightfully sensitive.}{if firstBreastRowSize > 0: As your thumb and index finger handles your sensitive nipple your other digits dig into your breast flesh as you massage one of your [pc.firstBreastRowSize] tits.}]}]

The bug is actually quite good at what he’s doing. You are uncertain whether it’s enthusiasm or actual skill, but it makes for an enjoyable ride all things considered. However, your greedy pussy wants more and you indulge it without a second thought. You use your [pc.legs] to bounce up and down on the insect’s dick, in tune with his own efforts. Quickened breath and pleasured gasps ensue as the exertion exponentially increases your stimulation.
...
[if pc has cock: Your own masculine organs reciprocate your vagina’s orgasmic pulses. Your hips buck forward as your [{if has balls: [pc.balls] pull tight against you}{if doesn’t have balls: your prostate twinges}] and your [pc.cum] is pumped through your urethra{s} to splatter against on the floor.}] The insect’s phallic organ throbs inside your feminine folds as he too is driven over the edge. He spurts his thick green load, coating your vaginal walls with his seed. His semen unloads within you until the excess dribbles out around him and eventually covers his entire body. Having nowhere else to go, the green sperm then drips down to the floor and adds to the puddle formed by your sexual fluids.

With the both of you drained of your energy and fluids, there is little that can be done other than collapse onto the floor. Your bug’s cock slides out of your vaginal lips, limp and completely spent, while his verdant semen oozes out in little dribbles. [{if hasFemaleInside = true: A tendril from your anal tenant briefly shoots out of her protruding orifice and slaps your ass in a whip-like motion. This causes an ‘eep’ to escape your mouth and leaves quite the mark on your bum. It was a bit physical, but you guess this is her way of scolding you for leaving her out of the fun.}]
]

[[display if ass is chosen or doesn’t have vagina]
You’ve been itching for some butt fun, and you know just the expert for the job. [{if hasFemaleInside = true: Judging from the leakage already coming from her poked out orifice, you’re pretty sure your own hilinara parasite is going to like this as well.}] You lower your hindquarters so that your puckered target is closer to the frantically enthusiastic body of the male insect. [{if hasFemaleInside = true: The closer your ass gets to him, the more impatient the female inside you gets. Her tiny feet wiggle against your anal walls, tickling the insides of your rectum and forcing a sultry giggle out of you.}] You use a free hand to help the male bug stay upright despite his 10 inch veiny length. Like a rocket that’s ready to pierce the heavens he is ready and raring to penetrate your [{if hasFemaleInside = true:–or rather her–}] anal star.

[{if hasFemaleInside = true: Before you have a chance do it yourself, the tentacles of the female parasite burst out to wrap around the shaft. Their slimy surfaces slither over and under your grasp until they’ve pried your digits from their objective. Not that they needed to. She lifts him from the ground and inserts his slimy tentacle-covered cock into the drippy anus you share with her. It feels like two dicks have entered you at once, yet you know that it’s only your ass spreading as you experience the stretching of the female’s own walls. Due to the female’s link with your nerves, you feel everything that is happening down there, from the swirls of the tendrils to the veiny girth of the male’s penis against the walls of the parasite’s insides.

As these sensations build up, it becomes impossible to keep your hands off yourself. One hand [{if has cock but no pussy: moves to jerk [pc.oneCock] as the other [{if has balls: rolls your [pc.balls] around in its palm.}{if doesn’t have balls: rubs its digits along the sensitive spots of your glans.}].}{if has pussy but no cock: rubs itself along your vaginal slit{s} as the other occasionally spreads them open so the first can dig deeper.}{if has cock and pussy: grips the base of [pc.oneCock] and sensually jerks your meat. The other [{if has balls: maneuvers around your [pc.balls]}{if doesn’t have balls: slides in between your [pc.legs]}] as you use some of your fingers to spread your lips while the others jill{s} {one of }your vagina{s}.}{if has no genitals: somehow finds itself against your bare groin while the other rubs at your chest. No matter how hard you try, the instinctual need to stimulate yourself is met with subpar sensitivity that amounts to little more than a tickle. Luckily the efforts of the butt bugs, along with the female’s senses and your roaming hands is enough to provide all the stimulation you need.}] With your hands out of their penetrative equation, you’ve been promoted to being the person who masturbates in a threesome. The only difference is that since the other two are fucking inside of you, it feels like you are the one that is receiving the dicking.
...
Your anal-embedded parasite squirts her own greenish liquid glee out around him onto the floor as [{if has cock but no pussy: [{if has balls: your balls churn}{if doesn’t have balls: prostate clenches}] and pump {its}their [pc.cum] load through your urethra{s}. Streams of your jizz fly through the air to splatter on whatever surface they find until your supply runs out and you’re left with half hard cock{s}.}{if has pussy but no cock: your digits dig deep into your vagina{s} one last time so there is at least something to squeeze around. Each of your phalanx appendages feels the force of your muscles clenching around them while they are covered in your [pc.girlcum]. The hand on your clit{s} tries to milk the orgasm for as long as it can, but you’re soon just too sensitive to touch, and you resolve to just ride out what’s left of your pleasures.}{if has cock and pussy: both your masculine and feminine organs undergo their own separate orgasms. The veins of your [pc.cocks] throb while its{their} urethra{s} bulge{s}, pumping your seed out from [{if has balls: your balls}{if doesn’t have balls: your prostate}] to meet the brisk air and make stains wherever it lands. Your vaginal lips quiver from their stimulation as they too wet the ground and your digits with their [pc.girlcum] discharge.}{if has no genitals: your mind undergoes orgasmic highs that are both yours and not. The sexual bliss from the female in your ass mingles with the explosion of nirvana throughout the entirety of your form, leaving your mouth agape from sensory overload. Not long afterwards you regain conscious control of your body, with the slowly softening cock of the male still in the grip of your parasite.}]
...
Every thrust feels as intense as the initial insertion as it noisily slams itself against your derriere. [{if pc.firstRowBreastSize > 1: Your breasts sway as your body rocks with the force of your living dildo’s efforts.}] Your thoughts don’t extend any further than keeping your balance as [{if has cock but no pussy: your cock{s} bob{s} with the reverberations of the ass-pounding you’re receiving. The underside of the male’s cock bumps your prostate each time he fully hilts himself, which jumpstarts your pre production. In no time at all your shaft{s} have{has} been covered in their{its} own [pc.cumColor] fluids.}{if has pussy but no cock: drops of [pc.girlcumColor] drip from the valley of your vaginal lips. While the base of the bug continues to slam against your ass, your inner labia quiver in response to the anal pleasure you’re receiving. The proof of your pleasures puddle below you and occasionally connect you and the floor with strands of [pc.girlcum].}{if has cock and pussy: your phallus{es} jump up and down in the air and slap against your lower belly. The muscles of your cunt{s} clench{es} with every lunge of the insect’s cock, and their{its} [pc.girlcumColor] lubrication increases in tandem to the amount of pleasure hitting your nerves.}{if has no genitals: a sense of warmth spreads from your ass towards to the rest of your body. It’s an indescribable heat,, almost as if a passionate flame has sparked itself within you. It builds and seems to have focussed upon the bare [pc.skinFurScales] of your groin as it becomes a source of pleasure on your mind.}]
...
With barely a second to prepare, [{if has cock but no pussy: your cock{s} throb{s} powerfully before its{their} urethra{s} dilate{s} with the oncoming liquid pressure. Your hips buck and [{if has balls: your balls draw tight against you}{if doesn’t have balls: your prostate clenches within you}] while the stream{s} of [pc.cum] from your [pc.cocks] spray and stain whatever is in their{its} path.}{if has pussy but no cock: your womanhood{s} release their{its} feminine fluids and add to the already present puddle of [pc.girlcum]. The muscles within your feminine folds try to spastically clench onto a cock that isn’t there, causing you to buckle in the throes of your bliss.}{if has cock and pussy: both phallic and vaginal sides of your genitals go into orgasmic overdrive. [{if has balls: Balls drawing tight}{if doesn’t have balls: Prostate clenching hard}], your hips buck forward while [pc.cum] flies from your dick{s}. Meanwhile, the floor below you gets drenched in [pc.girlCum] from the spasms of your vagina{s}.}{if has no genitals: your nerve system is set aflame in a lustful explosion. Your oversensitive bare groin and ass that radiate an ecstasy that leaves only calm happiness in its wake.}] The bug’s legs give out, and so does the rigidity of his penis. It slips out of you as the male insect collapses to the floor in a wet splatter of fluids. He’s so winded he doesn’t even care about his own seed that oozes out of your anus and onto his softening body. Post orgasmic drowsiness creeps up on you as well, and you become more sure by the minute that it wouldn’t hurt if you took a little nap.
}
]
...
Recovering before you, the male bug slowly scampers towards his pod like a good little insect, climbing back in to rest further. It’s good to know he won’t just run off after you’ve used him. Then again he could have an ulterior motive. Who knows? You gather your gear before you once more lock the pod with the phallic insect inside it. Bringing this guy along was definitely a good idea.
 ...
 [Leave]: appears after combat and after using the bug: Deciding against doing anything with it for now, you let the male be.
...
Loss:
Desc:
[{if lost through lust: Slime covered and breathing raggedly, you find yourself unable to resist your aphrodisiac-induced lusts. You fall to the ground, trying to relieve yourself. [{if pc.isNude = false: Your clothing just can’t come off fast enough, and there is no hesitation in the decision to rub yourself through it.}{if pc.isNude = true: You just can’t help yourself; your hands on your [pc.groin] feel so good.}]}{if lost through health: Bruised and beaten, that last knock hit you a little too hard, and now you’re finding it difficult to recover. No matter how you try, you just can’t find the energy to stand up or escape.}] 

After silently checking you out for a moment, the worm re-burrows out of sight. For a second, it seems as if you lucked out of this encounter, but the thought quickly vanishes from your mind as tremors cause the ground to shake. An eruption occurs, and a column of sand rockets burst from the dunes before it falls back down. As the column crashes towards the ground, it scatters dust everywhere and forces you to [{if lost through lust: stop your masturbation and}{if lost through health: gather what strength you can to}] cover your face. When the dust settles, and you can finally see again, you feast your eyes on the full form of the worm in front of you. Only now do you see just how massive it actually is! It is almost twenty times longer than what you saw during the fight, like five or six mega busses placed in a row, front to back. The width however stays relatively uniform from its head until it tapers around its tail end. 
...
But you can’t dwell on that now. It’s either going to eat you, screw you, or give you a parasite, and none of those options sound really good. [{if lost through lust: A nice screw would definitely help your little predicament, but then again, if it’s the parasitic kind it might not turn out in your favor.}][{if hasFemaleInside = true: The parasite you already have is giving you enough issues as it is.}]

You do your best to shimmy away, but your futile attempt to escape has only reminded you of the situation you’re in. The worm is much faster than you’ll ever be at this moment, and it’s right on you in seconds. From behind its jaws, there emerges four sets of pincer-ended appendages that all snap at your own limbs. Thoughts go through your head about whether it’s trying to kill you or restrain you as you move your arms and [pc.legs] in an evasive manner. Your answer is given when it snatches one of your arms and you discover that there isn’t a single sharp section in the entirety of the crab-like claws. Just lines upon lines of lime-colored soft tissue that act more like human fingertips than anything else.

Your moment of inspection has given the worm a perfect window of opportunity to get a hold of the rest of your body. [{if pc is bipedal: You find yourself on your back, with both your legs and now your other arm trapped in the clutches of the}{if pc is taur: Rather awkwardly, but still strongly held, you’ve ended up looking like one of those animals that get carried by their legs. Now in the grip of the}{if pc is naga: You’d think your coils would prove to be an issue for the sand worm, but no. Perhaps because it has fought with other worms in the past, it soon has your other arm as well as the entirely of your lower body’s length in its grip. You are now immobilized by the}{if pc is goo: Unfortunately for the worm, your goopy body slips out of its pincers with little to no effort. The beast is momentarily stumped over what to do with you, and you are briefly filled with hope. However, that hope is very short-lived as the creature begins to drool an almost solid form of its slime onto you, which mixes with your own goo and dries it out very quickly. The consistency of your goop changes to that of a gummy candy which doesn’t hurt, but it has made it easier for the worm to actually grab onto you. Now it is child’s play for it to restrain you, and you find yourself completely in the clutches of the}] cylindrical beast.
...
With the six tentacles in its mouth, the creature has its way with what’s on your body. Expertly, it maneuvers its appendages to remove your [pc.gear] and chuck it sideways. Once your [{if pc.isNude = false: now}] nude body is on display, slippery sounds emanate from its mouth as its pincers shift your form in such a manner as to lift your ass and expose your [pc.asshole]. When the sounds stop, it opens its mouth and starts drooling more of its slime. This time the target seems to be your exposed pucker. The goop is about body temperature and is actually sorta soothing in a strange way.

The way it drips into your crack, only to be spread around and liberally cover your ass is so sensual. It is actually slippery enough that very miniscule amounts of it find their way into your anus regardless of your tightness. You could sell this stuff as top market lube! The sensuality of the situation has even [{if has cock but no pussy: made [pc.eachCock] harden and throb}{if has pussy but no cock: made [pc.eachVagina] moisten}{if has cock and pussy: caused [pc.eachCock] and [pc.eachvagina] to swell}{if has no genitals: your bare groin to heighten in sensitivity}]t. Wait a minute. You know exactly what this thing’s slime does. It’s spiking your hormones while it lubes your ass for [{if sillymode = true: its anal shenanigans.}{if sillymode = false: what it has in mind.}]

Shlicks and other slippery sounds divert your eyes to their source, and you’re greeted with the sight of several of its smooth tentacles making their way towards your anus. [{if sillymode = true: You aren’t being filmed are you? A quick look around says no, but you’re hoping that you don’t end up on one of those sites that are loaded with all kinds of tentacle porn.}] What you’ve surmised to be it’s two-inch-thick tongues are already at your ass.

Each one looks as if it’s reluctant to go first. All of a sudden, the one in the middle prods its tip against your anal ring and pops it into you with little to no effort at all. [{if pc.analVirginity = true: What a way to lose your anal virginity: not a struggle, not an ounce of pain, and no uncomfortable stretching. Okay, you might be lying about the last one. It kind of felt as if your anus was one of those lengthy party balloons being filled with a slimy tentacle instead of air. </b>You’ve lost your anal virginity.</b> {set pc.analVirginity to false && {if analLooseness = 0: set analLooseness to 1}}}]

Your [pc.lips] pucker and your eyes squint at its uncomfortably effortless entry. Meanwhile, the tentacle wiggles around inside your rectum and spreads the slime further into you. You groan as your [pc.hips] [{if lost through lust: buck along with the rhythm of the tentacle spreading lube in your anal passage, [pc.groin] further pumped with blood with your enthusiasm.}{if lost through health: shake this way and that in an attempt to deter the worm from its anal invasion.}] The other tentacles prod their tips at your ass beside the first one as well.

They poke and push [{if pc.analLooseness = 1:, but only the first fits inside you.}{if pc.analLooseness = 2: until a second slips in alongside its brother.}{if pc.analLooseness = 3: until two extra tongue tentacles slide into your ass.}{if pc.analLooseness = 4: until four of its lengthy tongues manages to pop in with the first.}{if pc.analLooseness = 5: until all six of its tentacular tongues slip into your rectal passage.}]. [{if analLooseness = 1: It is}{if analLooseness: > 1: They are}] really digging into your ass, almost as if the worm is looking for something. After a couple of thrusts and inquisitive turns, [{if analLooseness = 1: it retracts}{if analLooseness: > 1: they retract}] from [{if hasFemaleInside = true: ‘your’ anal cavity. It must’ve found the parasite inside your rectum, and somehow you get the impression that they are kinda [{if swGender = 0: pissed}{if swGender = 1: glad}] with the discovery.}{if hasFemaleInside = false: your anal cavity, and somehow you get the impression that they are kinda [{if swGender = 0: glad}{if swGender = 1: pissed}] with the discovery.}]
[Insert next button]

[{(if swGender = 0 && hasFemaleInside = true) or (if swGender = 1 && hasFemaleInside = true) or (if swGender = 0 && hasFemaleInside = false): 
[{if swGender = 0: 
[{if hasFemaleInside = false: 
Uh oh... that means this must be the female parasite, and by the looks of your predicament it doesn’t seem like you’ll be walking away from this [{if pc has any other parasites: without another parasite}{if pc has no parasites: alone}]. Instinct takes over as this realization causes adrenaline to rush through your body. Strength born from desperation surges through you, and your struggle against the crab-like claws begins anew. You don’t know how you’re doing it, considering your previous state, but the once seemingly indestructible limbs that held you in place now shake from your thrashing.
...
Impact shoots through your form as she repeatedly smashes you into the sand. If it were any other sort of ground you’d really be receiving some serious damage, but all this really does is bruise you and sap your resolve. Sense of direction vanishes [{if sillymode = true: as the blue danube plays in your head}], and the energy you gained from your adrenal reserves is slowly being drained from you. Any time it seems like you might try something, she once again thrashes you into the reasonably soft ground. Then she stops, and you’re raised once more into the air so that she can see whether or not she’s finally beaten the resistance out of you. It doesn’t take long before she truly has; you can still move, but it takes so much effort to do so that it doesn’t seem worth it. Now that her target is unable to do anything but submit, she can finally commence with her original plan for you. Lower to the ground you go until you’re almost a meter from the sand, and it is then that you hear odd creaks and cracks coming from the worm.
...
More and more of its ringed form inches out while tiny nub-like legs reminiscent of a caterpillar’s can be seen underneath it. This must be the true form of the female controlling the worm. Your eyes widen as six tentacular tendrils deploy from her frontal orifice and wiggle in the air in front of you. Her tendrils touch the [pc.skinFurScales] of your butt cheeks before she completely plops out of the slit and onto the top of your [pc.asshole] like some strange form of birth. Though she is still relatively ooze covered, you can make out her [{if swfVariant = 0: beige colors and smooth, featureless textures}{if swfVariant = 1: dark green colors and the lighter green nubs that dot her otherwise smooth dermis}{if swfVariant = 2: orange stripe going along her top and bottom. This parasite’s much larger than she originally seemed. It’ll take a long time for your ass to recover from this, if it ever does}].
...
Heart beating, your blood rushes through your body, causing your [{if has cock: [pc.cocks] to [{if pc has sheath: slide from your [pc.sheath] as they}{if pc has genital slit: slip from your genital slit as they}] harden and visibly bulge with their veins [{if has pussy: . Your [pc.vaginas] go flush with need as well, and are already a drippy wet mess}]}{if has pussy but no cock: [pc.vaginas] to blush with need as your girly fluids drip from your feminine folds}{if has no genitals: barren groin to flush with the need to be touched}]. You could swear that there was pure evil in her bug-like eyes as she moves her feeler-like tendrils to much more naturally sensitive areas. It’s only light contact, but it feels like you’ve been edged for hours as she [{if has cock: twirls her tendrils once or twice around [pc.eachCock] [{if has pussy: and rubs their sides in between your vaginal lips}]}{if has pussy but no cock: lightly prods the ends of her tendrils into [pc.eachCunt] and occasionally flicks your [pc.eachClit]}{if has no genitals: tickles your bare groin with each of her six tendrils}]. Her tendrils are like brushes that paint orgasmic pleasure across your extremities while she forces loud moans out of you with every ounce of air your lungs have.
...
As each of the rings that make up her body press up against your anal ring, [{if swfVariant = 0: they smoothly slip in }{if swfVariant = 1: the nubs that decorate her body scrape against your ring as they pop in}{if swfVariant = 2: they stretch your ring even more. Each of her body rings thicken considerably as they slide in}] and make a squelching noise when your backdoor re-clenches around the next section. The parasite is widest at her half-way point; she makes you groan in strangely pleasurable discomfort while her girthiest body ring stretches you. With held breath and clenched teeth, you brace yourself.
...
A wave of muscle spasms roll through you as you experience an orgasm enhanced by the aphrodisiacs that now course through you. [{if has cock: Your [pc.cocks] unleash{es} [{if pc.cumVolume < 1000: several thin spurts and dots your body}{if pc.cumVolume > 1000 && if pc.cumVolume < 10000: thick volumous sprays and makes a mess of your body}{if pc.cumVolume > 10000: geyser-like arcs and utterly covers your body}] with [pc.cum] [{if has pussy:, while your [pc.cunts] spastically clench{es} and[{if pc.vaginalWetness = 1: trickles}{if pc.vaginalWetness = 2: drips}{if pc.vaginalWetness = 3: secretes a stream of}{if pc.vaginalWetness = 4: squirts}{if pc.vaginalWetness = 5: explosively squirts}] [pc.girlCum]}]}.{if has pussy but no cock: Your [pc.cunts] clench{es} with orgasmic spasms and[{if pc.vaginalWetness = 1: trickle{s}}{if pc.vaginalWetness = 2: drip{s}}{if pc.vaginalWetness = 3: secrete{s} {a }stream{s}{ of}}{if pc.vaginalWetness = 4: squirt{s}}{if pc.vaginalWetness = 5: explosively squirt{s}}] {their}its [pc.girlCum].}{if has no genitals: Your nerves explode with a lustful heat that radiates outwards from your [pc.asshole] and bare groin. As the sensations ebb from you, they leave only bliss and the occasional muscle twitch.}] One last twitch in your limbs sends you falling towards the sand not far below as the pincers of the worm snap off with a loud crack, shattering once you hit the muddy mess that is the ground. To crawl away from the statuesque worm is all you can really do at the moment. Ass still sore, and the rest of you still sensitive from the female’s deeds, you slowly regain your strength as you regather your [pc.gear]. [{if pc is goo: At least your gooey consistency has more or less returned to normal.}] Maybe you could try and extricate the worm from your asshole?
...
You pull at it, but every time you do so there’s extreme pain that shoots through your spine that almost causes you to lose your grip. Could she have already merged with you? [{if pc is goo: Are they so good with goop that they can even stay inside your body?}] Curious to check, your free hand is not whipped away in this instance. She must be acting cautiously, not wanting you to grab any more of her little tentacles. With inquisitive digits, you explore your butthole with tactile sensory reception as your guide.

She’s [{if swfVariant = 0: not that big all things considered, but she does remind you of one of those large butt plugs you’d see in anal focused videos on the extranet. [{if pc.analLooseness < 2: You’ve been stretched out a little though. {set pc.analLooseness to 2}}]}{if swfVariant = 1: a little bigger than you thought, and really reminiscent of those bluntly spiked butt plugs in more niche anal toy markets. [{if pc.analLooseness < 3: It was rough but you made it. Your anal ring has been stretched by a reasonable degree. {set pc.analLooseness to 3}}]}{if swfVariant = 2: absolutely fucking huge! How the hell is your ass even able to take her!? You could now be nominated for size queen contests and possibly win every time. [{if pc.analLooseness < 5: You could probably hide a rolled up pillow in your ass now if there wasn’t already a parasite in there. Maybe even then. {set pc.analLooseness to 5}}]}]

It feels as if your pucker is just a little bit more pronounced than usual, and it would seem that it still feels like your own anal ring. But you’re not receiving a direct nerve signal, almost as if you’re experiencing sensation through her. You take a snapshot of your pucker with your codex as your asscheek is spread by the tentacle in your hand. [{if pc.exhibitionism < 50: You’ve gotta remember to delete it after this. It’d be too weird and embarrassing to explain the contents of the picture to anybody}{if pc.exhibitionism > 50: It’s gonna be so hot to show this close-up to everyone you know later, you’re already tingling with excitement at the prospect of people having such a personal picture of you with them}]. It’s kind of a close shot but there is enough detail to discern what’s going on.
...
{set defendingFemale to BBFInside}
{set invadingFemale to swfVariant}
...
The heightened passion in your blood has [{if has cock: [pc.eachCock] throbbing as they{it} [{if pc has sheath: slide{s} from your [pc.sheath] and}{if pc has genital slit: slip{s} from your genital slit and}] rise{s} to full erectness. [{if has pussy: Your [pc.cunts] flush{es} with your lust as well, having already started to drip [pc.girlCum]}].}{if has pussy but no cock: [pc.eachCunt] quivering with a need to be touched. Your [pc.clits] have even made an appearance, and accompany the leakage of [pc.girlCum] from your feminine folds.}{if has no genitals: broken the knob off on your bare groin's sensitivity levels, to the point of the entire region being like one giant clit. Even the movement of your skin around your groin is enough to bring more pleasure to your mind.}]

There is little time before the chitin gets too weak to keep you, and the female knows this. Not to mention that the fight with the rival female parasite you're already housing is going to eat up her time as well. So lost in the throes of your pleasures you are that you barely even notice the female plopping out of the vagina-like slit. She lands squarely onto your ass, and your [p.hips] twitch as she makes contact with your posterior. A quick glance tells you she's [{if swfVariant = 0: beige and relatively featureless}{if swfVariant = 1: a dark green with lighter green nubs}{if swfVariant = 2: quite large with an orange stripe across her top and bottom}]. Her nubby little feet wiggle across your asscrack and produce a sensual vibratory sensation that gets stronger the closer she gets to your [pc.asshole].
...
You see a chance to end this quickly, but your incessant needs scream at you to take care of them. You've no choice but to try and do both at the same time. Instinctually, one of your hands go to your [pc.groin] while the other already has its digits prodding against your [pc.asshole]. There is no greater pleasure in this situation than to finally have your hand on [{if has cock: {one of }your cocks{s} after you've been unable to stimulate them{it} this entire time. Unfortunately though, you can't use both hands for stimulation, [{if has pussy: and your [pc.cunts] will have to wait for now, as}]}{if has pussy but no cock: your vaginal lips to give them the stimulation they've been craving. Proper fingerfucking will have to wait though, as}{if has no genitals: bare sensitive groin. There isn't any genitals there but it may as well be an erogenous zone on its own. If only you could use two of your hands but there is no time for that, but}] you have a few bugs to remove. It's sensitive, but your overstretched pucker makes it easy for your entire hand to fit inside.
...
Moaning and breathing raggedly, your orgasm hits you like a ton of bricks as the entire bug is finally pulled out with a squelching sound that comes from your rapidly closing anus. She is chucked to the side as [{if has cock: your [pc.cocks] unleash [{if pc.cumVolume < 1000: several thin spurts, dotting your body}{if pc.cumVolume > 1000 && if pc.cumVolume < 10000: thick volumous sprays, making a mess of your body}{if pc.cumVolume > 10000: geyser-like arcs, utterly covering your body}] with [pc.cum] [{if has pussy: while your [pc.cunts] spastically clench{es} and[{if pc.vaginalWetness = 1: trickles}{if pc.vaginalWetness = 2: drips}{if pc.vaginalWetness = 3: secretes a stream of}{if pc.vaginalWetness = 4: squirts}{if pc.vaginalWetness = 5: explosively squirts}] [pc.girlCum]}].}{if has pussy but no cock: your [pc.cunts] clench{es} with orgasmic spasms and[{if pc.vaginalWetness = 1: trickle{s}}{if pc.vaginalWetness = 2: drip{s}}{if pc.vaginalWetness = 3: secrete{s} a stream of}{if pc.vaginalWetness = 4: squirt{s}}{if pc.vaginalWetness = 5: explosively squirt{s}}] their [pc.girlCum].}{if has no genitals: your nerves explode with a lustful heat that radiates outwards from your [pc.asshole] and bare groin. As the sensations ebb from you, they leave only bliss and the occasional muscle twitch.}] As you gather your breath, you look to the side where the defeated bug lies.

By the looks of it, it seems your original parasite has [{if invasionSuccess = true: lost}{if invasionSuccess = false: won}]. [{if defendingFemale = invadingFemale: Then again, how could you know? The two parasites were identical, after all.}] Your [pc.hips] twitches once more as you feel another pinch in your ass [{if pc has cock: next to your prostate. It pushes just a little bit more cum from your cock{s} as you realize what's happened.}] The winner must've just merged with you! You curse to the skies [{if sillymode = true: and throw a little tantrum}] before regathering your [pc.gear] and walk off with a sore ass and a body that's just a little too sensitive.
}]
}
{if swGender = 1: 
Now that she's been properly roused, your parasite's tendrils slip out of your [pc.asshole] one by one. Each one that comes out makes slippery wet sounds and elicits a brief little groan from you. Finally, all four of her available tiny tapered tentacles wiggle in the air. What could only be the male in the worm sees this as a sign of acceptance from the female inside you, and already moves his worm into position to hold you while they fuck. The sandworm creaks and cracks while it slowly petrifies itself into a statue made of chitin. As soon as there is not a single inch of movement from the large creature, the chitinous forehead of the worm splits open in front of your ass and reveals its lime green flesh. An oddly vaginal slit sits in the middle of it all, and starts to ooze an emerald colored slime that soon starts to drip towards the sand below.
...
With no time to lose, he flops his large dick over himself, slapping it squarely into your asscrack and the embrace of your female's tendrils. Some of his cock teasingly rubs against [{if has cock: the underside of your [pc.cocks] and [{if pc has balls: balls}{if pc doesn't have balls: taint}], [{if has pussy: while some of it slides along your [pc.cunts]}].}{if has pussy but no cock: your vagina, and even hits your [pc.clits] along the way.}{if has no genitals: the flat of your bare groin}]. You can still feel the sensations of your extremities, even though the rest of you is mostly numb. There is a large cracking sound from the larger worm and suddenly both of the bugs emit a tiny, frightened squeal. For good reason too, as you could scarcely gather your thoughts before all of the pincers holding you snap under your weight.
...
Your arm moves to position the male's cock head in front of your bug-occupied anus while his shaft is quickly surrounded by her twirling tentacles. The parasite's lusts become your own as she wiggles within you and makes your [{if has cock: [pc.cocks] get erect and ooze [pc.cumColor] precum with the female's stimulation of your prostate [{if has pussy: while your [pc.cunts] twitch and leak their [pc.girlCumColor] fluids}].}{if has pussy but no cock: vaginal lips twitch with need while your [pc.clits] poke from their hoods and [pc.girlCum] drips from your folds.}{if has no genitals: flat groin sensitive with the need to be touched and caressed.}]
...
As you feel the duality of your and her anally penetrative feelings, your controlled hand finds [{if has cock: [pc.oneCock] and jerks it with unfamiliar yet pleasurable movements. [{if pc.cocks > 1: The rest of your masculinity simply throbs futilely in the air, being ignored by the puppeted ministrations.}] [{if has pussy: There is a twinge in your clefts, which causes the female to temporarily stop her jerking of your cock. Now aware of another source of pleasure, she leaves the stimulation of your dick to your thumb and index finger while she jills you vaginal lips with whatever digits are in range.}]}{if has pussy but no cock: your [pc.cunts] and gives one long palm rub through the entirety of your pleasurable femininity. She finds [pc.oneCunt] too irresistable not to play with, and focuses all of your hand's controlled attentions to vigorously frigging your feminine folds.}{if has no genitals: your featureless groin and gives it one or two testing pokes before she going utterly crazy in rubbing its flat, sensitive surface.}]
...
The deep thrusts into the female in your ass increase in frequency, and the sources of pleasure from your own senses and hers are quick to bring you to orgasm. Both you and the female experience a simultaneous orgasm when she clamps down onto him and the male squirts his slimy green cum into your and her depths. All three of you convulse as you senses are overloaded and [{if has cock: your [pc.cocks] unleash{es} [{if pc.cumVolume < 1000: several thin spurts}{if pc.cumVolume > 1000 && if pc.cumVolume < 10000: thick volumous sprays, making a tiny puddle}{if pc.cumVolume > 10000: geyser-like arcs, making a large lake}] of [pc.cum] onto the sand. [{if has pussy: Meanwhile your [pc.cunts] spastically clench{es} and [{if pc.vaginalWetness = 1: trickle{s}}{if pc.vaginalWetness = 2: drip{s}}{if pc.vaginalWetness = 3: secrete{s} a stream of}{if pc.vaginalWetness = 4: squirt{s}}{if pc.vaginalWetness = 5: explosively squirt{s}}] [pc.girlCum]}].}{if has pussy but no cock: your [pc.cunts] clench{es} with orgasmic spasms while[{if pc.vaginalWetness = 1: trickling}{if pc.vaginalWetness = 2: dripping}{if pc.vaginalWetness = 3: secreting a stream of}{if pc.vaginalWetness = 4: squirting}{if pc.vaginalWetness = 5: explosively squirting}] their [pc.girlCum].}{if has no genitals: the explosion of ecstasy radiating from your bare groin temporarily short circuits the female's connection with your body. The sensory traffic sends your body into a twitchy mess of muscular movement until your climax ebbs from you, and control falls half and half into the female's field.}]
...
[{Taken over bad end scene:
Unfortunately for you, your will is simply not strong enough to resist when she returns to taking full control of your body. Though your mind is still untouched, the rest of your head joins the fray of what's under her control. The male flops out of the female's cavity, falls to the sand, and loses his erection, at which point he he can finally move around on his own. What comes next is hard to say for certain.
Ll
...
Warm wetness runs down your sides as you see two of his tongues appear on both sides your chest. They tickle you and leave slick trails of green saliva wherever they slither and twirl around the [{if pc.firstBreastRowSize > 0: peaks of [{if pc.breastRows > 1: the top row of}] your [pc.firstBreastRowSize] breasts.}{if pc.firstBreastRowSize = 0: pecs on your flat chest}]. Your [pc.nipples] are flicked and played with until [{if pc has dick nipples: their [pc.dickNipples] slide out. The tongues seamlessly adapt to the situation, and now entwine around and jerk your newly revealed shafts, slickening them with slime from the tentacular tongues.}{{if pc has nipple cunts: you gasp as your vaginal nipple gashes open up and secrete their juices. The tentacular tongues find your [pc.nippleCunts] irresistible, and before you can even recover from their previous teasing, they penetrate your nipples with their tapered tips.}{{if pc has inverted nipples: they harden and your hidden nipples pop out to join the party. With your sensitive nubs in the fray, the tongues swirl around them and mercilessly tweak your [pc.nipples].}{{if pc has normal nipples: they harden under the teasing of the tongues which continue to stimulate your [pc.nipples] even then.}{{if pc has tentacle nipples: your [pc.nippleColor] tentacular nipples come out to play. Even your own nipples have betrayed you. They play with the slender tongues while you receive all of their sensitive tactile feedback.}]

Moans of pleasure escape you as a result of all the teasing, but they're cut off as a third tongue slides over your shoulder and plugs your mouth with several inches of its slippery tip. No matter where your head shakes, the prehensile oral organ answers your resistance with clever twists and turns that keep it from slipping out. Like a tentacle beast, he forces his tentacle into your mouth and down your throat and slips his tongue in and out until a frothy mess forms around your [pc.lips]. You'd bite down, but then you'd either choke on its length within your throat, or get killed afterwards. So for now you let it do what it wants. The mixed saliva froth spreads around the majority of his tongue's length, and dribbles down your chin. Most of it forms a stream that flows down your [pc.chest], continues past your [{if pc is bipedal, naga or goo: groin}{if pc is taur: tauric half's front}], and forms strands as it falls to the sand below.

All further sounds from you muffled, he continues his oral assault on your body. His other three tapered tongues [{if pc is bipedal, naga or goo: sneak their way towards your lower back. One pokes its tip into your asscrack, while the other two fork away from each other to go around your [pc.hips] while pointing straight at your [pc.groin].}{if pc is taur: wander over the back flanks of your multi-legged half. One slithers between your tauric ass cheeks, while the other two continue their trip across your body until [{if has cock but no pussy: they find [pc.eachCock] below your lower body.}{if has pussy but no cock: they find [pc.eachCunt] at the back of your body.}{if has cock and pussy: one finds [pc.eachCock] below your lower body while the other finds [pc.eachCunt] beneath your lubed pucker.}{if has no genitals: he realizes that you truly have no genitals to speak of. His targetless tongues resolve to join their brother at your pre-lubed asshole.}]}].
The first of his last three tongues delves further into your [pc.ass], where it finds your [pc.asshole] and without hesitation rams itself into your anal depths. You give grunts of discomfort that barely escape your tongue-stuffed mouth as several feet of its length is continuously shoved into your rectum. With most of the lube having dripped out of your ass you can now actually feel how your anal cavity is stretching to fit its two inch girth[{if pc.analLooseness > 2: , not that there is much stretching involved for your experienced asshole}]. Every thrust of tongue flesh bulges out your [{if pc is taur: tauric half's}] abdomen, occasionally invading the deepest part of your intestinal tract and makes you feel each inch of just how stuffed you are!. 

The last two tongues have nothing to do but torment you and their target, your [{if pc has genitals: [pc.groin]}{if pc has no genitals: [pc.asshole]}] further. Their tips poke [{if has cock but no pussy: into the sides of your shaft{s}. Right as he finds your cock{s} his tentacular organs swirl around {them}it in a death grip of slippery flesh. Tighter and tighter they get until you're almost certain that the blood supply to [pc.eachCock] has been cut off, but the throbbing of your masculinity continues. With a mixture of pain and pleasure, the green tapered appendages pump your manhood with relentless tightness, making you wonder if you're actually in the clutches of a poorly programmed cock milker.

You whimper through the tongue in your mouth as the pressure in your dick{s} reach its{their} maximum, and the torturously pleasurable ring of tentacles reaches the tip{s} of your [pc.cocks]. You moan in sensual relief when the tongues' grip descends back towards the base of your crotch. The tentacle tongue in your ass causes [pc.cumColor] pre to leak out of your urethra{s} in large amounts as it slides itself across your prostate. Repeated bouts of this painful jerking of your shaft{s}}{if has pussy but no cock: and slide through the wet indentation that is your vaginal lips. They slip and slide against each other on [pc.eachCunt] as they make you shakily breathe your pleasures through your nostrils. Each time they rub your cleft{s} they tease you by brushing against your [pc.clits] but just enough to barely feel it. If only they’d properly give you the pleasure you crave.

You loudly moan through the tongue flesh in your mouth as your hopes are answered and [{if pc.vaginas > 1: one parts the lips of your [pc.vagina " + vIdx0 + "] and the other penetrates the fleshy veil of your [pc.vagina " + vIdx1 + "]. [{if vIdx0 or vIdx1 has virginity: A tear forms in your eye as your hymen is broken. [{if vIdx0 has virginity: Your first vagina's virgnity has technically been taken.}{if 1 has virginity: Your second vagina's virgnity has technically been taken.}] Pretty soon the pain is replaced with pleasure, although these tentacles might have been just be a bit thicker than you wanted for your first time}]. [{if pc.vaginas = 3: Unfortunately for you, he doesn't have enough oral organs to use on your third pussy, but it clenches like it has a cock in it nonetheless.}] To have [{if pc.vaginas = 1: your}{if pc.vaginas = 2: both of}{if pc.vaginas = 1: two of your}] vaginal passage{s} stretched like this is truly a wonderland of pleasure. His tongues spread your labia around their 2-inch diameter as they twist and turn inside your vaginas. Your moans reverberate through your stuffed maw while their deep thrusts batter against your cervices, but luckily for you they never manage to enter them.}{if pc.vaginas = 1: one, then the other tongue parts your labial lips and penetrates into your vaginal passage. [{if vIdx0 has virginity: You tear a little bit at the rather painful loss of your virginity, but as you bear through it pain turns into pleasure and soon you find yourself enjoying it. You've lost your vaginal virginity to tongues.}] The tongues twirl around themselves into a helix as the worm orally fucks your [pc.vagina] with as much roughness as he can muster. So deep does he thrust his tongues that they threaten to penetrate your cervix. Luckily though, they never do, but continue to use their tips as battering rams regardless.}].}{if has cock and pussy: and slide around your groin until one wraps around your [pc.cock " + cIdx0 + "] and the other prods at your [pc.vagina " + vIdx0 + "]. The tentacle tongue around your cock continues to spiral around your shaft and quickly makes a slippery mess in your groin. You buck into it in an attempt to get more, but no matter your efforts it stays at its current tempo of twirl licking your dick.

[{if pc.cocks > 1: The rest of your shafts seem doomed to receive no stimulation at all assuming things go on like this.}] While your cock was being stimulated in the worm's frustratingly slow manner, his other tongue continues to prod at your lower lips. Why must he tease you so? You did lose to him but still, not at least penetrating you is torture in a situation such as this. Just as you thought you could take matters into your own hands and lower your [p.hips], he penetrates you in one swift movement. [{if vIdx0 has virginity: A high pitched whimper comes from you, muffled by the tentacle still stuck in your mouth as your hymen is torn. But it was so worth it! </b>Your vaginal virginity is lost along with your hymen.</b>}]

Finally! [{if pc.cunts > 1: Too bad your other cunts can't get any action.}] Deeper and deeper his tongue goes, scraping along your vaginal walls until it hits your cervix and can go no further. It tries to push forward, but cannot, and resolves to continue its vaginal exploration as it thrusts into you repeatedly. Occasionally he shoves in more of his tongue's length than your cootch can handle and coils quite a lot of it inside you before it goes back out again. His dual gendered assault on your erogenous senses}{if has no genitals: alongside the one that's been continually railing your ass, trying their luck at getting into your tongue fucked pucker. Their tips [{if pc.analLooseness ≥ 3: easily slide in next to the first}{{if pc.analLooseness < 3: knead at your already stretched hole before they too slip into the pre-lubed ring and </b>stretch out your anal passage</b>{set pc.analLooseness to 3}}].

Before they get far, the newest two in your butt twirl around the first like vines around a tree and royally wreck your ass with their new triple helix configuration. As the last two go deeper into you, you feel their presence like a whisk in your rectum. Unlike the first, the twirling duo does not go all the way into your intestines. Rather, it would seem that the worm gets more enjoyment out of having you moan your pleasures as he continually thrusts his last two tentacular tongues in and out of your abused anal passage. Your nerves aflutter with penetrative stimulation, his continued misuse of your [pc.asshole]}] leaves you a quivering mess of pleasure in no time at all.

What feels like an eternity passes, and all memory you have are flashes of your ass being deeply penetrated by tongue tentacles, and the rest of your body being painfully pleasured with no end. Your orgasm creeps up as muffled moans escape you and dopamine hits your mind with the climax of all the forced stimulation. [{if has cock: You buck your [pc.hips] forward as your [pc.Cocks] erupt{s} with [{if pc.cumVolume < 1000: several thin spurts}{if pc.cumVolume > 1000 && if pc.cumVolume < 10000: thick volumous sprays}{if pc.cumVolume > 10000: geyser-like arcs}] of [pc.cum] from your [pc.balls], [{if has pussy: while your [pc.vaginas] continuously clenches, [{if pc.vaginalWetness = 1: tricklling{if pc.vaginalWetness = 2: dripping}{if pc.vaginalWetness = 3: secreting a stream of}{if pc.vaginalWetness = 4: squirting}{if pc.vaginalWetness = 5: explosively squirting}] [pc.girlCum] around the tongue{s} inside you}].}] The sexual fluids coalesce into the greenish wet spot below you, making it all the muddier.}{if has pussy but no cock: Your [pc.vaginas] spastically clench{es} around the tongues within you, [{if pc.vaginalWetness = 1: trickling}{if pc.vaginalWetness = 2: dripping}{if pc.vaginalWetness = 3: secreting a stream of}{if pc.vaginalWetness = 4: squirting}{if pc.vaginalWetness = 5: explosively squirting}] their [pc.girlCum] before your juices mix with the greenish puddle below you already made by the creature's saliva.}{if has no genitals: Like a web set aflame from the center, your nerves light up, spreading orgasmic sensations with your thrice tentacle stuffed [pc.asshole] as the source of your pleasures. Pure bliss surrounds your mind once the sensory wave travels up your spine and hits your brain, leaving only sourceless happiness wherever it goes.}]
...
Eyes heavy from exhaustion, you doze off in the muddy sand and awake several hours later with somewhat sore muscles and [{if pc has genitals: musky, }] greenish, dried out, cracked sand covering the majority of you. It is thankfully easy enough to clean yourself up, but it takes a while to find your equipment as it had been almost entirely covered in sand while you were out. Re-equipping your [pc.gear] takes a couple of minutes, but then you're ready to go again. Hopefully there are no more worms for a while.
...
Item removal:
[Immunity booster's using text]

[{if hasFemaleInside = true:
You feel a pinch inside your ass, and get that feeling of really having to use the bathroom. Not sure whether it's the real deal you rush to the [{if in town: closest lavatory}{if in wild: nearest spot you'd consider private enough}], but before you can get there, you feel several tentacular objects squirming [{if pc.isNude = false: inside of your [pc.lowerGarment], and}] around the outside of your ass. You groan in discomfort as your [pc.asshole] is stretched around the parasite that is now desperate to get out of your ass. The boosters must have somehow made your butt into a really unsavory environment the parasite living there. It pops out [{if pc.isNude = false: , squirms out of your [pc.lowerGarment]}] and falls towards the ground while desperately trying to get away from you. Well... that worked. It would've been better if it didn't leave you with a sore ass though. [{if in any butt bug pregnancies: Slight nausea hits you as even the [{if BBFInside = 2: egg in your gut is}{else: eggs in your gut are}] dissolved by the immuno booster, leaving you with your usual non pregnant belly. {cancel butt bug pregnancy}}] [{if pc.analLooseness < 3: Your anal ring has been stretched a little. {set pc. analLooseness to [pc.analLooseness + 1]}}]
}]
...
A gurgling sound emanates from your [pc.belly] as the female butt bug inside you realizes that not only are the eggs she is caring for never going to hatch, but that she was never fucked by a male butt bug in the first place. The weight of the eggs shifts slightly downward as she gets both of you ready to lay the unfertilized eggs. Somehow, she also puts the thought in your head that squatting right now would be a very good idea. You do just that while dropping your [pc.gear]. Green lubricant oozes from your asshole as the female slightly expands her body inside you [{if pc has dick: , rubbing against your prostate and making your male member{s} twitch and harden instantly.}] All the while, she flexes her inner muscles in a slow, drawn-out and slightly pleasurable fashion.

[{if FTBBLay = true: Pressure fills your anal cavity further as she keeps expanding, stretching your inner walls to their limit as she makes you groan with this strange new sensation. Clutching your abdomen you experience pain and pleasure mixing into a weird cacophony of feelings. Her expansion reaches your sphincter, and even through your clenching, she forcefully pries open your anal ring with her expanding orifice. Employing some of her tendrils she manually spreads open your asscheeks, completly exposing you. Several verdant slime strands connect both your cheeks and the walls of the expanded female’s internal body, as she gapes you to the point where your insides are visible.}
{if FTBBLay = false: The all-too-well-known pressure in your lower abdomen rears its head again, and the pain you felt the first time is now nonexistent. Adapted to the capabilities of the parasite, you relax your anal walls and accompany the stretching as you enjoy the sensations rolling through your rectum. Not even needing to be forced this time, you insert a few digits of your own accord and help her manually spread you, parting your buttcheeks in the process. Your digits, and even most of your hands get covered in the green slime leaking from her, making the gaping process so much more fun. Without noticing it you’ve gaped yourself to the point of others being able to see into you and the parasite, although not yet not enough to see the other end of your passenger.}]

[{if in town: People around you stare in disbelief at what is happening in front of them, a [pc.gender] gaping their anus in front of everyone to see. A puddle of green liquid forms below you as the slime leaking from your ass accumulates. Looking down towards it you see people's faces looking at you in the reflection, and are reminded that you’re actually in public.

[{if pc.exhibitionism < 50: Your face flushes from the attention being given to you. You try to hide your face from the crowd, but you realize it is impossible. They’ve surrounded you, and are watching you with mixed expressions. Some are shocked, some judge you, and some are sporting tents, wet panties, or both. Why are they just staring at you? Can’t they see you need help or at least some privacy?}
{if pc.exhibitionism > 50: Their stares spur you on, making you masturbate as you specifically focus your attentions on some of them. Most are disgusted and leave, but some stay and even start openly pleasuring themselves to the show you’re putting on for them. Those that are adventurous enough grab your ass when it’s close to them, and the more curious ones even poke a hand into your gaping anus, making you gasp as it slicks in and slides out without effort. They step back once they notice you might actually be birthing something, but don’t stop their self pleasuring.}]}]

[{if BBFInside = 0: You feel the eggs moving already, pushed on by the powerful undulating muscles of the female inside you. You expected the eggs to bulge out your anus a little, but all you feel is the blissfully brief slipping out of each egg like a string of over-lubed anal beads, slightly caressing your passage [{if pc has dick: and your prostate}{if pc has vagina:, as well as making your [pc.vagina] slightly wet,}] with their movement as your [pc.belly] slowly deflates. Each egg adds to a pile of green eggs until you count there to be about 10 of them laying on top of each other.}
{if BBFInside = 1: As soon as you’re in position, your gaping anus floods out the cascade of slime-covered eggs that were all somehow crammed up your gut. They quickly cover the floor in their fluids, making wet plopping sounds as each one slips it out of you and splatters onto the already-wet surface below you. There seems to be no end to them as they slide out of your ass [{if pc has dick: while grinding against your prostate}{if pc has vagina:, and make you almost cream yourself}]. Before you can reach your peak though, the stream of barely--wide-enough eggs stop, with the last of them plopping out and joining its brethren with a soft ‘splat.’ It would be a rough guess but you could say there were about 30 of them in total, though it’s hard to tell with them all clumped together like that.}
{if BBFInside = 2: This egg feels larger than you expected it to be. It might actually require some effort to pass. You can feel it pressing against the ring of the female inside you as it is sucked through by the undulation of the female alone. The egg’s mass moves down, and with it so does the bulge in your abdomen. The female’s sucking of the egg has her orifice stretched wide over its surface, and is making your intestinal tract struggle to accommodate such a large object passing through.

Various anal birthing fluids follow its path soon after, as you start to see the surface of the egg in the reflection of the liquids below you. Its progress is slow, but not painfully so. There’s just enough movement to stimulate your insides [{if pc has dick:, making your dick{s} ooze a little precum}{if pc has vagina: and creating a bit of a river between your legs}] with its movement. She undulates more now than ever before. Her movements roll through your rectal passage and across the surface of the egg. Each inch she pushes the egg through you goes into new territories of wonderful fullness.

Every move of the spherical mass expands what you thought your ass was capable of handling. Your own tract is being pushed to its limits as it tries to fit the large egg without tearing. Reaching the end of her and your passage it comes to a halt, your own anal ring pressing down on the large female and making it harder for the egg to pass through her. Knowing now that you have to participate a bit, you try to push a little while relaxing your already gaped pucker. Sure enough the egg moves again before finally passing through at its widest point and plopping out with a satisfying ‘pop’. The egg glistens in the light as you breathe a sigh of relief of it finally being over.}] [{if in town: With the show now over [{if pc.exhibitionism < 50: people start to lose interest as one by one they walk away. Their looks did turn you on a little however. Maybe there is a little bit of an exhibishionist inside you.{Increase exhibitionism by 5}}{if pc.exhibitionism > 50: the people around you finish off their masturbation, some of them aim directly at you as they cover you with their juices.{inflict "cum covered" status effect}}]}]

With the female apparently done, she retracts her tendrils and [{if FTBBLay = true: you experience for the first time your anus rapidly closing back up as the female contracts herself back to her normal size. Crap this is weird, having your rectum just be manually closed like some kind of strange aperture.}{if FTBBLay = false: your anus closes back up once more while she retracts to her normal size, making you wish it wasn’t over as quick as it was}]. It almost looks like your pucker was never gaped open to begin with. You do notice you have been stretched a little bit though. This unfertilized laying was stimulating but not enough to sate you, so you stand up and dispose of the eggs as you resolve to find some other way to get off.
...
A rumble in your egg-gestating gut is all the warning you get before green slime oozes from your parasite-occupied [pc.asshole]. You [{if pc.exhibitionism < 50: find a secluded spot away from possible prying eyes to lay in peace}{if pc.exhibitionism > 50: drop into a crouch right where you are standing, regardless of who might be watching}] and remove your [pc.gear]. With your back against the closest vertical surface and ass as low as it can go, the laying starts. The eggs have already started to move down your digestive tract. As the smooth orbs make their way to your rectum, the tendrils of the female slip out of her poking-out orifice, one by one until all four of them are outside. At least a foot of their length is visible from your perspective, with two of them determined to spread your asscheeks while the others slither across your form.

One [{if has cock but no pussy: wraps itself around [pc.oneCock] and slowly pumps it while the other prods its end against your cock’s urethra. The sudden stimulation causes some [pc.cumColor] pre to dribble out and cover the prodding tendril’s tip.}{if has pussy but no cock: rubs itself teasingly along the folds of your cleft{s} while the other wraps around {one of }your clit{s}. It mostly just squeezes your sensitive button, but you almost cream yourself whenever it decides to tug mercilessly at your [pc.clit].}{if has cock and pussy: decides to jerk you off as it spirals around [pc.oneCock], but the other one looks unsure. It eventually decides to go for {one of } your cunt{s} and darts towards your womanly folds like a scorpion’s sting. Almost like the digit of some skilled bedfellow, it lightly penetrates your vaginal lips while frequently bending itself to get at the most hard to reach places.}{if has no genitals: doubles over as it bends back towards the orifice from which it first came to tentafuck your butt—technically the parasite’s anus. The sensations you’re getting through the parasite in your [pc.asshole] make it so you might as well have your ass be an erogenous zone. As if one wasn’t enough, the second twirls around the first to fuck you with the newly formed double helix tentacle.}]
...
[{if pc has no genitals: The tentacles fucking you retreat as}] {T}the slime-covered, anus-like orifice widens considerably as its dermis is stretched around the ovoid object. In a resounding ‘plop’ of finality, it pops out of the parasite and onto the floor, covered in layers of slick fluids. But it’s not over yet. Just as the first left your body, the second is already being enveloped by the bug’s internal aperture. [{if pc has no genitals: The free tendrils have resolved to massage your bare groin so as to avoid getting in the way of the rest of the eggs.}]

Time flies by as several more eggs follow the same insectoid oviposition of the first, until only one is left. The female’s efforts egg you on; release is so close you can taste it. The last egg plops out to top off the slimy pyramid below you as you’re pushed blessedly over the edge. [{if has cock but no pussy: You grunt as [pc.cum] blasts out of of the urethra{s} of your [pc.cocks] while the tendrils continue to stroke [pc.oneCock], with the one formerly at your tip now stimulating the glans of your dick.}{if has pussy but no cock: You squeal as [pc. girlCum] sprays from your quim{s} in arcing streams liquid pleasure. The muscles of your cunt{s} contract{s} around the tentacle inside you. Even more of your girl cum is sprayed when a final tug on your clit reawakens your spasming vaginal lips.}{if has cock and pussy: Loud moans escape your [pc.lips] as [pc.eachCock] throbs and your [pc.pussies] spasm in tandem. Your cock's{s'} veins bulge while [pc.cum] is let loose from its{their} urethra{s} and splatters onto the ground a few yards in front of you. Whilst your seed is being sprayed, your vagina{s} convulse{s}, leaking [pc.girlCum] onto the pile of eggs below.}{if has no genitals: Heat suffuses your nervous system while numbing sensory waves emanate outwards from your bare groin. Your mouth is left agape as your spine quakes with synesthetic tremors that leave only indescribable bliss afterwards.}]

As you ride out the last of your orgasmic waves, the tentacles slurp noisily back into the external orifice of your anal-lodged parasite. Breathless from exertion and with a pile of what seems to be 15 fertile eggs below you, you slowly regather your [pc.gear]. </b>Your anus does feel a bit more loose than before though.</b> After all, you just went through what was basically a bunch of large anal beads. The hilinara eggs will soon hatch, and if not taken care of they will likely pester the locals. [{if on tavros: As you are already on Tavros station, the decision seems obvious.}{if not on tavros: What do you do with them?}]
...
---------------------------------------------------------------------------------------------------------- //divider line for easy division of pregnancy scenes amongst female types
...
Your hips buck as her newest egg plops out into you to join the rest. A miniature orgasm rolls through you while your butt bug winds down her activities and your hand [{if has cock but no pussy: is covered in tiny spurts of [pc.cum].}{if has pussy but no cock: acquires a thin lamination of [p.girlCum].}{if has cock and pussy: gets covered in both [pc.cum] and [pc.girlCum].}{if has no genitals: gets covered in a thin layer of the female’s slime.}] Satisfied slightly but not completely, you redress in your [pc.gear] and continue your day with an extra egg bulging out your gut.
...
A wave of movement rolls through your digestive tract as the overproductive female parasite inside you decides that it's time to lay her egg[{if OPBBE > 1:s}]. [{if pc.isNude = true: Your anal parasite subconsciously convinces you to remove your [pc.gear] so that the laying can be as obstruction free as possible. Not that you really mind; it'd be a pain to get ooze on your gear anyway.}] The egg[{if OPBBE > 1:s}] must have been fertilized by that male butt bug from earlier. She wouldn't be laying them like this if that wasn't the case.

The path of the ovoid object{s} making their way to your parasite-occupied rectum is visible through your gut. Her four tendrils peek out of 'your' ass, strands of green semi-fluids connecting their slender bodies until the distance between the appendages causes the strands to snap. Your [pc.ass] is spread wide open by the parasite's surprisingly strong tentacular appendages while verdant slime trickles out of your anus. Groans escape your mouth as the butt bug within you expands herself, stretching your own anal walls in the process. One by one her sphincter muscles expose your inner passage in wet shlicks until finally there is a clear cut tunnel into your egg-occupied depths.

[{if OPBBE = 1: A single splurt of green slime exits the now gaping hole of your rectum. Along with the goop there plops out a single egg that lands safely into the glob of ooze. Something triggers in your mind with the exit of the egg, and you're left more clear headed than before, although shivers still run down your spine. Not completely the release you were looking for, but it does make you feel a bit better.
{decrease lust by 10}
}
{if OPBBE > 1 && OPBBE < 25: Several sputters of ooze exit your ass, each followed in quick succession by an egg. With each ovoid sphere that plops out you receive an absolutely divine tingle down your spine and throughout your limbs. The sensory reverberations are even affecting your groin. [{if has cock but no pussy: [{if has balls: Your balls churn}{if doesn’t have balls: Your prostate tingles}] with the sensation of the eggs being pushed through your shared anal passage. Evidence of your pleasures bubble out from your urethra{s} as [pc.cum] pre drips down your shafts{s}.}{if has pussy but no cock: Your vaginal lips quiver with indirect stimulations, leaking their [pc.girlCum] with every egg that exits your parasite-plugged anus.}{if has cock and pussy: [{if has balls: Your balls tingle with activity}{if doesn’t have balls: Your prostate is stimulated}] as the eggs slide through your anal canal. These indirect pleasures roll through your crotch as your own fluids are stimulated out of you. The proof of your pleasures leak from your cock's{s'} urethra{s} in tiny streams of [pc.cum] pre while [pc.girlCum] forms from the crevice{s} [{if has balls: behind your [pc.balls]}{if doesn’t have balls: below your shaft{s}}].}{if has no genitals: Your bare groin has become a hot spot of erogenous sensitivity, even the occasional breeze that hits it has your nerves aflutter with erotic sensations.}]. It's so good that you might, just, cum, but the last one pops out just before you make it. They all land safely into the goop on the floor that acts like some kind of shock absorber, and you're left ultimately unsatisfied. If only there were more eggs for you to lay.
{increase lust by 20}
}
{if OPBBE > 25: The force of the intermittent streams of egg-filled slime spurting out of the parasite's gaping orifice has you and your [pc.belly] toppled forward in short matter. The eggs and slime land in a heap behind you quickly piling up as time goes on. The slimy objects that escape your bowels slide against your digestive tract's walls, becoming all the more pleasurable when those that were deeper inside you slip out and [{if pc has cock: rub against your prostate, only to}] stimulate the female parasite's inner passage.
...
The indirect sensory stimulation is too much to resist. Around the sides of your bloated belly your hands wander towards [{if has cock but no pussy: the shaft{s} of throbbing meat nestled in your crotch. One grips the base of [{if pc.cocks > 1: one of }] your dick{s} and jerks it from top to bottom. The other [{if pc.cocks > 1: grabs hold of another dick and strokes it sensually before jerking it too in sync with the other like pistons.}{if pc.cocks = 1: [{if has balls: caresses your scrotum before directly massaging at the firm roundness that is your [pc.balls].}{if doesn’t have balls: rubs with its digits at your taint before moving to stimulate the tip of your cock in circular motions.}]}]

The ministrations of your hands along with the intermixed pleasures from the female parasite prove to be too much too fast. While the last of the eggs plop out into the pile, your [pc.cumColor] pre and increased breathing mark the arrival of your inevitable climax. Now on your [pc.knees] a groan escapes you as your hips buck and your [pc.cum] is blasted from the tip{s} of your dick{s}. It stains what it hits with [pc.cumColor] blotches and fills the air with its musky odour until you're utterly spent.
}
{if has pussy but no cock: your dripping cunt{s}. Their greedy lips ache for attention and you give into it graciously. Your digits spread your vaginal lips and expose the sensitive flesh within before digging into your ravenous depths. A thin sheen of [pc.girlCum] forms on your hands and -thighs as you frig your womanly folds. One hand is already knuckle deep into your cootch while the other [{if pc.cunts > 1: clones the actions of its sister on your second vagina, each of your thumbs having their way with your clit{s}.}{if pc.cunts = 1: has its way with {each of }your clits{s}.

Every tweak of your pleasure buzzer{s} is like a release valve for your girly fluids.}] The dual signals from the female parasite lodged in your anus as well as your own efforts with your [pc.cunts] push you over the edge in no time. Your breath goes ragged as you, now on your [pc.knees], buck your [pc.hips] along with the plops of the last eggs that join their siblings in the mound of eggs and goop. Girlish squeals escape your [pc.lips] as each buck is answered with the clamping down of your vaginal muscles onto your digits. Every muscle spasm renews the river flowing from your spastic cunt{s} until you're blissed out of your mind and rolling on the floor.
}
{if has cock and pussy: intersexed set of genitals. They slide alongside your groin before one grabs {one of }your cocks and the other slides its middle digit into {one of }your wet crevices [{if has balls: underneath your ballsack}]. You time your motions with the plopping eggs; each of the ovoid objects that exits your anus is a jerk of your dick and an inserted digit into your cunt. The dual sources of pleasure from your mixed genitals are already overwhelming, but the parasite's sensations are yet another source, and the barrage of sensations is already having its effect on you.

Sensual moans fill the air while the early signs of your orgasm show. [pc.Cum] pre oozes from your urethra{s} while streams of [pc.girlCum] leak from your cunt{s} and cover your hand in their juices. [pc.Belly] no longer supporting you, you now rest on your [pc.knees]. Moans turn into senseless babbling and your [pc.hips] buck in the throes of sexual nirvana as your genitals release their liquid pleasures. Two distinct puddles are formed, one a few yards in front of you consisting of [pc.cum] and one below you that still connects you and the floor with strands of [pc.girlCum].
}
{if has no genitals: your bare groin. Though you have no genitals, the hilinara parasite's sensations have become one with yours. It may just be the feminine physiology of the insect, but her pleasures have concentrated themselves in your flat crotch and made it quite sensitive. The sensitivity makes even the slightest caress of your digits trace lines of pleasure through your [pc.skinFurScales]. The sensual bolts arc up your spine along with the bug's own sensations, building in intensity with each egg laid until they explode as the last one makes it out.
...
</b>Your anus does feel a bit stretchier than before.</b> Perks of an egg-laying ass parasite, you guess. Your mind wanders towards the egg{s} as you regather your gear. The hilinara eggs will soon hatch, and if not taken care of they will likely pester the locals. [{if on tavros: Since you are already on Tavros station, the decision seems obvious.}{if not on tavros: What do you do with them?}]
...
Abdominal contractions strike you out of the blue. They're painful enough that you're quickly forced to the ground in order to alleviate your symptoms. It does not help. Even belly rubs aren't soothing the aches in your egg-occupied gut. A gurgle and slight movement in the egg's position follows the latest of your cramps, and you realize that the egg’s due date is today! As fast as your pains allow you, [{if pc.isNude = true: you shift your nude form into a more comfortable position and}] you remove your gear just to make it easier on yourself.

You feel three of the butt bug's tendrils deploy inside of you while the fourth snakes its way out of her external orifice. The fourth's slimy surface tickles you as it coils around your [pc.legs] and pulls itself into a tentacular knot. Is she planning to pull the egg out of you?! Groans escape you as the internal tentacles scrape their pointed tips against your digestive tract's walls and travel upwards to meet with your ovoid cargo.

Their presence in your passage can be clearly seen as three lengthy tubes that bulge out your [pc.skinFurScales]. You slide your hands across them and feel through your [pc.skinFurScales] how they push against the insides of your egg-carrying gut. Each of their advances inside you are answered in a drawn out 'oooh', making you feel fuller than you already are. Once they meet with the egg lodged in your lower intestine, the tentacles diverge from their group exploration of your insides.
...
You feel like you're in one of those holovids where they [random: {insert oversized objects into the asses of experienced pornstars}{hold anal dildo size queen competitions}], only with you it's in reverse. Your little fantasy has sparked some arousal from you and your [{if has cock but no pussy: [pc.eachCock] is already starting to get erect}{if has pussy but no cock: [pc.cunts] are already moistening}{if has cock and pussy: dual gendered genitals are already being pumped with blood}{if has no genitals: bare groin has already gotten tingly}] because of it.

Subconsciously you sensually caress yourself. From the dome of your belly your hands travel up towards your [pc.chest] and back down again until they hover just above your groin. You just can't resist your need to indulge in your carnal desires and so you let your hands dive straight to business. With every pull on the egg that the insect makes, [{if has cock but no pussy: you tug on your [{if feminimity > 50: girlish}] penile shaft{s} and relish in the sensations of her movements stimulating your prostate.}{if has pussy but no cock: you dig your digits into your womanly folds. You alternate thumbs as you polish your pearl{s}, all of your other fingers wet to the second knuckle in girl juice.}{if has cock and pussy: you jerk [pc.oneCock] and frig [pc.oneVagina]. When at the top of your cock you rub your tip in circular motions with your thumb while you do the same with [pc.oneClit]. You might just have a birth fetish, your digits are already wet with [pc.girlCum].}{if has no genitals: you rub your palms against your sensitive featureless groin. Like an ember sparking a flame in dry tinder, you've stoked your nerves into a heat filled passion.}]
...
Determined and sweat covered, you brace for the egg to stretch the second part of your anus. Your large anal parasite's presence has already prepared you for this moment, but this egg is larger than even her! To help quicken the process, you push with your anal muscles whenever you feel her start her own pulls. Slowly but surely, the smooth point of the oval object is enveloped by your butt bug's inner orifice until it unexpectedly slips past its halfway point [{if pc has cock: and forces some pre from your [pc.oneCock]}].
...
You increase your masturbatory pace as you [{if has cock but no pussy: vigorously pump your pre-slicked [pc.oneCock].}{if has pussy but no cock: fervently jill the lips of your [pc.oneVagina].}{if has cock and pussy: lustfully stroke your cock to the beat of the bug's undulations and frig your [pc.girlCum] leaking cunt[{if pc has balls: behind your [pc.balls]}].}{if has no genitals: deeply massage your digits into the sensitive flesh of your bare groin, setting your nerves set on fire with the indescript lust coursing through you}]. Starting as a low groan, your voice raises in volume and intensity until you loudly grunt in tactile overflow while 'your' pucker parts around the large slimy tip.

You're pushing with all you can manage as well. Sputters of green slime spurt out around its sides, blessedly lubricating the passage. The natural lube speeds things up exponentially as the oval egg passes its halfway point and slips out of you completely. It's so large that it doesn't even fall much once it totally separates from your body. You exhale in pleasurable relief as you're driven over the edge into your climax. Gasps followed by moans escape you while [{if has cock but no pussy: your [pc.eachCock] throb{s} and twitches in your hands in tandem to the [pc.cum] being pumped from your [pc.balls]. Your seed blasts through your urethra{s} and flies through the air before it splatters onto the ground.}{if has pussy but no cock: vaginal walls spastically clench down onto your invading digits. Streams of [pc.girlCum] flow from [pc.eachVagina] liberally covering your hands and the floor in [pc.girlCumColor].}{if has cock and pussy: your [pc.hips] buck in the throes of your pleasures. The bulging veins of [pc.eachCock] and the spastic clenches of [pc.eachCunt] harkens the eruptive diplay of [pc.cum] and [pc.girlCum] emanating from your groin. Pretty soon, your hands and inner thighs become messy with your fluids and all you're thinking about is in how much bliss you're now in.}{if has no genitals: your nerves briefly explode in a lustful supernova of pleasure. The sensual heat stays for a while before a state of bliss radiates throughout your form that leaves you as calm as a still lake.}] The rest of the slime splashes out of your rectum and falls onto the already ooze covered egg like liquid afterbirth and forms quite a large puddle beneath you.

Dizziness creeps upon your consciousness and you almost fall into a blackout as you collapse onto to fluid covered floor. The parasite's tendril around your [pc.legs] unties itself, slipping back into your bug's now gaping birthing canal and disappearing from your sight. Damn it, your ass is fucking sore! Was it worth it? Maybe. </b>You're pretty sure your asshole won't fully recover from this for quite a while.</b> A bit sensitive but glad you're free of the extra weight while you gather you gear, your mind wanders towards the egg you've laid. The hybrid hilinara egg will soon hatch and if not taken care of it will likely pester the locals. Either that, or try and integrate with them. [{if on tavros: Since you are already on Tavros station, the choice seems obvious.}{if not on tavros: What do you do with it?}]
...
Post laying Choices:
[Leave them]
You don't want anything further to do with [{if eggsLaidBy =/= "large": these eggs}{{if eggsLaidBy = "large": this egg}], and {they}it can apparently take care of {themselves}itself. Your parasite doesn't seem to be objecting to your choices either so you guess they're going to be fine. The butt plug-like parasite in your ass is slowly closing her fleshy aperture so as to once more blend in with your rectal passage. [{if FTBBLay = true: The sensation of her resettling in like this is making you a little uncomfortable with her movements.}{if FTBBLay = false: You've become used to her resettling into your anal tract after her egg laying, but it's always a little weird.}] She does eventually settle down however, allowing you to continue with her inside you in relative comfort.

[Nursery] 
You've put a lot of effort into carrying [{if eggsLaidBy =/= "large": these}{{if eggsLaidBy = "large": this}] to term and you're not about to just leave [{if eggsLaidBy =/= "large": them}{{if eggsLaidBy = "large": it}] here. A quick call to the nursery with your codex and barely five minutes later a delivery drone appears to haul what you've lain. The butt plug-like parasite in your ass is slowly closing her fleshy aperture so as to once more blend in with your rectal passage, and [{if FTBBLay = true: the sensation of her resettling like this is making you a little uncomfortable with her movements.}{if FTBBLay = false: while you've become used to her resettling into your anal tract after her egg laying, it's always a little weird.}] She does eventually settle down however and allows you to continue with her inside you in relative comfort.

[{if eggsLaidBy = "normal": The drone scoops the eggs up rather quickly, and fits them snugly into three holding trays. Like three dice with the number '5' facing upwards, the egg trays fit into a box-like container and are then locked inside the bot. With 'your' offspring secured, the bot's thrusters fire up once more as it lifts into the air to deliver its cargo.}
{if eggsLaidBy = "productive": The aerial drone deploys what seems to be a [{if OPBBE = 1: little box}{if OPBBE > 1: hardlight net}] in order to transport it's targeted cargo. Once [{if OPBBE = 1: it is put into the box by a mechanical arm and sealed away inside the drone}{if OPBBE > 1: all are surrounded by the net the gaps in between the thin light streams close up to form a sealed bag in which your eggs are to be carried}]. It's security checks pass and the thrusters on it's sides start up as it goes into the air to deliver the egg{s}.}
...
As the drone flies off to the nursery, a strange sense of maternal longing creeps up on you. Like what you would feel sending your child to school for the first time. Surely these aren't your feelings, [{if eggsLaidBy =/= "large": they technically aren't even your own offspring}{if eggsLaidBy = "large": but then again there is a small part of your genetics in there}]. They must be coming from your parasite.
