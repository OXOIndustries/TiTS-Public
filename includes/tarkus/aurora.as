...
public function approachAurora():void
{
	...
	if(flags["MET_AURORA"] == undefined)
	{
		...
		output("\n\nAs you gauge her, your thoughts start drifting away from concern about her absent chaperone is to simply how her hair stays “down\" despite her dangling nature until she speaks;");
...
public function choppyBecauseWarhammer40kDidntDoThisAlready():void
{
	...
	output("The first entry on the list that you actually have to ask about for context and, it turns out, you really hadn’t. Aurora awkwardly hefts up a extremely unbalanced looking waraxe, the unnecessarily oversized head of which made by awkwardly fusing a host of other weapons to it, all merging on a thick, sharpened point. It’s easy to see that many of these “borrowed\" weapons are augmented in some way and not merely sharpened bits of metal by the utter mess of wires coiled around the hilt. Various miniature power sources carved into the stalk provde the power. There must be three different thermal and two vibro blades alone!");
	...
	output("\n\nBrandishing the weapon and turning it on by a button at the hilt, Aurora barely keeps the now growling, shaking weapon steady in her claws. It vibrates aggressively, little shocks of electricity racing across the miniscule parts in the blades as hissing steam intermixes with it, a mixture of vibro, shocking and heat weaponry coming together among potential others. It looks rather unstable, though despite that the shopkeep doesn’t seem to worried - even as her arm quakes, causing the leather of her lengthy wings to buzz like a softly tapped drum. Besides, the fact that it works at all is rather inspiring to begin with.");
	output("\n\n<i>\"So! Interested?\"</i>");
