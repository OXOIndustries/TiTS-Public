...
public function playSemithChessScene():void 
{
	...
	if(hours >= 12 && hours <= 17 && !playedChessWithSemithToday()){
		showSemith();
...
public function approachSemithPlayingChess():void 
{
	...
	if(!flags["MET_SEMITH"]) {
		...
		output(" feels hot and strangely smooth in your grip; aside from his head the alien's skin is hairless and has a buff glaze to it, like latex. <i>“The name's Semith, pleased to meet you. And you might be...?”</i>");
		output("\n\nYou tell him.");
...
public function semithTalksAboutKnowingThePc():void 
{
	...
	output("<i>“So you know my name?”</i> you ask quizzically, as you move a knight to c3(y)2.");
	...
	output("\n\n<i>“How is Steele Tech doing?”</i> you ask, as you");
	
	if(random == 0) output(" take");
	else output(" don't take");
...
public function semithTalksAboutVulkrimi():void 
{
	...
	output("<i>“Tell me about the vulkrim,”</i> you say, as you ponder that part of the board which can be seen in three dimensions. <i>“Are you guys pretty active in the rush?”</i>");
...
public function wrapUpSemithChessMatch(): void 
{
	...
	{
		output("\n\nSemith taps a knight forward and then looks at you with the smug smirk of someone waiting for you to get the joke. It takes you a few moments to see it. He's had you in checkmate for... how long?... It just wasn't observable in this part of the universe. You scowl.");
	...
	{
		output("\n\nSemith taps a knight forward and then looks at you with the smug smirk.");
		output("\n\n<i>“Checkmate.”</i>");
...
public function playSemithsApartmentScene():void {	
	...
	else if (flags["SEMITH_INVITED_PC"] && (hours < 17))
	{
		...
		if(flags["SEMITH_HAS_PC_IN_APARTMENT"] == true)
		{
			...
			{
				output(" A chess player who says exactly what's on their mind after a few puffs! You are truly wonderful.”</i>");
			}
			else output(" It's always good talking to you.”</i>");
			flags["SEMITH_HAS_PC_IN_APARTMENT"] = false;
		}
		//player tried to visit before 17 o'clock
		else {
			clearOutput();
			output("Semith doesn't appear to be home, maybe you should come back later.");
		}
	}
	
	//in either case throw them out
	if(!flags["SEMITH_INVITED_PC"] || (flags["SEMITH_INVITED_PC"] && (hours < 17))) {
...
public function semithMenu():void {
	clearOutput();
	showSemith();
	
	output("<i>“Oh! [pc.name]. How nice,”</i> the vulkrim says as he turns towards you. Classical jazz billows from a positively old-fashioned looking device sitting on a nearby shelf. He looks more dishevelled than he usually does in the park, with the top buttons on his shirt undone and his swept back hair is slightly ruffled.");
...
public function semithEnjoysSomeRoshanBlue():void {
	...
	output("\n\nWhat an intriguing way of doing... that. You sigh as you feel the warm, perfumed air permeate through you, making you feel like your mind is slowly, carefully being unravelled. The bubbles hanging in the air and the piano noodling stop seeming slightly weird, more positively delightful.");
...
public function semithSexMenu():void {
	...
	output("Semith's eyebrows arch slightly, and he considers you up and down with measured interest.");
	output("\n\n<i>“I think,”</i> he murmurs, <i>“I could get on board with that.”</i>");
	output("\n\nHe curls his hot, latex-skinned hand into yours and leads you into his living room. The big, squashy, leather-encased sofas are perfect to curl your [pc.legOrLegs] onto and explore the compact demon's body with your hands, removing his shirt and trousers to reveal more and more of his lightly toned, smooth, red-skinned body");
...
public function semithSexButtfuck():void {
	...
	output("\n\nSemith can only groan woozily, flopped over the arm of his settee as you clasp his naked, latex-like flesh and pack his guts full of [pc.cum], flexing into him again and again, surging out every last drop of your liquid heat.");
...
public function semithSexCatchAnal(): void {
	...
	output("You enjoy the sensation of the hot, compact weight of the latex-skinned vulkrim trapped beneath you for a few moments - rubbing your body up and down his smooth, sexy skin");
	...
	else output("\n\nIt's only tough for a moment; like the rest of him, Semith's dick is almost plastic-like in its flawless smoothness, exactly meant to open you up like this.");
	...
	output(", dick still wedged firmly in your rear. <i>“As much as getting suffocated by you would be a pleasant way of passing, I'd quite like to do this... </i>” he trails off in a groan as you tighten up around his length, biting your [pc.lip] as delicious sensation pulses through you as you do it.");
