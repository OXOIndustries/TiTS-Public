...
public function brothelTurnTrixWhoring(service:String = "none"):Number
{
	...
	else if(service == "all")
	{
		...
		while(scenesLimit > 0 && scenesTotal > 0)
		{
			// Cunt-tail:
			if(pc.hasTailCunt() && tailType == GLOBAL.TYPE_CUNTSNAKE && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			if(pc.vaginalCapacity(x) >= pp.cockVolume(0) && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			pp.createCock();
			// Any: Ausar Male
			if(scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			// D cups or larger:
			if(pc.canTitFuck() && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			// Cunt/mouth nipples:
			if(pc.hasBreasts() && pc.hasFuckableNipples() && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			// Naga
			if(pc.isNaga() && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			// Centaur:
			if(pc.isTaur() && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
	...
	else if(service == "rooms")
	{
		...
		while(scenesLimit > 0 && scenesTotal > 0)
		{
			...
			if(pc.isTreated() && pc.isBimbo() && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			// Biped:
			if(pc.isBiped() && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
			...
			// Goo:
			if(pc.isGoo() && scenesLimit > 0 && scenesTotal > 0 && rand(scenesTotal) == 0)
			{
				pp.shiftCock(0, GLOBAL.TYPE_CANINE);
