...
SERA_MERCHANT					Sera’s merchant flag--also tracks item pools
...
public function sentientAcquisitionsBuyOutSera():void
{
	...
	clearMenu();
	// [Yes] [No]
	addButton(0, "Yes", sentientAcquisitionsBuyOutSeraYes);
...
public function sentientAcquisitionsBuyOutSeraYes():void
{
	...
	if(pc.isMale()) output(" - particularly of the male persuasion -");
	...
	clearMenu();
	addButton(0, "Penis", sentientAcquisitionsBuyOutSeraYesPenis);
...
public function approachServantSera(introText:Boolean = false):void
{
	...
	if(flags["SERA_OBEDIENCE_MIN"] <= 0)
	{
		...
		if(pc.hasCock()) addButton(1, "Tease", seraBitchTrainingTease, undefined, "Tease", "Tease your demonic slave.");
	...
	else
	{
		...
		if(pc.hasStatusEffect("Seranigans Punishment")) addButton(4, "Punish", seraBitcheningPunishMenu, true, "Punish", "Punish Sera for being a bad slave.");
		else addDisabledButton(4, "Punish", "Punish", "Sera hasn’t done anything bad to deserve punishment for.");
...
public function seraBitchTrainingTease():void
{
	...
	else
	{
		...
		output("\n\nYou squeeze one of her breasts in response, sinking your fingers deep into the softness there, making her exaggeratedly catch her breath. It is honestly difficult to not get aroused by the nearness of her exquisitely crafted body and her teasing, [pc.eachCock] hardening against her round behind");
		if(pc.hasVagina()) output(" and your " + (pc.vaginas.length == 1 ? "pussy juicing itself" : "pussies juicing themselves") + " eagerly");
...
public function seraBitchTrainingTeaseFinale():void
{
	...
	output("\n\nThrough the long minutes of the fierce, lush rutting that follows you reposition her, letting your hands roam over her body as you push and pull her this way and that, only ever pulling your cock free of the soaking suck of her sex in order to drive into it even harder from another angle. You bounce her big, soft behind in your lap/coils face-to-face for a time, your fingers in her mouth as you lick then chew upon her cherry-like nipples, waiting for the grunts to turn into groans around your digits before ordering her to cum again. Sera contracts in helpless delirium on top of your [pc.cock " + fits + "], breasts");
...
public function seraBitchTrainingRide():void
{
	...
	var tinyVag:Boolean = (pc.vaginalCapacity(vIdx) < chars["SERA"].cockVolume(0));
...
public function seraBitchTrainingPunishCumRation(fromMenu:Boolean = false):void
{
	...
	if(flags["SERA_BITCHENING_PUNISH_RATION"] == undefined || obedience <= 40)
	{
		...
		else
		{
			...
			output("\n\nYou return her smirk, revelling quietly in her arousing mixture of insolence and obedience, and point downwards silently. You give her murmured instructions when she resumes, pumping your [pc.hips] gently so she gets used to taking");
...
public function seraBitcheningStoreInventory(totalDays:int):void
{
	...
	{
		flags["SERA_MERCHANT"]++;
		if(flags["SERA_MERCHANT"] > 5) flags["SERA_MERCHANT"] = 0;
		nWeeks--;
	}
}
public function seraBitcheningStore(response:String = "buy"):void
{
	...
	switch(flags["SERA_MERCHANT"])
	{
		...
		case 0:
			...
			chars["SERA"].inventory.push(new JunkTrunk());
			chars["SERA"].inventory.push(new Condensol());
			chars["SERA"].inventory.push(new Virection());
			break;
		// Group B
		// DendroGro, Bumpy Road, Knot A Problem, Circumscriber, Equilibricum, Semen’s Friend
		case 1:
			chars["SERA"].inventory.push(new DendroGro());
			chars["SERA"].inventory.push(new BumpyRoad());
			chars["SERA"].inventory.push(new KnotAProblem());
			chars["SERA"].inventory.push(new Circumscriber());
			chars["SERA"].inventory.push(new Equilicum());
		...
		case 2:
			...
			chars["SERA"].inventory.push(new Lactaid());
			chars["SERA"].inventory.push(new MightyTight());
			chars["SERA"].inventory.push(new Anusoft());
			chars["SERA"].inventory.push(new MagicMilker());
		...
		case 3:
			chars["SERA"].inventory.push(new Throbb());
			chars["SERA"].inventory.push(new Gush());
			chars["SERA"].inventory.push(new RubberMade());
			chars["SERA"].inventory.push(new ClearYu());
			chars["SERA"].inventory.push(new Dumbfuck());
		...
		case 4:
			...
			chars["SERA"].inventory.push(new AusarTreats());
			chars["SERA"].inventory.push(new Nepeta());
			chars["SERA"].inventory.push(new DracoGuard());
			chars["SERA"].inventory.push(new OvirAce());
			chars["SERA"].inventory.push(new NukiCookies());
			break;
		// Group F
		// Mino Charge, Lucifier, Sylvanol, Dovebalm, Bovinium, Hornitol
		case 5:
			chars["SERA"].inventory.push(new MinoCharge());
			chars["SERA"].inventory.push(new Bovinium());
			chars["SERA"].inventory.push(new Lucifier());
			chars["SERA"].inventory.push(new Sylvanol());
			chars["SERA"].inventory.push(new DoveBalm());
			chars["SERA"].inventory.push(new Hornitol());
			break;
	}
...
public function seraBitcheningSexMenu():void
{
	...
	{
		if(pc.hasStatusEffect("Sera Tease Cooldown")) addDisabledButton(btnSlot++, "Tease", "Tease", "Probably best to let her recharge. Build that frustration up.");
		else addButton(btnSlot++, "Tease", seraBitcheningSexTeaseF, undefined, "Tease", "There’s no longer an objective to it, but it’s still fun to make her beg, right?");
	}
...
public function seraBitcheningSexTitfuck():void
{
	...
	output(".");
	output("\n\n<i>“Seen something you like, [pc.master]?”</i> Sera smirks. She puts her hands behind her horns and stretches, cat-like. Her boobs protrude, the ample flesh quivering restlessly. <i>“Gawping like a brat who’s been told how to disarm the childlock on his dex.”</i>");
...
public function seraBitcheningSexDoublePenetration():void
{
	...
	output("\n\nShe pants lustily, her anal and vaginal walls wringing your [pc.cockNounSimple " + cIdx + "] for all they’re worth, trying to milk you of your [pc.cum]. You slap her ass as you pull out, making her tighten up wonderfully around you [pc.cockNounComplex " + cIdx + "]. This time, it’s your turn to moan as your senses are assaulted by the sheer tightness of her holes. You slam roughly back in, a hand grabbing and using her spaded tail as leverage.");
	
	chars["SERA"].buttChange(pc.cockVolume(cIdx));
	chars["SERA"].cuntChange(pc.cockVolume(cIdx2));
	
	output("\n\nYour other hand goes to her breasts, and fondles one before grabbing and pinching a nipple. She gasps, and pushes back into you, trying to get as much of your [pc.cock " + cIdx + "] into her as possible. You don’t hold back, and pull out before ramming back in, your thrusts now slapping out a steady rhythm against her ass, accompanied by the sounds of Sera vocalizing in complete bliss. <i>“Yes, yes! Use me like the cheap slut I am, master! Fill me with your seed! Give it to me, give it all to me!”</i> she screams. You grab her chin and tilt it up to face you, and you notice her eyes are glazed over with ecstasy. They lock onto yours briefly before staring back into space.");
	...
	output("\n\nAfter a few more minutes of slamming into her tight holes, you feel Sera’s sphincter and vagina tighten and spasm around your [pc.cockNounSimple " + cIdx + "]. <i>“Don’t you dare cum until I tell you to do so, you little slut,”</i> you growl into her ear. She grimaces as she tries to control her urges, and you pick up the pace, roughly fucking her until you feel an all too familiar twitch in your " + (pc.balls > 0 ? "[pc.sack]" : "[pc.cock " + cIdx + "]") + ", signalling your imminent explosion. You hold out as long as you can, then let the feeling rush through [pc.eachCock], [pc.cum] blasting out and painting her colon and vagina [pc.cumColor].");
	...
	output("\n\nHer sphincter and vagina clamp down around your [pc.cockNounSimple " + cIdx + "] as she cums to the feeling of your [pc.cum] filling her up to the brim. She cums quite hard, femcum drenching your [pc.legOrlegs],");
...
public function seraBitcheningSexTeaseF():void
{
	...
	output("\n\nIt’s wet and eager as ever, beading lustily onto your fingers, conditioned now to practically sit up and beg for you. You travel upwards and easily find the sturdy nub of her clit, protruding out of her lips as it is. You circle and then gently play with it, nudging and sliding over it as you continue to run your fingers over her silky, gooey pink slit.");
	if(chars["SERA"].hasCock()) output(" As you do so you slide your other hand upwards, grasping her cock and begin to jerk her, tightening, softening and bending your grasp around her thick meat.");
	output(" Sera grunts, flexing her hands in her bonds. She knows it’s useless to struggle or to exhort you to fuck her, and in the place where she is now, she’d much rather play up to it anyway. She pushes her pear of a behind back into you, sinuously writhing her form so it melds with your own, rubbing your erect [pc.eachCock] and [nipples] insistently, drawing you further and further into the delight of touching and teasing her over-sexed form.");
...
public function seraBitcheningSexTeaseFDeny():void
{
	...
	output("You keep curling your fingers into her with a deceptively steady rhythm, feeling the evidence of her excitement trickling warmly down your fingers, enjoying the heave of her soft flesh against your [pc.chest] and [pc.eachCock]. You’ve got her in the palm of your hand and you glory in it, drinking in her high scent, her neck is thrown back, her mouth open, all but tasting the incoming orgasm... and then you stop dead.");
	output("\n\n<i>“No... [pc.master], no!”</i> she whines in bitter frustration, humping your frozen hand" + (!chars["SERA"].hasCock() ? "" : "s") + ".");
...
{
	if(initialize) chars["SERA"].inventory = [];
...
public function seraBitcheningPunishWalkAcc():void
{
	...
	if(chars["SERA"].inventory.length > 0)
	{
		for(var i:int = 0; i < chars["SERA"].inventory.length; i++)
		{
			output("\n* " + (StringUtil.toDisplayCase(chars["SERA"].inventory[i].longName)));
		}
	}
	...
	{
		if(!chars["SERA"].hasItem(setItem)) addButton(0, setItem.shortName, seraBitcheningPunishWalkAccSelect, setItem, StringUtil.toDisplayCase(setItem.longName), "Use this item.");
	...
	if(pc.hasItem(setItem) || pc.hasItemInStorage(setItem))
	{
		if(!chars["SERA"].hasItem(setItem)) addButton(5, setItem.shortName, seraBitcheningPunishWalkAccSelect, setItem, StringUtil.toDisplayCase(setItem.longName), "Use this item.");
	...
	if(pc.hasItem(setItem) || pc.hasItemInStorage(setItem))
	{
		if(!chars["SERA"].hasItem(setItem)) addButton(1, setItem.shortName, seraBitcheningPunishWalkAccSelect, setItem, StringUtil.toDisplayCase(setItem.longName), "Use this item.");
	...
	if(pc.hasItem(setItem) || pc.hasItemInStorage(setItem))
	{
		if(!chars["SERA"].hasItem(setItem)) addButton(2, setItem.shortName, seraBitcheningPunishWalkAccSelect, setItem, StringUtil.toDisplayCase(setItem.longName), "Use this item.");
		else addDisabledButton(2, setItem.shortName, StringUtil.toDisplayCase(setItem.longName), "This item is already being used.");
	}
	setItem = (new BioWhip());
	if(pc.hasItem(setItem) || pc.hasItemInStorage(setItem))
	{
		if(!chars["SERA"].hasItem(setItem)) addButton(7, setItem.shortName, seraBitcheningPunishWalkAccSelect, setItem, StringUtil.toDisplayCase(setItem.longName), "Use this item.");
		else addDisabledButton(7, setItem.shortName, StringUtil.toDisplayCase(setItem.longName), "This item is already being used.");
	}
	
	if(chars["SERA"].inventory.length > 0) addButton(13, "Reset", seraBitcheningPunishWalkies, true, "Reset Accessories", "Try taking different items.");
	else addDisabledButton(14, "Reset");
	addButton(14, "Let’s Go", seraBitcheningPunishWalkiesGo, undefined, "Let’s Go", "Ready to go!");
	
	return;
}
public function seraBitcheningPunishWalkAccSelect(item:*):void
{
	if(item != null && !chars["SERA"].hasItem(item)) chars["SERA"].inventory.push(item);
...
public function seraBitcheningPunishWalkiesGo():void
{
	...
	var hasSchoolgirl:Boolean = (chars["SERA"].hasItem(new SchoolgirlOutfit()) || chars["SERA"].hasItem(new SchoolgirlCostume()));
	var hasCuffs:Boolean = (chars["SERA"].hasItem(new GravCuffs()));
	var hasWhip:Boolean = (chars["SERA"].hasItem(new Whip()) || chars["SERA"].hasItem(new BioWhip()));
...
public function seraBitcheningPunishWalkiesGoPtII():void
{
	...
	if(witness.length > 1)
	{
		...
		if(rand(2) == 0 && eventQueue.indexOf(passerB) == -1) eventQueue.push(passerB);
	}
...
public function seraBitcheningPunishWalkiesWitnessSemith():void
{
	...
	output("\n\nYou run into Semith the vulkrim along the Eastern Plaza, engrossed in his holo-pad. The besuited little devil gives you a distracted grin at first glance, a more interested gaze at the second.");
