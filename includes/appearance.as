...
function appearance(target:Creature):void {
	...
	if(target == pc) {
		...
		if(target.faceType == GLOBAL.NALEEN_FACE || target.faceType == GLOBAL.HUMAN || target.faceType == GLOBAL.SHARK || target.faceType == GLOBAL.SIREN || target.faceType == GLOBAL.LAPINE || target.faceType == GLOBAL.NAGA) {
			...
			if(target.skinType != GLOBAL.FUR && target.skinType != GLOBAL.SCALES) {
				output2(" Your face is human in shape and structure, with " + target.skin(true,true));
				if((target.skinTone == "ebony" || target.skinTone == "black") && (target.skinType == GLOBAL.SKIN || target.skinType == GLOBAL.GOO)) output2(", though with your dusky hue, the black raccoon mask you sport isn't properly visible.");
			...
			//appearance furscales
				...
				if(((target.hairColor == "black" || target.hairColor == "midnight") && (target.skinType == GLOBAL.FUR || target.skinType == GLOBAL.SCALES))) output2(" Under your " + target.skinFurScales(true,true) + " hides a black raccoon mask, barely visible due to your inky hue, and");
		...
		else if(target.faceType == GLOBAL.KUITAN) {
			...
			//(if skin)
			if(target.skinType == GLOBAL.SKIN) output2(" It looks a bit strange with only the skin and no fur.");
			else if(target.skinType == GLOBAL.SCALES) output2(" The presence of said scales gives your visage an eerie look, more reptile than mammal.");
		}
		else if(target.faceType == GLOBAL.VULPINE) {
		...
		else if(target.faceType == GLOBAL.MOUSEMAN) {
			//appearance
		...
		else if(target.faceType == GLOBAL.MOUSE) {
			//appearance
			output2(" You have a snubby, tapered mouse's face, with whiskers, a little pink nose, and ");
			if(target.skinType != GLOBAL.FUR && target.skinType != GLOBAL.SCALES) output2(target.skin(true,true));
			else output2(target.skin(true,true) + " under your " + target.skinFurScales(true,true));
			output2(". Two large incisors complete it.");
		}
		//Naga
		if(target.faceType == GLOBAL.SNAKE) {
			output2(" You have a face that resembles a combination between a human's and a snake's,");
			if(target.skinType == GLOBAL.SKIN) output2(" oddly uncovered by scales. Instead, your snake-like snout is covered in " + target.skin(true,true) + ".");
			if(target.skinType == GLOBAL.FUR) output2(" covered in a layer of " + target.skinFurScales(true,true) + ".");
			if(target.skinType == GLOBAL.SCALES) output2(" covered in " + target.skinFurScales(true,true) + ", as one would expect of such a reptilian visage.");
			output2("In addition, a pair of poisonous fangs protrude from your mouth over your lower lip.");
		}
		//horse-face
		if(target.faceType == GLOBAL.EQUINE) {
			if(target.skinType == GLOBAL.SKIN || target.skinType == GLOBAL.GOO) output2(" Your face is equine in shape and structure. The odd visage is hairless and covered with " + target.skinFurScales(true,true) + ".");
			if(target.skinType == GLOBAL.FUR) output2(" Your face is almost entirely equine in appearance, even having " + target.skinFurScales() + ". Underneath the fur, you believe you have " + target.skin(true,false) + ".");
			if(target.skinType == GLOBAL.SCALES) output2(" You have the face and head structure of a horse, overlaid with " + target.skinFurScales(true,true) + ".");
		}
		//dog-face
		if(target.faceType == GLOBAL.CANINE) {
			if(target.skinType == GLOBAL.SKIN || target.skinType == GLOBAL.GOO) output2(" You have a dog-like face, complete with a wet nose. The odd visage is hairless and covered with " + target.skinFurScales(true,true) + ".");
			if(target.skinType == GLOBAL.FUR) output2(" You have a dog's face, complete with wet nose and panting tongue. You've got " + target.skinFurScales(true,true) + ", hiding your " + target.skin(true,true) + " underneath your furry visage.");
			if(target.skinType == GLOBAL.SCALES) output2(" You have the facial structure of a dog, wet nose and all, but overlaid with " + target.skinFurScales(true,true) + ".");
		}
		//cat-face
		if(target.faceType == GLOBAL.FELINE) {
			if(target.skinType == GLOBAL.SKIN || target.skinType == GLOBAL.GOO) output2(" You have a cat-like face, complete with a cute, moist nose and whiskers. The " + target.skin(true,true) + " that is revealed by your lack of fur looks quite unusual on so feline a face.");
			if(target.skinType == GLOBAL.FUR) output2(" You have a cat-like face, complete with moist nose and whiskers. Your " + target.skinFurScales(true,true) + " hides " + target.skin(true,true) + " underneath.");
			if(target.skinType == GLOBAL.SCALES) output2(" Your facial structure blends humanoid features with those of a cat. A moist nose and whiskers are included, but overlaid with " + target.skinFurScales(true,true) + ".");
		}
		//Minotaaaauuuur-face
		if(target.faceType == GLOBAL.BOVINE) {
			if(target.skinType == GLOBAL.SKIN || target.skinType == GLOBAL.GOO) output2(" You have a face resembling that of an anthropomorphic bovine, with cow-like features, particularly a squared off wet nose. Despite your lack of fur elsewhere, your visage does have a short layer of " + target.furColor + " fuzz.");
		...
		if(target.faceType == GLOBAL.KANGAROO) {
			output2(" Your face is ");
			output2("shaped like that of a kangaroo ");
			if(target.skinType == GLOBAL.SKIN) output2("but bald, not covered in fur");
		...
		if(pc.hairLength == 0) {
			...
			else if(pc.earType == GLOBAL.VULPINE) output2(" A pair of large, adept fox ears sit high on your skull, always listening.");
		...
		//not bald
			...
			if(pc.earType == GLOBAL.HUMAN) output2(" Your " + target.hairDescript(true,true) + " looks good on you, accentuating your features well.");
			else if(pc.earType == GLOBAL.EQUINE) output2(" The " + target.hairDescript(true,true) + " on your head parts around a pair of very horse-like ears that grow up from your head.");
			...
			else if(pc.earType == GLOBAL.VULPINE) output2(" The " + target.hairDescript(true,true) + " atop your head is parted by a pair of large, adept fox ears that always seem to be listening.");
		...
		//Tongue
		if(pc.tongueType == GLOBAL.NAGA || pc.tongueType == GLOBAL.SNAKE) output2(" A snake-like tongue occasionally flits between your lips, tasting the air.");
		else if(pc.tongueType == GLOBAL.DEMONIC) output2(" A slowly undulating tongue occasionally slips from between your lips. It hangs nearly two feet long when you let the whole thing slide out, though you can retract it to appear normal.");
		else if(pc.tongueType == GLOBAL.DRACONIC) output2(" Your mouth contains a thick, fleshy tongue that, if you so desire, can telescope to a distance of about four feet. It has sufficient manual dexterity that you can use it almost like a third arm.");
		//Horns
		//Demonic horns
		if(pc.hornType == GLOBAL.DEMONIC) {
		...
		//Lizard horns
		else if(pc.hornType == GLOBAL.LIZAN) {
		...
		//Antlers!
		else if(pc.hornType == GLOBAL.DEER) {
		...
		else if(target.tailType == GLOBAL.VULPINE) {
		...
		else if(target.legType == GLOBAL.LAPINE) output2("  Your legs thicken below the waist as they turn into soft-furred rabbit-like legs.  You even have large bunny feet that make hopping around a little easier than walking.");
		else if(target.legType == GLOBAL.AVIAN) output2("  Your legs are covered with " + target.furColor + " plumage.  Thankfully the thick, powerful thighs are perfect for launching you into the air, and your feet remain mostly human, even if they are two-toed and tipped with talons.");
		else if(target.legType == GLOBAL.KANGAROO) output2("  Your furry legs have short thighs and long calves, with even longer feet ending in prominently-nailed toes.");
		else if(target.legType == GLOBAL.ARACHNID) output2("  Your legs are covered in a reflective black, insectile carapace up to your mid-thigh, looking more like a pair of 'fuck-me-boots' than exoskeleton.");
		else if(target.legType == GLOBAL.DRIDER) output2("  Where your legs would normally start you have grown the body of a spider, with eight spindly legs that sprout from its sides.");
		...
		if(target.balls > 0) {
			...
			if(target.hasStatusEffect("Uniball")) {
				...
				if(target.skinType == GLOBAL.SKIN) output2("A " + target.sackDescript(true,true) + " with " + target.ballsDescript(true,true) + " swings heavily under where a penis would normally grow.");
				if(target.skinType == GLOBAL.FUR) output2("A fuzzy " + target.sackDescript(true,true) + " filled with " + target.ballsDescript(true,true) + " swings low under where a penis would normally grow.");
				...
				if(target.skinType == GLOBAL.GOO) output2("An oozing, semi-solid sack with " + target.ballsDescript(true,true) + " swings heavily under where a penis would normally grow.");
			}
			else {
				if(target.skinType == GLOBAL.HUMAN) output2("A " + target.sackDescript(true,true) + " with " + target.ballsDescript(true,true) + " swings heavily beneath your " + target.multiCockDescript() + ".");
				if(target.skinType == GLOBAL.FUR) output2("A fuzzy " + target.sackDescript(true,true) + " filled with " + target.ballsDescript(true,true) + " swings low under your " + target.multiCockDescript() + ".");
				if(target.skinType == GLOBAL.SCALES) output2("A scaley " + target.sackDescript(true,true) + " hugs your " + target.ballsDescript(true,true) + " tightly against your body.");
