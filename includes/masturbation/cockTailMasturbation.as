...
public function dickTailFuck():void
{
	...
	if(9999) output("Your tail is acting oddly, seemingly under the control of your baser urges. ");
	output("You smile wide as a rather devious thought crosses your mind. Before you can give said thought a second thought, you ");
	...
	if(pc.hasVagina()) 
	{
		output("\n\nYou let out a gasp of pleasure as your [pc.tailCock] tries to force itself inside of you, its head rubbing against the lips of your [pc.vagina]. Your tail finally manages to push its way into your orifice, spreading you wide, causing you to cry out in delight as the sinuous organ buries itself deeper inside you, falling back onto the ");
		if(currentLocation == "SHIP INTERIOR") output("mattress");
		else output("ground");
		output(", clawing at ");
		if(currentLocation == "SHIP INTERIOR") output("your sheets");
		else output("the floor");
		output(".");

		pc.cuntChange(0,pc.tailCockVolume())

		output("\n\nIt isn’t long before your [pc.tail] starts thrusting in and out of your [pc.vagina], slowly building up speed, setting you moaning as your love canal hugs at the appendage’s girth. Surprisingly, your tail seems to be fucking you with enough force to ");
		if(currentLocation == "SHIP INTERIOR") output("set your mattress rocking slightly");
		else output("make your body rock back and forth slightly");
		output(", each inward thrust forcing your [pc.vagina] wide open, making you whimper like a well-fucked whore.");
		...
		pc.loadInCunt(chars["COCKVINE"], 0);
	}
	//noVagina:
	else
	{
...
public function tailSucking():void
{
	...
	showName("TAIL\nSUCKING");
	author("HugsAlright");
	//9999 if(ifCouchMysteryTF) output("Your tail is acting oddly, seemingly under the control of your baser urges. ");
	output("You smile wide as a rather devious thought crosses your mind. Before you can give said thought a second thought, you ");
	...
	output("plop yourself down ");
	if(currentLocation == "SHIP INTERIOR") output("on the edge of your bed");
	...
	output("\n\nAs you wind down, your tailcock retracts itself from your mouth, leaving your jaw relatively sore. You cough up a bit of your [pc.tail]’s cum as it finishes its exodus. Smiling at your happily relieved tail, you lick your spunk-covered [pc.lipsNoun], stroking the appendage appreciatively as it returns to its position behind you. Nothing left to do now besides clean up and get ready to move again.");
