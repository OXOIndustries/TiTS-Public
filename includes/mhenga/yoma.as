 ...
 * edit Vaginal scene with HL support
 ...
 *
 *
 * conditions for the bar									DONE
 * Randomize bar
 * prevent yoma from spawning in the jungle while wainting in the bar
 * Move timers so PC cant instantly continue dates?
 * ADD PLANETS as requirements to proceed with the dates	DONE
 * 
 * add times to the scenes									DONE
 * Add pc.orgasm and stuff to the sex scenes in the dates	DONE
 * Update Codex												DONE
 * add HL to vaginal sex scene?								DONESKI
 * add penis/vagina selector?								NOPE!
 */
...
public function yomaDEBUG():void
{
	...
	output("this is just for debugging, choose your option below");
...
public function yomaAtBurts():Boolean {
	...
	if (flags["YOMA_SEEN_NAKED"] == undefined || flags["YOMA_TALKED_ABOUT_MASTER"] == undefined || flags["YOMA_GAMES_PLAYED"] == undefined) return false;
...
public function yomaCooldown(infinite:Boolean = false):void
{
	//doesnt seem to work the way I want
	if (infinite) pc.createStatusEffect("Yoma Cooldown");
...
public function yomaDateComeBackLater():void
{
	...
	processTime(3);
	// disable jungle encounter until you proceed
	yomaCooldown(true);
