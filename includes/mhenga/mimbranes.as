	...
	if ((pc.hasStatusEffect("Mimbrane Foot Left") && pc.statusEffectv3("Mimbrane Foot Left") >= 15) || pc.hasStatusEffect("Mimbrane Foot Right") && pc.statusEffectv3("Mimbrane Foot Right") >= 15)
	{
		...
		if (trustedRepro && rand(100) <= 3)
		{
			...
			eventQueue.push(function():void {
				...
				output(" spasms suddenly. Your toes clench and stretch. Has some nerve disorder crept within you? Has fucking the galaxy’s many offerings proven to be your undoing? Throwing your covers off rids you of these qualms. It’s merely your Mimbrane");
...
public function neglectedMimbranesCombat():void
{
	...
	{
		output("\n\nIt's difficult for you to focus as the hungry Mimbrane");
		if (attachedMimbranes() > 1) output("s");
		output(" revolt");
		if (attachedMimbranes() == 1) output("s");
		output(" against you,");
	...
	for (var ii:int = 0; ii < eligibleMimbranes.length; ii++)
	{
		...
		if (rand(4) == 0) // 1 in 4 (0-3)
		{
			...
			if (rand(5) == 0) // 1 in 5 (0-4)
			{
				...
				{
					output("\nIt's bad enough that the Mimbrane encasing your head has drenched you in a rich layer of sweat, but now you’re having trouble seeing through the thick cloud of salacious intent that’s forming as a result. You involuntarily go to wipe your brow, which accidentally causes you to inhale some of the dangerous fumes. The parasite manipulates your [pc.face] enough to avoid any further exposure to its premature attack.\n");
				}
