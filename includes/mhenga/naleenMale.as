...
public function getButtfuckedByDudeleen():void {
	...
	else output("You bite your lip, approaching the feline-reptile hybrid with an obvious excitement. He looks at you, following your gaze down to his twin reptilian dicks.");
	...
	if(pc.hasVagina()){
		output("\n\nbut as he moves to thrust you feel a cock testing the entrance of [pc.oneVagina].");
	...
	output("\n\n<i>“More,”</i> you say, turning back to look him in the eye, <i>“");
		if(!silly) output("Faster! Harder!”</i>");
		else output("Harder, better, faster stronger!”</i>");
	...
	if(pc.analCapacity() > 300) output("Your stomach swells in time with his movements, the combined girth of his dicks stretching you to an incredible extreme.")
	...
	output("\n\nThere’s only so much you can take, though, and you feel your limits being tested. The naleen is barely holding on himself, shivering whenever your [pc.ass] ");
		...
		else if(pc.analCapacity() > 300) output("squeezes his dicks.");
	...
	output(" Eventually a long groan overcomes him as he slams into you one final time, burying himself as deep as he can. Both of his cocks pulse as they blow their load, and the sensation of being ");
		if(pc.hasCuntTail() || pc.analCapacity() > 300) output("simultaneously covered and ");
...
public function loseToDudeleenSex(tailIntro:Boolean = false):void {
	...
	if(pc.hasVagina())
	{
		x = pc.cuntThatFits(chars["NALEEN_MALE"].cockVolume(0));
		y = -1;
		if(x < 0) x = rand(pc.totalVaginas());
		if(y < 0 && pc.totalVaginas() > 1)
		{
