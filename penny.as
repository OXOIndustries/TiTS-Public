...
function pennyRoomDesc():void {
	...
	if(flags["ZIL_PROBLEM_DEALT_WITH"] == undefined) {
		...
		addButton(0,"Need Help?",askPennyIfSheNeedsHelp);
	}
	//Friendly penny
		...
		addButton(0,"Approach",approachFriendPenny);
	}
	//Girlfriend penny
	else {
		//Non-Futa
		...
		addButton(0,"Approach",approachGirlfriendPenny);
	}
}
...
function askPennyIfSheNeedsHelp():void {
	...
	clearMenu();
	addButton(2,"Get Intercom",getAnIntercom);
	addButton(0,"Fuck Zil",fuckTheZilForPennyBecauseYouAreADumbStupidSlut);
	addButton(1,"Fight Them",fightDemZil);
...
function fuckTheZilForPennyBecauseYouAreADumbStupidSlut():void {
	...
	clearMenu();
	addButton(0,"Explain",explainWhyYoureAHugeSlut);
	addButton(1,"FlirtItOff",flirtySlutPCsTellPennyFlirts);
	addButton(4,"Nevermind",askPennyIfSheNeedsHelp);
}
...
function explainWhyYoureAHugeSlut():void {
	...
	clearMenu();
	addButton(0,"Yes",getSpitRoastedForPenny);
	addButton(1,"Nope",fuckTheZilForPennyBecauseYouAreADumbStupidSlut);
}
...
function getSpitRoastedForPenny():void {
	...
	if(pc.isLactating()) {
		...
		if(pc.milkType == MILK) output(" If your mouth wasn’t full of cock, you’d moo like the universe's sluttiest cow.");
	}
	...
	clearMenu();
	addButton(0,"Next",getFucked4PennyEpilogue);
}
...
function getFucked4PennyEpilogue():void {
	...
	clearMenu();
	addButton(0,"Next",postSexShower);
}
...
function fightDemZil():void {
	...
	//Fight them
	clearMenu();
	addButton(0,"Pussy Out",askPennyIfSheNeedsHelp);
	addButton(1,"Fight Them",seriouslyFightTheZil);
}
...
function seriouslyFightTheZil():void {
	...
	clearMenu();
	//Start combat
	addButton(0,"Next",startCombat,"zilpack");
}
...
function loseToZilPair():void {
	...
	pc.lust(100);
	clearMenu();
	addButton(0,"Next",loseToZilpackPartTwoLoseHarder);
}
...
function loseToZilpackPartTwoLoseHarder():void {
	...
	clearMenu();
	addButton(0,"Next",finishLosingToZilYouLoser);
}
...
function defeatZilPair():void {
	...
	pc.lust(10);
	clearMenu();
	addButton(0,"Neat",beatPennysZilPartII);
}
...
function approachFriendPenny(outputT:Boolean = true):void {
	...
	addButton(0,"Talk:Yourself",talkToPennyAboutYourself);
	addButton(1,"Talk:Youth",pennysYouth);
	addButton(2,"Talk:Fun",whatDoesPennyDoForFun);
	addButton(3,"Talk:Species",talkToPennyAboutSpecies);
	if((pc.hasCock() || pc.hasVagina()) && pc.lust() >= 33) addButton(4,"Sex",pennySexFirstTime);
...
function pennySexFirstTime():void {
	...
	//Done All Talks (Req’s Gender)
		...
		clearMenu();
		if(pc.hasCock()) {
			if(pc.cockThatFits(penny.vaginalCapacity()) >= 0) addButton(0,"Next",firstTimePennyShowerSex);
			else addButton(0,"Next",stickAHugeDickInPennysWorld);
		}
		else addButton(0,"Next",femaleFirstTimePennyLesboz);
		flags["SEXED_PENNY"] = 1;
	}
}
...
function femaleFirstTimePennyLesboz():void {
	...
	else 
	{
		...
		if(pc.breastRows[0].nippleType == FLAT) output("even though they have no proper tips, only engorged areolae, ");
	...
	if(pc.hasTongueFlag(LONG)) 
	{
		...
		if(pc.hasTongueFlag(PREHENSILE)) output(" Your dextrous muscles make it easy to control your tongue inside her, and you coil it so that it can press on all the portions of her walls at once, sliding over every fold and sensitive portion with skilled licks.");
	}
...
function stickAHugeDickInPennysWorld():void {
	...
	else if(pc.hasCockFlag(x,FLARED)) output("When she sees your flare, she licks around the edge of the expanding dick-rim with a happy hum.");
	else if(pc.hasCockFlag(x,NUBBY)) output("When she feels the nubs, she stops and suckles on them one after another, happiness plastered on her face.");
	else if(pc.hasCockFlag(x,STINGER_TIPPED)) output("When she feels the tendrils pricking on her cheek, she gasps and turns, unintentionally getting stung all over her lips. They swell slightly, but her eyelids lower as the aphrodisiac venom sinks in. Her oral service resumes in short order with even greater enthusiasm.");
	else if(pc.hasCockFlag(x,AMORPHOUS)) output("When the heat half-melts your gooey boner, Penny purrs, blowing bubbles over your semi-liquid shaft to happily pleasure it. The bubbly bliss is intimately pleasurable, and Penny seems to greatly be enjoying the goo-play.");
...
function approachGirlfriendPenny():void {
	...
	processTime(1);
	clearMenu();
	addButton(0,"Talk",talkToGirfriendPenny);
	if(pc.lust() >= 33) addButton(1,"Sex",pennySexMenu);
...
function talkToGirfriendPenny():void {
	...
	clearMenu();
	addButton(0,"Her ‘Species’",askGFPennyAboutSpecies);
	addButton(1,"Happy",isGFPennyHappy);
	addButton(2,"Goals",goalsForGFPenny);
	addButton(3,"TouchFluffTail",touchFluffyTail);
	if(flags["HAD_PENNY_GOAL_CHAT"] == 1) addButton(4,"Futanari",pennyFutanariTalk);
	if(flags["PENNY_THROBB_PURCHASE_UNLOCKED"] == 1) addButton(5,"Buy Throbb",buySomeThrobbFrompenny);
...
function pennyFutanariTalk():void {
	...
	if (flags["PENNY_FUTA_TALK"] == undefined)
	{
		...
		clearMenu();
		addButton(0,"Stay Girl",stayAGirlYouBitch);
		addButton(1,"Get A Dick",pennySproutsAWeiner);
	}
	...
	else
	{
		...
		else
		{
			if(penny.longestCockLength() < 10)
			{
				...
				clearMenu();
				addButton(0,"Give Throbb",pennyToBigPenny);
				addButton(14,"Back",approachGirlfriendPenny);
			}
			//Footlong Penny
			else if(penny.longestCockLength() < 17)
			{
				...
				clearMenu();
				addButton(0,"Give Throbb",pennyBecomesHYPERPENNY);
				addButton(14,"Back",approachGirlfriendPenny);
			}
			//Hyper Penny
			else {
...
function growADickPenny():void {
	...
	addButton(0,"Grow It",pennySproutsAWeiner);
	addButton(1,"Nevermind",stayAGirlYouBitch);
}
...
function buySomeThrobbFrompenny():void {
	...
	output(".");
	clearMenu();
	if(pc.credits >= 300) addButton(0,"Buy It",buyThrobbTransact);
	else addDisabledButton(0,"Buy It");
	addButton(14,"Back",approachGirlfriendPenny);
}
//[Buy] [Back]
//Transact It
...
function pennySexMenu(outputT:Boolean = true):void {
	...
	if (pc.hasCock())
	{
		...
		if (pc.cockThatFits(penny.vaginalCapacity()) >= 0) addButton(1,"OverDeskFuck",pennyOverTheDeskRoughFuck);
		else addDisabledButton(1,"OverDeskFuck");
		if(penny.hasCock()) addButton(2,"Frottage",pennyFrottage);
		else addDisabledButton(2,"Frottage");
		addButton(3,"Pawjob",penniliciousPawjob);
		addButton(4,"FaceFuckHer",faceFuckPenny);
	}
	else 
	{
		addDisabledButton(0,"Onahole Her");
		addDisabledButton(1,"OverDeskFuck");
		addDisabledButton(2,"Frottage");
		addDisabledButton(3,"Pawjob");
		addDisabledButton(4,"FaceFuckHer");
	}
	addButton(5,"EatHerOut",gardeFordWritesPennySmex);
	addButton(14,"Back",approachGirlfriendPenny);
...
function faceFuckPenny():void {
	...
	clearMenu();
	if(pc.hasCock()) addButton(0,"Fuck Her Butt",fuckPennyButtFromFraceFuckSceneNyahhhh);
	if(penny.hasCock()) addButton(1,"GetButtFucked",getAssFuckedByPenny);
	addButton(2,"Nah, girl",nahhhhGurrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrllllllllllllllllllllllllllllllllllll);
}
...
function penniliciousPawjob():void {
	...
	clearMenu();
	addButton(0,"Lick Out",lickOutSomePennyDawg);
	if(penny.hasCock()) addButton(1,"Suck Off",suckOffPennyDawgYoMahDiggity);
	addButton(2,"GetGroundOn",getGroundOnByPenny);
	if(penny.hasCock()) addButton(3,"Frot",pennyFrottage);
}
...
function pennyFrottage():void {
	...
	if(pc.cumType == CUM) output("matches it nicely");
	...
	pc.orgasm();
	clearMenu();
	addButton(0,"Next",postPennyFrottage);
}
...
function jackinPennyOffWithAnOnahole():void {
	...
	addButton(0,"Throw Away",throwPennysCumAway);
	addButton(1,"Bite It",bitePennysCondom);
	addButton(2,"Feed2Her",feedPennysCondomToHer);
}
...
function gardeFordWritesPennySmex():void {
	...
	if (pc.tallness < 60)
	{
		...
		if(!pc.hasTongueFlag(LONG)) output(" You slather her walls with your saliva, and it mixes with her juices as the combined fluids run down her legs like a softly flowing stream.");
		...
		if(!pc.hasTongueFlag(LONG)) output("juices");
		else output("fluid");
		output(".");
	...
	else
	{
		...
		//if short tongue: 
		if(!pc.hasTongueFlag(LONG)) output("You slather her walls with your saliva, mixing it with her juices and letting it run into your mouth and down her body like a mountain stream.");
