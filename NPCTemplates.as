...
function initializeNPCs(justUpdate:Boolean = false):void {
	...
	//Rival!
	if (!justUpdate || (justUpdate && characters[RIVAL].short == "uncreated"))
	{
	...
	if (!justUpdate || (justUpdate && characters[GEOFF].short == "uncreated"))
	{
	...
	if (!justUpdate || (justUpdate && characters[FLAHNE].short == "uncreated"))
	{
	...
	trace("JUST BEFORE ZIL UPDATE. Justupdate: " + justUpdate + " short: " + characters[ZILPACK].short);
	if (!justUpdate || (justUpdate && characters[ZILPACK].short == "uncreated"))
	{
		trace("UPDATING ZIL");
		characters[ZILPACK] = new creature();
		zilpack = characters[ZILPACK];
		...
		zilpack.hairType = BEE;
		zilpack.beardLength = 0;
		zilpack.beardStyle = 0;
		zilpack.skinType = CHITIN;
		zilpack.skinTone = "black";
		zilpack.skinFlags = new Array();
		zilpack.faceType = BEE;
		zilpack.faceFlags = new Array();
		zilpack.tongueType = BEE;
		zilpack.lipMod = 0;
		zilpack.earType = 0;
		zilpack.antennae = 2;
		zilpack.antennaeType = BEE;
		zilpack.horns = 0;
		zilpack.hornType = 0;
		zilpack.armType = BEE;
		zilpack.gills = false;
		zilpack.wingType = BEE;
		zilpack.legType = BEE;
		zilpack.legCount = 2;
		zilpack.legFlags = new Array(PLANTIGRADE);
		//0 - Waist
		//1 - Middle of a long tail. Defaults to waist on bipeds.
		//2 - Between last legs or at end of long tail.
		//3 - On underside of a tail, used for driders and the like, maybe?
		...
		zilpack.milkType = HONEY;
		//The rate at which you produce milk. Scales from 0 to INFINITY.
		zilpack.milkRate = 0;
		zilpack.ass.wetness = 0;
	}
	if (!justUpdate || (justUpdate && characters[ZIL].short == "uncreated"))
	{
		trace("UPDATING ZIL!");
		//Zil mob a singular clone of zilpack!
		characters[ZIL] = clone(characters[ZILPACK]);
		zil = characters[ZIL];
		zil.plural = false;
		zil.short = "zil";
		zil.removeCock(0,1);
		...
		zil.XP = 20;
		zil.HPRaw = zil.HPMax();
		trace("ZIL SHORT: " + zil.short + " characters[ZIL]: " + characters[ZIL].short);
	}
	
	if (!justUpdate || (justUpdate && characters[PENNY].short == "uncreated"))
	{
		characters[PENNY] = new creature();
		penny = characters[PENNY];
		penny.short = "Penny";
		...
		penny.faceType = VULPINE;
		penny.faceFlags = new Array();
		penny.tongueType = 0;
		penny.lipMod = 1;
		penny.earType = VULPINE;
		penny.antennae = 0;
		...
		//3 - On underside of a tail, used for driders and the like, maybe?
		penny.genitalSpot = 0;
		penny.tailType = VULPINE;
		penny.tailCount = 1;
		penny.tailFlags = new Array(FLUFFY,LONG,THICK,FURRED);
		//Used to set cunt or dick type for cunt/dick tails!
