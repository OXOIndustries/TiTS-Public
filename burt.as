...
function burtBarMenu():void {
	...
	clearMenu();
	addButton(0,"Drink",burtDrinkMenu);
	addButton(1,"Talk",talkToDisBurtGuey);
	if(pc.lust() >= 33) addButton(2,"Sex",burtSex);
	else addDisabledButton(2,"Sex");
	if(flags["BURT_COLLECTABLE_SHOP"] == undefined) addButton(3,"Collectables?",BurtShopCollectables);
	else addButton(3,"Shop",BurtShopCollectables);
...
function burtDrinkMenu():void {
	...
	clearMenu();
	if(pc.credits >= 1) addButton(0,"Crabbst",crabbstBlueRibbonPurchase);
	else addDisabledButton(0,"Crabbst");
	if(pc.credits >= 10) addButton(1,"Mead",buyMeadFromBurt);
	else addDisabledButton(1,"Mead");
	if(pc.credits >= 50) addButton(2,"X-Zil-rate",burtXilErAte);
	else addDisabledButton(2,"X-Zil-rate");
...
function talkToDisBurtGuey():void {
	...
	clearMenu();
	addButton(0,"Him",askBurtAboutHimself);
	addButton(1,"Mead Hall",talkToBurtAboutMeadHall);
	addButton(2,"Zil",talkToBurtAboutDeseZilGueys);
	addButton(3,"Work",talkToBurtAboutWork);
	addButton(14,"Back",burtapproach);
}
...
function burtSex():void {
	...
	if(flags["TIMES_FUCKED_BURT"] == undefined)
	{
		...
		clearMenu();
		addButton(0,"Next",burtFirstTimeSexPartI);
	}
	//Repeat
	else {
		clearOutput();
...
function burtFirstTimeSexPartI():void {
	...
	clearMenu();
	//[Yes]		[No, Sorry]
	addButton(0,"Yes",yesIWantYerCrazyHoarseWeinerBurt);
	addButton(1,"No, Sorry",noRoboHorseWeinersHere);
}
...
function burtSexMenu():void {
	...
	addButton(0,"Give BJ",giveBurtABJ);
	if(pc.hasVagina() || pc.hasCock()) addButton(1,"ReceiveOral",getOralFromBurt);
	else addDisabledButton(1,"ReceiveOral");
	addButton(2,"Anal",burtsWeinerInYourButt);
	if(pc.hasVagina()) addButton(3,"Vagina",burtSticksItInThePCsVajayjay);
	else addDisabledButton(3,"Vagina");
	//[Handjob]	[Rimjob]		[Titfuck]**	[Tailfuck]*
	addButton(4,"Handjob",giveBurtAHandyYouSlooooooooooot);
	addButton(5,"Rimjob",lickBurtsAssholeYouFilthyButtlicker);
	if(pc.biggestTitSize() >= 7) addButton(6,"Titfuck",titFuckBurtBecauseYoureAFilthyFatChestedHo);
	if(pc.hasCuntTail()) addButton(7,"Tailfuck",tailFuckBurtBecauseWhyNot);
	addButton(14,"Back",burtapproach);
...
function BurtShopCollectables():void {
	...
	for(var x:int = 0; x < pc.inventory.length; x++) {
		if(pc.inventory[x].quantity > 0) {
			if(x >= 14) addButton(x+1,pc.inventory[x].shortName + "x" + pc.inventory[x].quantity,burtPurchase,x);
			else addButton(x,pc.inventory[x].shortName + "x" + pc.inventory[x].quantity,burtPurchase,x);
		}
	}
...
function burtMakesAPurchase(arg:int = 0):void {
	...
	//[pg]
	clearMenu();
	addButton(0,"Next",BurtShopCollectables);
}
...
function crabbstBlueRibbonPurchase():void {
	...
	processTime(8);
	clearMenu();
	addButton(0,"Next",burtDrinkMenu);
}
...
function askBurtAboutHimself():void {
	...
	clearMenu();
	addButton(0,"Next",talkToDisBurtGuey);
}
...
function giveBurtABJ():void {
	...
	pc.lust(3);
	clearMenu();
	addButton(0,"Next",giveBurtABJPartII);
}
...
function giveBurtABJPartII():void {
	...
	processTime(10+rand(3));
	clearMenu();
	addButton(0,"Next",randomBurtSexExit);
}
...
function giveBurtAHandyYouSlooooooooooot():void {
	...
	pc.lust(5);
	clearMenu();
	addButton(0,"Next",burtHandyPartDeux);
}
...
function lickBurtsAssholeYouFilthyButtlicker():void {
	...
	output("\n\nBurt moans softly and says,<i> “So it turns out that Zil queens have a chemical on their ovipositors that is designed to make sure that their eggs get the proper nourishment... after my time out there, well... It seems to have become permanent.”</i>");
...
function tailFuckBurtBecauseWhyNot():void {
	...
	output("\n\nYou watch as his scrotum tightens beneath that equine spear, and then you feel it. Pulse after pulse of his hot seed, flowing into your tail’s greedy cuntal throat. Your tail swallows it all down as your orgasm hits you hard, almost knocking you to your knees.");
...
function randomBurtSexExit():void {
	...
	//Three-Dee Rape
		...
		processTime(1);
		clearMenu();
		addButton(0,"Next",threeDSurprise);
	}
}
