...
public function aWildJoggingAnno():void
{
	...
	output("\n\nWell, what will it be? A little exercise couldn't hurt, right? On the other hand, you don't really need it, and watching Anno run would be enjoyable in itself. And with all that talk of laps… maybe you could go back to the ship for a few <i>“laps.”</i>");
	...
	addButton(1,"Watch",watchItJiggle,undefined,"Watch","Watch Anno run.");
	if (pc.hasGenitals() &&!pc.isTaur())
	{
		addButton(2,"Ship",shipSexyFunTimes);
	}
	else
	{
	addDisabledButton(2,"Ship","Ship","You must have a fitting cock and/or cannot be a taur.");
	}
	addButton(3,"Leave",howCouldYouLeaveAnno,undefined,"Leave","Leave Anno to her run.");
}
...
public function runWithAnno():void
{
	...
	if(pc.hasCock())	
		{
			output(" If you're not careful, you'll be too encumbered by your swollen erection to keep up with your puppy girl.");
		}
	...
	if(flags["ANNO_HUSKARRED"] != undefined)
		{
			...
			output(" The sudden shift in position allows her full, fluffy tits to spring free from her ill-fitting tank top. Anno lets out a bark of surprise and clutches at her exposed breasts. You grab two handfuls of Anno's abundant rack and do your best to return it to the reluctant tank top.");
	...
	addButton(1,"Showers",showerSex,undefined,"Showers","Hit the showers with Anno.");
	...
	if (pc.cockThatFits(anno.vaginalCapacity()) != -1 && flags["ANNO_HUSKARRED"] == undefined &&!pc.isTaur())
	{
		addButton(2,"Sexercise",gimmeDatBootySweat,undefined,"Sexercise","You like it sweaty.");
	}
	else if (pc.hasCock()&&flags["ANNO_HUSKARRED"] != undefined&&!pc.isTaur())
	{
		addButton(2,"Sexercise",gimmeDatBootySweat,undefined,"Sexercise","You like it sweaty.");
	}
	else
	{
		addDisabledButton(2,"Sexercise","Sexercise","You must have a fitting cock and/or not be a taur.");
	}
}
...
//Get Squeaky Clean	
public function showerSex():void

{
	...
	output("You follow suit, throwing your things in with hers. You are aware of hungry eyes watching the pair of you disrobe. At least a dozen bulls hastily remove their own clothing, some not even bothering with a locker. Anno latches a biosecurity lock onto the locker containing your gear.");
	...
	output("Anno's steady, confident fingers sweep across your back and scalp, then your [pc.chest], soothing every inch of [pc.skinNoun]. You're not sure, but she seems to take just a little more time rubbing your [pc.butt] than elsewhere. After her stellar performance, it's your turn to wash Anno's succulent body.");
...

{
	...
	{
		output("Her fingers trail down to your [pc.cock], wrapping sensuously around it"); 
		...
		if(pc.hasKnot()) output("As your [pc.cock] becomes fully erect, your [pc.knot] begins to swell, too. Anno mercilessly fondles your knot, giving you an uncomfortable feeling of closeness in your belly and exposure in your unsheathed cock.");
	...
	if (pc.hasCock()) output("[pc.cumColor] precum-lubed");
	...
	if (pc.cockThatFits(anno.vaginalCapacity()))
	{
		output("\n\n<i>“Let's just slide this in here for now, okay?”</i> Anno murmurs sweetly as she slowly mounts your [pc.cock]. Her inner passage parts to accept your cock into its warm embrace. This alone is nearly enough to make your dick erupt. <i>“Make sure you get it nice and clean in there.”</i> Her hips begin to rock against yours, producing a slippery spurting noise from where the two of you are connected."); 
		if(pc.hasKnot()) output("You can feel your knot swell to its full size, trapping your [pc.cock] in her love tunnel.");
	...
	if (pc.cockThatFits(anno.vaginalCapacity()) != -1)
		{
			output("\n\n\You cry out as your [pc.cock] explodes, firing shot after shot into her waiting womb.");
			...
			if(pc.hasKnot()) output("But with your knot sealing her entrance shut, your swimmers are trapped in her with nowhere to go, leaving her insides anything but clean.");
	...
	output("\n\n You share a few more pleasant moments with Anno before returning to the locker room. Soon after, you walk back out into the gym."); 
	pc.orgasm();
	processTime(30);
	clearMenu();
	addButton(0, "Next", mainGameMenu);
...
{	
	...
	output("Anno isn't alone in getting some action. A couple of the bulls who came in after you have gotten busy with a trio of cowgirls, faces and cocks buried in mouths, tits, and puss as they go to town.");
	...
	output("Once inside, it still takes quite a bit of humping and straining for him to fit it deeper down her passage. You're not sure where there's room in there for it, but she somehow manages to fit it in. With her mouth busy, it'd be impossible to ask, but if her moans of pleasure are any indicator, she appears to enjoy being spitroasted.");
...
{	
	...
	author("Twin Flare Tails");
	showBust(annoBustDisplay(true)); 
	output("You tell Anno that you feel clean enough now. The bulls and cows watch with envious eyes as you leave the shower room with Anno. If you were a gambling Steele, you'd wager on there being a few bulls blowing their loads in the shower in short order. After you and Anno have dried and dressed, you leave the locker room."); 
	clearMenu();
	addButton(0, "Next", mainGameMenu);
...
public function gimmeDatBootySweat():void

{
	...
	if(!pc.isNude())
		{
			output("\n\nWith her body for cover, you slip your [pc.cock] out of your [pc.gear], only for it to be wrapped in the confines of Anno's sweaty thighs. You resume your facade of struggling to lift weights as Anno pulls rhythmically, raising herself up your length, then pushing back down it."); 
		}
	else
		{
			output("\n\nAnno traps your [pc.cock] in the slick, sweaty embrace of her thighs. You continue with your weightlifting charade, fighting against the frankly insane amount of weight you set the machine to while Anno works her crotch against yours.");
		}
	if(flags["ANNO_HUSKARRED"] != undefined)
		{
			output("\n\nAs you struggle against the machine and Anno rubs against your erect cock, the inevitable happens. Her too-small exercise shorts are pushed just a little too far and, with an audible ripping noise, split a seam in the crotch. Anno looks down with surprise at her own huskar pussy, glistening with a mixture of sweat and girlcum, its lips wrapped around the base of your [pc.cock]."); 
		}
	else
		{
		output("\n\nAnno lets out a grunt as she bucks her hips into yours, the wetness of your mixed sweat, precum, and girlcum lubricating her thrusts. After several minutes grinding against your bone, the Ausar girl can't seem to take it anymore. Anno stands and slides her shorts and panties off, tosses them to one side, and returns to schlicking your [pc.cock], only now with the lips of her bare pussy."); 
		}
	output("\n\nA moan escapes Anno's lips as her orgasming pussy coats your [pc.cock] in her love juice. You can't help but be aware that some of the nearer patrons of the gym have taken notice, but there's nothing you can really do about it while you're receiving lip service from your puppy's sopping wet cunt. Panting heavily with her tongue out and thrusting lewdly into you with wet slaps, Anno appears to have given up any pretense of engaging in a normal exercise routine. Ah, what the hell! You release your grip on the weight machine and take hold of Anno's tank top. In one motion you lift it over her head ");
	...
	if(flags["ANNO_HUSKARRED"] != undefined)
		{
			...
			if(pc.biggestCockLength() >= 12)output(" With every thrust, your dick is immersed not only in wet snatch, but also in her amazing tits.");
			output(" Your fingers dig into her thick, squeezable body. She's softer and squishier than any sextoy known to man. As tightly as her thighs are squeezing you, they mould into the perfect shape to massage your [pc.cock]. You can feel a building pressure");
			if(pc.balls > 0)output("in your spunk-makers");
			...
			else output(" your [pc.cock] empties your load into Anno's spacious cleavage, marking your huskar lover's [anno.chest] with hot [pc.cum]. ");
			if(pc.hasCuntSnake())output("Your tailpussy swiftly descends on the tip of your cock, catching the last few spurts for its own greedy hunger, then sucking futilely at your shaft as it begs for more."); 
		}
	else
		{
		output("\n\nYour Ausar lover lets out a cry of pleasure as your tongue tickles her salty puppy teat. ");
		if(pc.balls > 0)output(" You feel your [pc.balls] jump a little as Anno wraps her fingers around your [pc.cock] and angles it into herself. With a growing moan, Anno squeezes her own tits while you continue to tease her nipple orally. ");
		if(pc.biggestCockLength() >= 12)output(" Your dick plows Anno's innermost depths ");
		...
		output("You can feel Anno picking up the pace as you near your own climax. Slamming your hips together furiously, you simultaneously reach orgasm, cumming hard. You empty yourself into Anno like it's your last chance at procreation"); 
	...
	output("\n\nAs you both come down from your climaxes, you notice that all eyes are on you. Admittedly, the weight most people came to the gym to pump was not liquid, so it's hardly surprising you attracted this much attention. After taking the time to rearrange your clothes and cleaning up the mess you made with supplies from the janitor's closet, you leave the weight room.");
	if(flags["ANNO_HUSKARRED"] != undefined)output("\n\n<i>“That was a lot of fun,”</i> your Huskar companion says conversationally. <i>“We might have to do it again sometime, after I replace these work out clothes.”</i>");
...
//Watch Anno Jog
public function watchItJiggle():void
{	
	clearOutput();
	showName("ANNO");
	author("Twin Flare Tails");
...
public function shipSexyFunTimes():void
{
		clearOutput();
		author("Twin Flare Tails");
		showName("ANNO");
		...
		if(pc.hasCock()&&pc.hasVagina())	
		{
			...
			if(random == 0)
			{
				...
				if(pc.hasCock())output(" Your [pc.cock] blows its load all over the sheets.");
				output("A hot warmth spreads through your love tunnel. Anno must have chosen a model with synthcum. Behind you, Anno works her way through her own orgasm, her breath sharp and fast."); 
				output("When she has come the rest of the way down, Anno looks back over her shoulder at you. <i>“That was some exercise. I think I may have to try that routine again sometime.”</i> You attempt to pull free and express a similar sentiment, but find your ass glued to hers. <i>“Oh, right. This model is really faithful. It's got the knot and everything. Guess we'll just have to wait it out.”</i> There are worse ways to spend a cool down period. Only it's too bad you're not able to hold Anno in your arms while knotted together like this.");
			}
			else
			{
				output("\n\nAnno takes [pc.oneCock] in her talented mouth. Her tongue massages the underside of your dick, urging it to cum right then and there. Only with great effort do you resist the mating urge you feel growing in your gut. She proceeds to put your prick through every trick in the codex. She sucks heavily on it, floods it in puppy saliva, teases the tip with her tongue…. If every trip to the gym ended with a blowjob like this, you'd go more often.");
		...
		else if (pc.hasCock())
		{
				...
				else
				{
					...
					output(" into your [anno.race] workout partner. As she feels your [pc.cum] filling her, Anno loses it. Her legs squeeze tight around you and she buries her face in your [pc.chest]. The warm wetness of her orgasm soaks sheets already drenched with sweat.");
					output("\n\nYou lie back with Anno after you've both had some time to recover from the experience. She holds you close, breathing in your scent just as you are hers.");
					output("\n\n“Who needs a gym when I've got you? Besides, this way I get free scratches.”");
