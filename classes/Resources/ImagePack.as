package classes.Resources
{
	public class ImagePack
	{
		public static var ImageRequestT:Class = null;
		
		CONFIG::IMAGEPACK {
			//AMBER
			[Embed(source = "../../assets/images/imagepack/amber/jojocite_amber_nude.png", mimeType = "image/png")]
			public static const AmberNakkers:Class;

			//ANNO
			[Embed(source = "../../assets/images/imagepack/anno/CheshireCatSmile37_Anno_Maid.png", mimeType = "image/png")]
			public static const AnnoMaid:Class;

			//BIANCA
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_bentOver.png", mimeType = "image/png")]
			public static const bianca_octomush_bentOver:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_cunnilingus.png", mimeType = "image/png")]
			public static const bianca_octomush_cunnilingus:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_thighSex.png", mimeType = "image/png")]
			public static const bianca_octomush_thighSex:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_working_marked_NOLIMIT.png", mimeType = "image/png")]
			public static const bianca_working_marked_NOLIMIT:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_fellatio_human.png", mimeType = "image/png")]
			public static const Bianca_Human_BJ:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_fellatio_suula.png", mimeType = "image/png")]
			public static const Bianca_Suula_BJ:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_fellatio_cat.png", mimeType = "image/png")]
			public static const Bianca_Feline_BJ:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_fellatio_horse.png", mimeType = "image/png")]
			public static const Bianca_Equine_BJ:Class;
			[Embed(source = "../../assets/images/imagepack/bianca/bianca_octomush_fellatio_dog.png", mimeType = "image/png")]
			public static const Bianca_Canine_BJ:Class;

			//CAMERON SPLAYED
			[Embed(source = "../../assets/images/imagepack/cameron/javkiller_cameron.png", mimeType = "image/png")]
			public static const CameronSplayed:Class;

			//CELISE
			[Embed(source = "../../assets/images/imagepack/celise/ucite_celise.png", mimeType = "image/png")]
			public static const CeliseGreeting:Class;
			[Embed(source = "../../assets/images/imagepack/celise/lazydez_celise_human.jpg", mimeType = "image/png")]
			public static const CeliseFuckHuman:Class;
			[Embed(source = "../../assets/images/imagepack/celise/lazydez_celise_horse.jpg", mimeType = "image/png")]
			public static const CeliseFuckHorse:Class;
			[Embed(source = "../../assets/images/imagepack/celise/lazydez_celise_cat.jpg", mimeType = "image/png")]
			public static const CeliseFuckCat:Class;
			[Embed(source = "../../assets/images/imagepack/celise/lazydez_celise_dog.jpg", mimeType = "image/png")]
			public static const CeliseFuckDog:Class;

			//ERRA
			[Embed(source = "../../assets/images/imagepack/erra/shou_erra_aftercuddle.png", mimeType = "image/png")]
			public static const ErraAfterCuddle:Class;
			[Embed(source = "../../assets/images/imagepack/erra/shou_erra_aftercuddle_panties.png", mimeType = "image/png")]
			public static const ErraAfterCuddlePanties:Class;

			//FERUZE
			[Embed(source = "../../assets/images/imagepack/feruze/dsan_feruze_human_cum.jpg", mimeType = "image/png")]
			public static const feruzePussyFuck:Class;
			[Embed(source = "../../assets/images/imagepack/feruze/dsan_feruze_horse_cum.jpg", mimeType = "image/png")]
			public static const feruzePussyFuckHorse:Class;

			//FLAHNE
			[Embed(source = "../../assets/images/imagepack/flahne/whitedevil_flahne_wallpaper_HQ.png", mimeType = "image/png")]
			public static const UpCloseFlahnePuss:Class;
			[Embed(source = "../../assets/images/imagepack/flahne/Whitedevil_Flahne.jpg", mimeType = "image/png")]
			public static const FlahneRecliningFuta:Class;
			[Embed(source = "../../assets/images/imagepack/flahne/AF_Under_Flahnes_Desk.jpg", mimeType = "image/png")]
			public static const FlahneUnderDesk:Class;
			[Embed(source = "../../assets/images/imagepack/flahne/AF_Under_Flahnes_Desk_Futa.jpg", mimeType = "image/png")]
			public static const FlahneUnderDeskFuta:Class;
			
			//GIANNA
			[Embed(source = "../../assets/images/imagepack/gianna/PZero_Gianna.png", mimeType = "image/png")]
			public static const GiannaCockSuck:Class;
			[Embed(source = "../../assets/images/imagepack/gianna/PZero_Gianna_Superior.png", mimeType = "image/png")]
			public static const GiannaCockSuckHorse:Class;
			[Embed(source = "../../assets/images/imagepack/gianna/waterproof_pidgeon_gianna.jpg", mimeType = "image/png")]
			public static const GiannaHotdog:Class;
			[Embed(source = "../../assets/images/imagepack/gianna/waterproof_pidgeon_gianna_futa_smalldong.jpg", mimeType = "image/png")]
			public static const GiannaHotdogFuta:Class;
			[Embed(source = "../../assets/images/imagepack/gianna/waterproof_pidgeon_gianna_futa_lizard_1.jpg", mimeType = "image/png")]
			public static const GiannaFutaGetsFucked:Class;

			//Jade
			[Embed(source = "../../assets/images/imagepack/jade/rajii_Jade_BG.jpg", mimeType = "image/png")]
			public static const JadeBreastplay:Class;
			[Embed(source = "../../assets/images/imagepack/jade/rajii_Jade_BG_Nips.png", mimeType = "image/png")]
			public static const JadeBreastplayNips:Class;
			[Embed(source = "../../assets/images/imagepack/jade/tktktk_Jade_Shaded.png", mimeType = "image/png")]
			public static const JadeTitfuck:Class;
			[Embed(source = "../../assets/images/imagepack/jade/tktktk_Jade_nipples_Shaded.png", mimeType = "image/png")]
			public static const JadeTitfuckNips:Class;

			//Jumper
			[Embed(source = "../../assets/images/imagepack/jumpers/jumperHumanVaginalMoira.jpg", mimeType = "image/jpg")]
			public static const jumperHumanVaginalMoira:Class;
			[Embed(source = "../../assets/images/imagepack/jumpers/jumperCanineVaginalMoira.jpg", mimeType = "image/jpg")]
			public static const jumperCanineVaginalMoira:Class;
			[Embed(source = "../../assets/images/imagepack/jumpers/jumperFelineVaginalMoira.jpg", mimeType = "image/jpg")]
			public static const jumperFelineVaginalMoira:Class;
			[Embed(source = "../../assets/images/imagepack/jumpers/jumperHorseVaginalMoira.jpg", mimeType = "image/jpg")]
			public static const jumperHorseVaginalMoira:Class;

			//KELLY
			[Embed(source = "../../assets/images/imagepack/kelly/dsan_nenette_kelly_herm_horsecock_laquine.jpg", mimeType = "image/jpg")]
			public static const KellyDP:Class;

			//KIRO
			[Embed(source = "../../assets/images/imagepack/kiro/kiro_and_kally_MQ.jpg", mimeType = "image/jpg")]
			public static const kiroAndKallyTogether:Class;
			[Embed(source = "../../assets/images/imagepack/kiro/Wolfy-nail_kiro_pov_final.jpg", mimeType = "image/jpg")]
			public static const KiroLowPOV:Class;
			[Embed(source = "../../assets/images/imagepack/kiro/DCL_kiro_and_kally_kiss.png", mimeType = "image/png")]
			public static const KiroKallyKiss:Class;
			[Embed(source = "../../assets/images/imagepack/kiro/sukebepanda_kiro_HR.png", mimeType = "image/png")]
			public static const SukebeKiroPoV:Class;
			[Embed(source = "../../assets/images/imagepack/kiro/Nazuu_Kiro_bed_fap_finish.jpg", mimeType = "image/jpg")]
			public static const KiroFapFinish:Class;
			[Embed(source = "../../assets/images/imagepack/kiro/DCLZexon_Kiro_Tamahime_Spunky_Color.png", mimeType = "image/png")]
			public static const KiroDCLCummy:Class;
			[Embed(source = "../../assets/images/imagepack/kiro/nazuu_kiro_milker_finish.jpg", mimeType = "image/jpg")]
			public static const KiroMilkerNazuu:Class;
			[Embed(source = "../../assets/images/imagepack/kiro/dsan_kiro_kally_questycommissioned.jpg", mimeType = "image/jpg")]
			public static const KiroKallyTeamMilk:Class;
			
			//KIROQUEST
			[Embed(source = "../../assets/images/imagepack/kiroquest/dcl_kiroquest_kiro_quest_pos_dolls_motivational_poster_obedience.png", mimeType = "image/png")]
			public static const MindfuckPoster:Class;
			[Embed(source = "../../assets/images/imagepack/kiroquest/mrPink_Adjatha_colors_mindfuck_poster.png", mimeType = "image/png")]
			public static const ObediencePoster:Class;
			
			//KORG DOSED
			[Embed(source = "../../assets/images/imagepack/korg_generic/korg_dosed.png", mimeType = "image/png")]
			public static const KorgDosed:Class;
			[Embed(source = "../../assets/images/imagepack/korg_generic/korg_dosed_dicked.png", mimeType = "image/png")]
			public static const KorgDosedDicked:Class;

			//Lieve
			[Embed(source = "../../assets/images/imagepack/lieve/maniacpaint_lieve.png", mimeType = "image/png")]
			public static const LieveAgainstWall:Class;
			
			//MILLIE
			[Embed(source = "../../assets/images/imagepack/millie/CyanCapsule_Millie_Clothed_BG_MR.jpg", mimeType = "image/jpg")]
			public static const MillieGreeting:Class;
			[Embed(source = "../../assets/images/imagepack/millie/CyanCapsule_Millie_Nude_BG.png", mimeType = "image/png")]
			public static const MillieGreetingNude:Class;
			/*No scene. [Embed(source = "../../assets/images/imagepack/millie/4334wh.png", mimeType = "image/png")]
			public static const MillieCockSuck:Class;*/

			//MIRRIN
			[Embed(source = "../../assets/images/imagepack/mirrin/Rattlepool_SoAndSoCommed_Young_Mirrin.jpg", mimeType = "image/jpg")]
			public static const MirrinYoung:Class;

			//MITZI
			[Embed(source = "../../assets/images/imagepack/mitzi/jojocite_mitzi_ahegao_goblin_cum_tongue_bimbo.jpg", mimeType = "image/png")]
			public static const MitziAhegaoLick:Class;
			/*UNUSED ATM.
			[Embed(source = "../../assets/images/imagepack/mitzi/friendlyalienfriend_mitzi_titfuck.png", mimeType = "image/png")]
			public static const mitziTitfuck:Class; */
			[Embed(source = "../../assets/images/imagepack/mitzi/friendlyalienfriend_mitzi_titfuck_jizzy.png", mimeType = "image/png")]
			public static const mitziTitfuckMessy:Class;

			//OGGY & AMMA
			[Embed(source = "../../assets/images/imagepack/oggy/maniacpaint_oggy_amma_ogram.png", mimeType = "image/png")]
			public static const OggyAmmaBJScene:Class;
			
			//SEER
			[Embed(source = "../../assets/images/imagepack/seer/Aullama_commission_by_Cherrygig.png", mimeType = "image/png")]
			public static const seerLezDom:Class;
			[Embed(source = "../../assets/images/imagepack/seer/Seer_and_Steele_by_Octomush.jpg", mimeType = "image/png")]
			public static const seerTentadom:Class;

			//PAIGE
			[Embed(source = "../../assets/images/imagepack/paige/shou_paige_yoga.png", mimeType = "image/png")]
			public static const PaigeYoga:Class;

			//PENNY
			[Embed(source = "../../assets/images/imagepack/penny/wolfy-nail_penny_autofellatio_final.jpg", mimeType = "image/jpg")]
			public static const PennyGrewDick:Class;
			[Embed(source = "../../assets/images/imagepack/penny/doxy_Penny.jpg", mimeType = "image/jpg")]
			public static const PennyBentOverDesk:Class;
			[Embed(source = "../../assets/images/imagepack/penny/doxy_Penny_dick.jpg", mimeType = "image/jpg")]
			public static const PennyBentOverDeskFuta:Class;
			[Embed(source = "../../assets/images/imagepack/penny/xpr_commission_fenoxo_penny_color_h03.jpg", mimeType = "image/jpg")]
			public static const PennyOnaholeEquine:Class;
			[Embed(source = "../../assets/images/imagepack/penny/xpr_commission_fenoxo_penny_color_f01.jpg", mimeType = "image/jpg")]
			public static const PennyOnaholeSmall:Class;
			[Embed(source = "../../assets/images/imagepack/penny/xpr_commission_fenoxo_penny_color_f02.jpg", mimeType = "image/jpg")]
			public static const PennyOnaholeMedium:Class;
			[Embed(source = "../../assets/images/imagepack/penny/xpr_commission_fenoxo_penny_color_f03.jpg", mimeType = "image/jpg")]
			public static const PennyOnaholeBig:Class;
			[Embed(source = "../../assets/images/imagepack/penny/Adjatha_Penny_Horse_Color_Corrected.jpg", mimeType = "image/jpg")]
			public static const PennyEquineAutofellate:Class;
			[Embed(source = "../../assets/images/imagepack/penny/DCLzexon_Penny_futa_panties.jpg", mimeType = "image/jpg")]
			public static const PennyFutaPantiesCG:Class;

			//PRINCESS
			[Embed(source = "../../assets/images/imagepack/princess/DCLzexon_Princess_Taivris_Fullsize.png", mimeType = "image/png")]
			public static const PrincessIntro:Class;

			//PYROPUP
			[Embed(source = "../../assets/images/imagepack/pyropup/buntcake_pyropup_flamer_corona_lord_final.jpg", mimeType = "image/jpg")]
			public static const PyropupChokeFuck:Class;

			//RATS
			[Embed(source = "../../assets/images/imagepack/rats/octomush_earfuck_ffion.jpg", mimeType = "image/jpg")]
			public static const OctoEarfuck:Class;

			//ROO
			[Embed(source = "../../assets/images/imagepack/roo/octomush_dealer_table_fuck_nomilk.jpg", mimeType = "image/jpg")]
			public static const RooTableFuck:Class;
			[Embed(source = "../../assets/images/imagepack/roo/octomush_dealer_table_fuck.jpg", mimeType = "image/jpg")]
			public static const RooTableFuckMilk:Class;
			[Embed(source = "../../assets/images/imagepack/roo/octomush_dealer_under_table.jpg", mimeType = "image/jpg")]
			public static const RooUnderTableLick:Class;

			//SECOP (PUNK)
			[Embed(source = "../../assets/images/imagepack/secOp/dsan_wall_slut_jumper_thersaadi_laquine_horsecock_1.jpg", mimeType = "image/jpg")]
			public static const VRWallSlut:Class;

			//SHADE
			[Embed(source = "../../assets/images/imagepack/shade/waterproof_pidgeon_shade_sweat.jpg", mimeType = "image/jpg")]
			public static const ShadeTailfuck:Class;

			//SHEKKA
			[Embed(source = "../../assets/images/imagepack/shekka/jamesAB_Shekka_bent_over.png", mimeType = "image/png")]
			public static const ShekkaBentOver:Class;
			[Embed(source = "../../assets/images/imagepack/shekka/jamesAB_Shekka_bent_over_futa.png", mimeType = "image/png")]
			public static const ShekkaBentOverFuta:Class;
			[Embed(source = "../../assets/images/imagepack/shekka/jamesAB_shekka_horsecock_fap.png", mimeType = "image/png")]
			public static const ShekkaHorseOnahole:Class;
	
			//SYRI
			[Embed(source = "../../assets/images/imagepack/syri/rajii_syri_final.jpg", mimeType = "image/jpg")]
			public static const SyriButtfuck:Class;
			[Embed(source = "../../assets/images/imagepack/syri/bashko_syri_lingerie.jpg", mimeType = "image/jpg")]
			public static const SyriLingerie:Class;

			//SLYVEREN SLAVEBREAKER
			[Embed(source = "../../assets/images/imagepack/slavebreaker/mrpink_shoucolor_slyveren_blowjob.png", mimeType = "image/png")]
			public static const SlavebreakerBJ:Class;

			//TERENSHA
			[Embed(source = "../../assets/images/imagepack/terensha/DCLZexon_terensha.png", mimeType = "image/png")]
			public static const TerenshaCumBubble:Class;

			//ULA
			[Embed(source = "../../assets/images/imagepack/ula/sukebepanda_korgonne_princess.png", mimeType = "image/png")]
			public static const UlaDildo:Class;
			[Embed(source = "../../assets/images/imagepack/ula/mrPink_adjColors_ula_war_alpha_sex.png", mimeType = "image/png")]
			public static const UlaWarAlphaSex:Class;

			//VERUSHA
			[Embed(source = "../../assets/images/imagepack/verusha/DCLZexon_Verusha_HQ.png", mimeType = "image/png")]
			public static const VerushaFuck:Class;
			[Embed(source = "../../assets/images/imagepack/verusha/fubMistress_Verusha.png", mimeType = "image/png")]
			public static const VerushaBJ:Class;

			//VKO
			[Embed(source = "../../assets/images/imagepack/vko/Renezuo_VKo_MQ.jpg", mimeType = "image/jpg")]
			public static const VKOBJ:Class;

			//WALL SLUTS
			[Embed(source = "../../assets/images/imagepack/wall_sluts/wall_fanfir.png", mimeType = "image/png")]
			public static const WallFanfir:Class;
			[Embed(source = "../../assets/images/imagepack/wall_sluts/wall_gabilani.png", mimeType = "image/png")]
			public static const WallGabilani:Class;
			[Embed(source = "../../assets/images/imagepack/wall_sluts/wall_leithan.png", mimeType = "image/png")]
			public static const WallLeithan:Class;
			[Embed(source = "../../assets/images/imagepack/wall_sluts/wall_raskvel.png", mimeType = "image/png")]
			public static const WallRaskvel:Class;
			[Embed(source = "../../assets/images/imagepack/wall_sluts/wall_tura.png", mimeType = "image/png")]
			public static const WallTura:Class;

			//ZIL FEMALE
			[Embed(source = "../../assets/images/imagepack/zil_female/aldergames_zilfemB_final_01.png", mimeType = "image/png")]
			public static const ZilTongueOut:Class;
		}
	}
}