	...
	public class JaneriaCore extends Creature
	{
		...
		private function janeriaCoreBeginGrapple(target:Creature):void
		{
			...
			else
			{
				...
				var damage:TypeCollection = new TypeCollection( { electric: 25 + bon });
				damageRand(damage, 15);
				applyDamage(damage, this, target, "minimal");
				target.energy(-5);
		...
		private function janeriaCoreSustainGrapple(target:Creature):void
		{
			...
			var damage:TypeCollection = new TypeCollection( { electric: 35 + bon });
			damageRand(damage, 15);
			applyDamage(damage, this, target, "minimal");
			target.energy(-5);
		}
		...
		private function janeriaCoreDensityShift(target:Creature):void
		{
			...
			if (rn >= 95)
			{
				...
				damage = new TypeCollection( { freezing: 40 + (bon * 2) });
				damageRand(damage, 15);
				applyDamage(damage, this, target, "minimal");
			...
			else
			{
				...
				damage = new TypeCollection( { freezing: 20 + bon });
				damageRand(damage, 15);
				applyDamage(damage, this, target, "minimal");
		...
		private function janeriaCoreTelepathicOverwhelm(target:Creature):void
		{
			...
			if (rn >= 95)
			{
				...
				damage = new TypeCollection( { psionic: 40 + (bon * 2) });
				damageRand(damage, 15);
				applyDamage(damage, this, target, "minimal");
				target.energy(-10);			
			}
			...
			else
			{
				...
				damage = new TypeCollection( { psionic: 20 + bon }, DamageFlag.PSIONIC);
				damageRand(damage, 15);
				applyDamage(damage, this, target, "minimal");	
				target.energy(-5);			
			}
		...
		private function janeriaCoreElectricFlood(target:Creature):void
		{
			...
			if (target.hasStatusEffect("Generator Off"))
			{
				...
				//damage steele
				damage = new TypeCollection( { electric: 80 + bon });
				damageRand(damage, 15);
				applyDamage(damage, this, target, "minimal");
				...
				//damage janeria
				damage = new TypeCollection( { unresistablehp: 50 });
				damageRand(damage, 15);
