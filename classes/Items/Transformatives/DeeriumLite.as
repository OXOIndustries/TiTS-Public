//Deerium Lite item by VoidLord. Heavily based on Taurico Venidae item, with added eye/face/penis TFs and no chance of taur TF.
//Sold by Jade on Tavros
//Added new deer eye/face/penis parts to appearance.as and added them to the deer-morph racial score calculation in Creature.as

/* 
 * Author's notes:
 * All other text is already included in-game, so I'm not going to bother copying it over. The only new content is the face and eyes since I wanted to include a full furry version as a rare/final transformation.
 * I also added a humanoid deer face for people who don't want the muzzled one.
 * Deer-cock now added. (Could be added to the Taurico version?)
*/
...
package classes.Items.Transformatives 
{
	...
	public class DeeriumLite extends ItemSlotClass
	{
		
		public function DeeriumLite() 
		{
			...
			this.description = "a medipen labeled “Deerium Lite”";
		...
		override public function useFunction(target:Creature, usingCreature:Creature = null):Boolean
		{
			...
			if((target.eyeType != GLOBAL.TYPE_DEER) && ((target.eyeColor != "brown") && (target.eyeColor != "amber"))) TFs.push(deerEyeTF);
			if(((target.faceType != GLOBAL.TYPE_HUMAN) && target.faceType != GLOBAL.TYPE_DEER) && rand(4) == 0 && !target.hasFaceFlag(GLOBAL.FLAG_MUZZLED)) TFs.push(humanFaceTeef);
			if(((target.faceType == GLOBAL.TYPE_HUMAN) || (target.hasFaceFlag(GLOBAL.FLAG_MUZZLED) && target.faceType != GLOBAL.TYPE_DEER))) TFs.push(deerFaceTF);
			if((target.faceType == GLOBAL.TYPE_DEER) && (!target.hasFaceFlag(GLOBAL.FLAG_MUZZLED))) TFs.push(deerFaceTF);
			...
			if((target.hasLegFlag(GLOBAL.FLAG_FURRED) && ((target.legType != GLOBAL.TYPE_DEER && target.furColor != "brown") || (target.legType == GLOBAL.TYPE_DEER && target.furColor != "white-dappled brown"))) && rand(4) == 0) TFs.push(skinToFur);
			if(target.legType != GLOBAL.TYPE_DEER || target.isTaur() || target.legCount != 2) TFs.push(loseYouACentaurBod);
			if(hasTFableDix(target)) TFs.push(growADeerCawk);
			if(needDeerCunt(target)) TFs.push(vaginaDeerTF);
			if(target.legType == GLOBAL.TYPE_DEER && target.armType != GLOBAL.TYPE_EQUINE && rand(4) == 0) TFs.push(deerArmsTF);
			if(target.hasCock() && (target.horns == 0 || target.hornType != GLOBAL.TYPE_DEER)) TFs.push(hornTFForDeertaur);
			if(target.hasCock() && target.horns >= 0 && target.hornType == GLOBAL.TYPE_DEER && target.hornLength < 9) TFs.push(deerHornLengthExtension);
		...
		//Deer face
		/*
		 * Deer face comes in two types: human-like flat face and furry muzzled face.
		 * A single TF function covers both.
		 * Any other face first turns into human face, then human-like deer face, before finally becoming furry muzzled deer face.
		 * I allowed a muzzled, non-deer face to turn directly into a muzzled deer face because it seemed odd to me that it would have to turn human first.
		 * This may or may not be a terrible way to program this.
		 */
		
		public function deerFaceTF(target:Creature):void
		{
			if ((target.faceTypeUnlocked(GLOBAL.TYPE_DEER)) && ((target.faceType == GLOBAL.TYPE_DEER) || (target.hasFaceFlag(GLOBAL.FLAG_MUZZLED))))
			{
			...
			else if ((target.faceTypeUnlocked(GLOBAL.TYPE_DEER)) && !target.hasFaceFlag(GLOBAL.FLAG_MUZZLED))
			{
		...
		public function deerEyeTF(target:Creature):void
		{
			if (target.eyeTypeUnlocked(GLOBAL.TYPE_DEER))
			{
				...
				{
					output("top set of eyes shrink into your head, while your primary ");
				}
				...
				var deerEyeColors:/*String*/Array = ["brown", "amber"];
					if (!InCollection(target.eyeColor, deerEyeColors))
					{
						var color:String = RandomInCollection(deerEyeColors);
		...
		//Tinkered with the dialogue to account for merging multiple tails
		public function deerTailTF(target:Creature):void
		{
			if(target.tailTypeUnlocked(GLOBAL.TYPE_DEER))
			{
				...
				else if(target.tailCount > 1) 
				{
					...
					output("becoming softer, and merging together at the base. By the time they settle into their new form, you realize that <b>you’ve got yourself a single short fluffy tail - like a deer’s!</b>");
				}
		...
		//Change legtype to deer. Lose centaur body. Change to two legs. 
		public function loseYouACentaurBod(target:Creature):void
		{
			if(target.legCountUnlocked(2) && target.legTypeUnlocked(GLOBAL.TYPE_DEER))
			{
				//if no deer legs: 
				if(target.legType != GLOBAL.TYPE_DEER)
				{
				...
				if(target.isTaur())
				{
					...
					if(target.legCount < 2) output("larger");
					else if(target.legCount > 2) output("smaller");
					output(" by the second... and more bipedal in shape. All you can do is endure it, trying to get used to the stretching feeling that’s gripping your entire form. Slowly but surely, your rear end shifts into the shape of a deer’s backside, covering itself in fur.");
					if(target.legCount < 2) output(" A new leg begins to bud out of your changing body, growing down to support your frame. You’ve got two of them now, just enough to be able to carry your lightly-built, powerful equine frame.");
					output(" After a few minutes, you’re able to scrabble onto your animalistic legs and give your body a bit of a test. You quickly end up bounding about, just like a nimble " + target.mf("buck","doe") + ". <b>You’ve definitely got a bipedal, deer-like body now</b>, complete with fuzzy fur and agile shape.");
				}
				//extra check for non-taurs that have some non-two number of legs
				if(!target.isTaur())
				{
					output("\n\nSuddenly, your whole body is assailed by a sensation of stretching. Your body contorts and strains, growing ");
					if(target.legCount < 2) output("larger");
					else if(target.legCount > 2) output("smaller");
					output(" by the second. All you can do is endure it, trying to get used to the stretching feeling that’s gripping your entire form. Slowly but surely, your rear end shifts into the shape of a deer’s backside, covering itself in fur.");
					if(target.legCount < 2) output(" A new leg begins to bud out of your changing body, growing down to support your frame. You’ve got two of them now, just enough to be able to carry your lightly-built, powerful equine frame.");
					output(" After a few minutes, you’re able to scrabble onto your animalistic legs and give your body a bit of a test. You quickly end up bounding about, just like a nimble " + target.mf("buck","doe") + ". <b>You’ve definitely got a bipedal, deer-like body now</b>, complete with fuzzy fur and agile shape.");
				}
				//if not-tauric already: 
				//Removed this particular line since I couldn't come up with a way to make it read non-redundantly.
				//Probably don't need text specifically for changing from two-legs to another two-legs; the base leg TF should cover it well enough.
				//else
				//{
				//	output("\n\nYour already bipedal body itches and crawls, beginning to change as a result of the transformative you took. Your skin furs over, outcrops of brown fur popping up across your body. Your legs thin slightly, hooves on the end of your feet. The transformation is slow, but fills you with a subtle pleasure the whole time. <b>Your legs are now those of a deer!</b>");
				//}
		...
		//Arms TF
		//Changes arms to equine iff already deer legs
		//TF text copied from HorsePill since the document didn't include specific text for it
		public function deerArmsTF(target:Creature):void
		{
		...
		//Cock TF
		//Changes cocktype to deer. Increases in size, up to 12 inches long.
		//Mostly copied from TauricoVenidae version. Changed to deer cock type since this project adds that.
		//Decreased growth length due to non-taur.
		//Massive Lust spike. 
		public function growADeerCawk(pc:Creature):void
		{
			if(pc.cockTypeUnlocked(0, GLOBAL.TYPE_DEER))
			{
				...
				{
					if(pc.cocks[x].cType != GLOBAL.TYPE_DEER) eligible.push(x);
					else if(pc.cocks[x].cLength() < maxGrowth) eligible.push(x);
				}
				var y:int = eligible[rand(eligible.length)];

				//if not deer already:
				if(pc.cocks[y].cType != GLOBAL.TYPE_DEER)
				{
					output(" It begins to shift form, forming a ring around the middle and thickening at the head - perfect for stretching a cute deer-morph wiiide open so you can slide the rest of your considerable length in after it. You groan in satisfaction as it finishes thickening, settling into place as a <b>thick, flared deer cock!</b> Vesperians must believe in walking softly and carrying a big stick.");
					pc.lust(40);
					pc.shiftCock(y,GLOBAL.TYPE_DEER);
					pc.libido(2);
					if(pc.cocks[y].cockColor != "pink")
						{
						pc.cocks[y].cockColor = ("pink");
						}
				}
				//if less than 12" long: 
				else if(pc.cocks[y].cLength() < maxGrowth)
				{
					//Megagrow - one shot four-inch growth
		...
		public function vaginaDeerTF(pc:Creature):void
		{
			if(pc.vaginaTypeUnlocked(0, GLOBAL.TYPE_DEER))
			{
				...
				for(var x:int = 0; x < pc.totalVaginas(); x++)
				{
					...
					if(pc.vaginas[x].vaginaColor != "black")
						{
						pc.vaginas[x].vaginaColor = ("black");
						}
				}
				pc.orgasm();
				pc.libido(2);
		...
		//Height Change
		//Document doesn't have a listing for height, so this function shouldn't be called
		...
		//Increase horn length to 9 inches if below
		//This function is only called if the PC already has deer horns, and those horns are under 9 inches.
		public function deerHornLengthExtension(pc:Creature):void
		{
			var numLength:int = 9;
			if (pc.hornLengthUnlocked(numLength))
			{
				pc.hornLength = numLength;
			}
		}
