	...
	public class Reptilum extends ItemSlotClass
	{
		...
		override public function useFunction(target:Creature, usingCreature:Creature = null):Boolean
		{
			...
			if (target.hairLength > 0)
			{
				...
				{
					output("\n\nYou notice something [pc.hairColor] drifting down past your eyes. As you reach up to snag it, you recognize it as part of your [pc.hair]. Lifting your hands up, you feel your [pc.hair] coming loose at the slightest touch. Before you know it, your entire head is barren, your [pc.hair] decorating the floor around you. <b>You are now bald.</b>");
			...
			if (target.armType != GLOBAL.TYPE_LIZAN)
			{
				...
				{
					output("\n\nYour arms begin to feel very odd. Everything seems to be alternating between irritation and numbness, the sharp contrast causing greater discomfort than either sensation alone.");
					if (!target.hasScales() && !target.hasArmFlag(GLOBAL.FLAG_SCALED)) output(" You idly begin to scratch some particularly itchy spots, only to unwittingly tear away patches of [pc.skinFurScales]. You cautiously remove portions of molted [pc.skinFurScales], revealing the scales that now cover your arms.");
			...
			if (target.isTaur())
			{
				...
				{
					output("\n\nYou feel woozy as the entire room seems to rock around you. You sit (as best you can) and try and wait out the vertigo, only to feel your additional [pc.legs] go numb. As you try to investigate the source of this sensation, your tauric rear suddenly falls clean off,");
					if (kGAMECLASS.silly) output(" before " + (target.legCount <= 4 ? "stumbling" : "galloping") + " off into the sunset.");
					else output(" bloodlessly laying on the floor.");
					output(" It takes a moment to regain your balance, as your readjust to your new, bipedal stance.");
					if (!kGAMECLASS.silly && target.hasTail()) output(" As you dispose of your former extraneous body parts, you notice that your " + (target.hasTails() ? "tails are" : "tail is") + " still attached to you, rather than the hunk of meat you’re removing.");
			...
			if (target.tongueType != GLOBAL.TYPE_OVIR)
			{
				...
				{
					output("\n\nYou focus on breathing deeply, attempting to keep a level head as the transformative does its work. You keep calm as the insides of your mouth begin to violently itch, but quickly realize that the itching comes and goes with your breath. You try and hold your breath, breathing through the nose until the discomfort dissipates. Once it does, you bring up your Codex’s camera to inspect the changes, only to immediately retch the moment you stick out your tongue. <b>Your tongue is thinner and slightly longer than it used to be, and your sense of taste has sharpened to the point where it can taste the air itself.</b> That’ll take some getting used to...");
			...
			else if (!target.hasTongueFlag(GLOBAL.FLAG_LONG) || !target.hasTongueFlag(GLOBAL.FLAG_PREHENSILE))
			{
			...
			if (target.legCount >= 2 && target.tailType != GLOBAL.TYPE_LIZAN)
			{
				...
				else validTFs.push(function (target:Creature):void
				{
					if (!target.hasTail())
					{
						...
						output(". The throbbing continues as your tailbone grows outward, a new limb growing around the skeletal extension. A few minutes later, this new tail is several feet long, " + target.scaleColor + " scales quickly growing ");
					...
					{
						output("\n\nThough the transformation has dissipated, you still feel a faint tingling from the tip of your tail. Probing the tip, you find your tail genitalia is still present (albeit more lizardlike than it used to be).");
						...
						else output(" Your tailcunt remains at the tip of your tail, though it has become every bit as reptilian as the limb it adorns. It remains clenched shut, for now, but with a thought or a urge you can make it open it’s lips, revealing this cum-parched twat to the world.");
					}
					
					output("\n\n" + (target.isAssExposed() ? "Y" : "After you re-dress, y") + "ou move around a bit, testing your balance. Your center of weight as shifted,");
					...
					if (target.tailCount == 0) target.tailCount = 1;
			...
			if (availableCocks.length > 0) 
			{
				...
				validTFs.push(function (target:Creature):void
				{
					...
					if (!target.isCrotchExposed() && kGAMECLASS.flags["REPTILUM_MOVED_CLOTHES"] == undefined) output(" You remove your [pc.crotchCovers], your attention fully focused on your throbbing [pc.cockNoun " + cockIdx + "].");
					output(" Your prick tapers out, it’s color shifting and darkening until it settles on a deep purple. Bulbous lumps form along your cock’s length,");
					...
					output(" [pc.Cum] leaks from the tip like a faucet as waves of arousal flow without any sign of release. The changes finally recede, leaving your prick throbbing for attention. <b>Your dick is now reptilian!</b>");
					output("\n\n");
			...
			if (target.balls > 0 && !target.hasACockFlag(GLOBAL.FLAG_SHEATHED) && target.ballSize() >= 1 && !target.hasStatusEffect("Uniball"))
			{
			...
			if (target.hasCock(GLOBAL.TYPE_NAGA) && !target.hasCocks() && target.hasStatusEffect("Genital Slit"))
			{
				...
				{
					output("\n\nA sudden, hot throb in your slit is the only you get before you feel the start of " + (kGAMECLASS.flags["REPTILUM_CHANGES"] == undefined ? "a" : "another") + " change. It feels like your [pc.cock] is getting thicker inside you, fattening up so nicely that you wonder just how much dick you can bear, even soft and unaroused. That question is answered by the momentary pain of something separating inside you. You can feel your dick being rubbed from above and below at the same time, both places feeling as phallic as the source of your sensation. It’s then that <b>you pull open your slit to reveal a newly grown pair of reptilian dicks where once there was one.</b>");
			...
			if (target.hasCock() && (target.balls == 0 || target.ballSize() <= 1) && !target.hasACockFlag(GLOBAL.FLAG_SHEATHED) && !target.hasStatusEffect("Genital Slit"))
			{
				if (target.ballsUnlocked(0)) validTFs.push(function (target:Creature):void
				{
					output("\n\nYour crotch is suddenly assailed discomfort, pulsing waves of queasiness flavored by a hint of arousal.");
					if (InPublicSpace(target) && kGAMECLASS.flags["REPTILUM_DUCKED"] == undefined) output(" You quickly duck into a secluded spot so you can inspect the changes as they happen.");
					if (!target.isCrotchExposed() && kGAMECLASS.flags["REPTILUM_MOVED_CLOTHES"] == undefined)
					{
						...
						output(", your attention fully focused on your throbbing [pc.cock].");
					}
					...
					output(" squirm out of your grasp, sinking into your groin before your eyes. Panicked, you rub the spot where your cock vanished, finding a new opening");
					if (target.hasFur()) output(" hidden beneath your fur");
					else if (target.hasScales()) output(" concealed between your scales");
					else output(" forming where your cock used to be");
					output(". Rubbing the new orifice, your ministrations are soon rewarded as your [pc.cock] peeks out, dripping with [pc.cum]. <b>You seem to have gained a new gital slit</b>");
			...
			if (target.breastRows.length > 0 && target.biggestTitSize() > (target.isMasculine() ? 0 : 2) && !target.isLactating()) 
			{
			...
			if (target.hipRating() < 10 && (target.isFeminine() || target.isPregnant()))
			{
				...
				{
					output("\n\nPain spikes through your waist as your pelvis shifts slightly. You adjust your clothing, noting that <b>Your [pc.hips] are now wider.</b>");
			...
			{
				output("\n\nPain spikes through your [pc.nipples], only to fade a few moments later. " + (target.isChestExposed() ? "You inspect your [pc.breasts]," : "You peek inside your [pc.chestCover],") + " finding that your nipples have become pinched holes. Rubbing the changed area a bit, you find that your nipples pop back out as arousal flows through them. <b>Your nipples are now inverted!</b>");
