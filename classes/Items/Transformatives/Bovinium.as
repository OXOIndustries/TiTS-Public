	...
	public class Bovinium extends ItemSlotClass
	{
		...
		private function t1Changes(target:Creature, tChanges:int):int
		{
			...
			if (target.hasCock())
			{
				...
				if (tCocks.length > 0)
				{
					...
					output("\n\nA bolt of... not quite pain, but certainly not pleasure races up your spine! You clutch your crotch, the source of your discomfort,");
		...
		private function t2Changes(target:Creature, tChanges:int):int
		{
			...
			if (target.breastRows[0].breastRatingRaw < 40)
			{
				...
				output("\n\nYou feel a sudden rush of pressure onto your [pc.chest]. You grunt and find yourself making a bestial mooing sound as your chest expands");
			...
			else if (target.milkMultiplier < 150) // SWINGING IN THE DARK HERE
			{
				...
				output("\n\nYou feel a shift of weight in your [pc.chest], and you get a distinctly more... full... feeling from your breasts. You give your teat an experimental squeeze, and are rewarded with a much more vigorous jet of [pc.milkNoun] that spurts out at your touch. You’re pretty sure that <b>you’re lactating even more now!</b>");
			}
		...
		private function t3Changes(target:Creature, tChanges:int):int
		{
			...
			if (target.hipRatingRaw < 17 && rand(3) == 0 && changes < tChanges)
			{
				...
				output("\n\nYou wobble unsteadily, feeling your body shifting and expanding");
			...
			{
				output("\n\nWith a gasp, you feel weight shifting around in your body, guided by the strange, transformative treat. Shivers of sensation explode from your backside, and your hands instinctively move to your rump -- and you find that it’s growing! After a few moments, you find that your butt is bigger and jigglier than it was before! You spend a few moments bouncing around, enjoying the feeling of your new posterior.");
			...
			if (rand(3) == 0 && changes < tChanges && (target.tailType == 0 || target.tailCount == 0))
			{
				...
				{
					output("\n\nYou feel a shifting of flesh behind you. Something’s growing, sprouting from your backside. You look over your shoulder, watching as a mass of flesh twists and grows from just above your [pc.butt]. After a few moments, the mass of flesh resolves into a distinctly tail-like shape with a tuft of hair at its tip. <b>You now have a bovine tail!</b>");
			...
			if (rand(6) == 0 && changes < tChanges && target.tone > 0)
			{
				...
				changes++;
				output("\n\nYour muscles become a little less obvious as your body transforms, hiding deep in your body. While you’re no more or less physically fit now, you’re less visibly toned than you were before -- much softer and fleshier.");
			}

			//Low chance to get quad nipples
			if (rand(20) == 0 && changes < tChanges && target.nipplesPerBreast != 4)
			{
				output("\n\nYour [pc.nipples] ache all of a sudden! Pain courses through them, a tearing sensation that makes you yelp. When it abates, your hands instinctively rub at your nipples... and you find yourself feeling more nipples than there ought to be. A quick count confirms: <b>you now have quad nipples!</b>");
			...
			if (rand(6) == 0 && target.nippleLengthRatio < 2)
			{
				...
				output("\n\nYou feel your nipples stretching, not quite painfully. A little [pc.milk] squirts out of the tips, worked out by the strange movements in your breasts. When they settle down, you realize that <b>your teats are larger now!</b>");
		...
		private function t4Changes(target:Creature, tChanges:int):int
		{
			...
			if (rand(3) == 0 && changes < tChanges && target.hasVagina() && target.fertilityRaw < 2)
			{
				...
				output("\n\nYour Codex beeps to alert you that your fertility rating has increased, and delivers a short warning against having unprotected sex. You’re much more likely to conceive now, even with alien creatures!");
			}
