package classes.Items.Transformatives
{
	...
	public class Goblinola extends ItemSlotClass
	{
		...
		private function minorGoblinMutations(target:Creature):void
		{
			...
			if((target.rawmf("m", "f") == "m" && femininity > 30) || (target.rawmf("m", "f") == "f" && femininity < 70))
			{
			...
			{	
				...
				if(select == 0)
				{
					eventBuffer += "Your stomach is still somewhat uneasy from the health bar.";
				}
				...
				else if(select == 1)
				{
					...
					if(target.skinToneUnlocked(newSkinTone))
					{
						...
						if(target.skinType == GLOBAL.SKIN_TYPE_SKIN) eventBuffer += "You notice something odd about your skin and find, much to your surprise, that your flesh tone has changed! <b>You now have [pc.skinColor] colored skin.</b>";
						// Transformation text (non-human texture):
						else eventBuffer += "You touch your face in a moment of idle contemplation, and find that it feels very different than usual. You check your reflection in and see that not only has your skin’s texture changed to something much more in line with what a human’s normally is, it’s also turned an unusual [pc.skinColor] color. <b>You now have [pc.skinColor] human skin!</b>";
					}
					else
					{
						eventBuffer += target.skinToneLockedMessage();
					}
				}
				//#2 Goblin ears: Ears become elven/goblin ears (long and pointed).
				else if(select == 2)
				{
					if(earTypeUnlocked(GLOBAL.TYPE_GABILANI))
					{
						// Transformation text:
						eventBuffer += "Your ears have been bothering you for a bit now. You give them a another scratch a discover that they’ve reshaped themselves! <b>You now have long pointy gabilani ears!</b>";
						target.earType = GLOBAL.TYPE_GABILANI;
					}
					else
					{
						eventBuffer += target.earTypeLockedMessage();
					}
				}
				...
				else if(select == 3)
				{
					...
					{
						eventBuffer += "Your scalp has been feeling very itchy for the last hour or so. It hasn’t been bad enough to really mess with your concentration,";
						target.hairColor = newHairColor;
						// Transformation text (have hair):
						if(hairLength > 0) eventBuffer += " but now a few strands come away in your hands to reveal that your hair color has changed. <b>You’re now sporting [pc.hairColor] hair!</b>";
						else eventBuffer += " but you’re sure something about your head has definitely changed...";
					}
					else
					{
						eventBuffer += target.hairColorLockedMessage();
					}
				}
				...
				else if(select == 5)
				{
					if(target.faceTypeUnlocked(GLOBAL.TYPE_GABILANI))
					{
						...
						{
							eventBuffer += "You double over as a sudden wave of pain passes through your [pc.face]. It feels like the bones are grinding against one another! Finally, the pain becomes dull enough that you can keep moving, but it’s still there.";
							// [increase PC fatigue]
							target.energy(-50);
					...
					{
						eventBuffer += target.faceTypeLockedMessage();
					}
				...
				else if(select == 6)
				{
					...
					if(target.tallnessUnlocked(newTallness))
					{
						...
						if(rand(3) == 0) eventBuffer += "You’ve been feeling a bit stiff all over for the last half hour or so. Finally, the feeling passes and you give relaxed sigh, only to hear your codex give a warning beep. When you check it, you discover that it’s detected a height change in you. <b>Apparently you’ve shrunk slightly in height!</b>";
						// Transformation text (variation 2):
						else if(rand(2) == 0) eventBuffer += "Your clothes feel unusually loose on you, and you hear a warning beep come from your codex. Apparently, over the last hour or so, <b>you’ve shrunk slightly in height!</b>";
						// Transformation text (variation 3):
						else eventBuffer += "A groan escapes your lips as you give your [pc.legOrLegs] a rub. They’ve been feeling a bit stiff, and you stretch them out. There is a beep from your codex as it informs you that your recorded height is inaccurate with your actual stature. <b>You’ve shrunk a little over the last hour.</b>";
						
						target.tallness = newTallness;
					}
					else
					{
						eventBuffer += target.tallnessLockedMessage();
					}
				}
				...
				else if(select == 7)
				{
					...
					if(target.femininityUnlocked(newFem))
					{
						...
						if(rand(2) == 0)
						{
							eventBuffer += "You feel a strange tingling in your face, and gently touch it. It feels";
							if(nChange > 0) eventBuffer += " softer, smoother, and more feminine";
							else if(nChange < 0) eventBuffer += " harder, rougher, and more masculine";
							else eventBuffer += " different at first, but it quickly passes";
							eventBuffer += ".";
						}
						// Transformation text (variation 2):
						else
						{
							eventBuffer += "An odd feeling comes over you. Thinking you know what it is, you take a look at yourself with the codex.";
							if(nChange > 0) eventBuffer += " Your suspicions are confirmed, your face has taken on more feminine characteristics.";
							else if(nChange < 0) eventBuffer += " Your suspicions are confirmed, your face has taken on more masculine characteristics.";
							else eventBuffer += " Nothing really changed at all.";
						}
					}
					else
					{
						eventBuffer += target.femininityLockedMessage();
					}
				}
				...
				else if(select == 8)
				{
					...
					{
						eventBuffer += target.legCountLockedMessage();
					}
					else
					{
						eventBuffer += target.legTypeLockedMessage();
					}
				}
				//#9 Change arms to human: Arms change to human arms. Advances time by 15 minutes when it triggers.
				else if(select == 9)
				{
					if(armTypeUnlocked(GLOBAL.TYPE_HUMAN))
					{
						// Transformation text:
						eventBuffer += "The flesh in your [pc.arms] feels weak and palpitant, and as the sensation increases, you find yourself unable to do anything with them.";
						if(target.hasFur()) eventBuffer += " Fur sloughs off them in big, uneven chunks.";
						eventBuffer += " It’s with some relief when after about fifteen minutes the transformation completes, and you are able to move your now human arms around and admire them in all their lightly haired, elbow-y glory.";
					...
					{
						eventBuffer += target.armTypeLockedMessage();
					}
				}
				...
				else if(select == 10)
				{
					if(target.tailTypeUnlocked(0) && target.tailCountUnlocked(0))
					{
						...
						// Transformation text:
						eventBuffer += "For the last little while you feel like you’re missing something. After checking your pockets and possessions, you suddenly become aware that you’ve managed to lose your tail";
						if(target.tailCount != 1) eventBuffer += "s";
						eventBuffer += " somewhere! <b>You no longer have";
						if(target.tailCount == 1) eventBuffer += " a tail";
						else eventBuffer += " any tails";
						eventBuffer += ".</b>";
					...
					{
						eventBuffer += target.tailCountLockedMessage();
					}
					else
					{
						eventBuffer += target.tailTypeLockedMessage();
					}
				}
				totalTFs--;
			}
		...
		private function majorGoblinMutations(target:Creature):void
		{
			...
			while(TFList.length > 0 && totalTFs > 0)
			{
				...
				if(select == 0)
				{
					eventBuffer += "Your stomach gives an audible gurgle. It is still a bit unsettled by the health bar.";
				}
				...
				else if(select == 1)
				{
					...
					if(target.cockTypeUnlocked(n, GLOBAL.TYPE_GABILANI))
					{
						...
						if(target.hasLowerGarment()) eventBuffer += "Your [pc.cock + " n " +] tingles, rubbing against your [pc.lowerUndergarment] somewhat uncomfortably. Finally you check what’s going on downstairs and discover that your member has actually reformed into an alien cock!";
						// Transformation text (PC has no undergarments, but does lower garments):
						else if(target.isCrotchGarbed()) eventBuffer += "Your [pc.cock " + n + "] tingles, rubbing against your [pc.lowerGarments] somewhat uncomfortably. Finally you check what’s going on downstairs and discover that your member has actually reformed into an alien cock!";
						// Transformation text (PC has no undergarments, or lower garments):
						else eventBuffer += "Your [pc.cock " + n + "] tingles, feeling somewhat uncomfortable as the air moves around it. Finally you check what’s going on downstairs and discover that your member has actually reformed into an alien cock!";
						eventBuffer += " Its tip is now a pair of spherical crowns separated with a deep ridge with an otherwise relatively human shaft linking it to your crotch. <b>You now have a gabilani cock!</b>";
					...
					{
						eventBuffer += target.cockTypeLockedMessage();
					}
				}
				...
				else if(select == 2)
				{
					...
					{
						// Transformation text 
						eventBuffer += "A strange feeling in your nethers overtakes you, eliciting an unexpected moan of pleasure from your lips. After basking in the sensation for a few moments, you give an experimental pull and discover that a whole new set of muscles have grown in and around";
						// If PC has multiple vaginas:
						if(target.vaginas.length > 0) eventBuffer += " one of";
						eventBuffer += " your female sex";
						if(target.vaginas.length > 0) eventBuffer += "es";
						eventBuffer += " which you can control with great ease.";
						// (non-human vagina)
						if(target.vaginas[n].type != GLOBAL.TYPE_HUMAN) eventBuffer += " After checking its appearance, you find that from the outside it looks pretty close to a human’s.";
						eventBuffer += " <b>You now have a";
						if(target.totalVaginas(GLOBAL.TYPE_GABILANI) > 0) eventBuffer += "nother";
						eventBuffer += " gabilani vagina!</b>";
					...
					{
						eventBuffer += target.vaginaTypeLockedMessage();
					}
				}
				...
				else if(select == 3)
				{
					...
					{
						// Transformation text:
						eventBuffer += "You grunt in surprise";
						//Not exhibitionist:
						if(target.exhibitionism() < 50) eventBuffer += " and slight embarrassment";
						eventBuffer += " as [pc.eachCock] swells up and oozes pre in a burst of hard heat. You cautiously touch and examine it with your fingers. It’s warm and smells musky as ever, but the color and texture of it has changed to a semi-transparent oil. You snort as you imagine attempting to massage someone with it. <b>You now ejaculate gabilani cum.</b>";
						
						target.cumType = GLOBAL.FLUID_TYPE_GABILANI_CUM;
					}
					else
					{
						eventBuffer += target.cumTypeLockedMessage();
					}
				}
				...
				else if(select == 4)
				{
					...
					{
						eventBuffer += target.girlCumTypeLockedMessage();
					}
				}
				//#5 Intelligence increase towards 85%
				else if(select == 5)
				...
				{
					eventBuffer += "Your movements feel jerkier, as if your joints were acting up. As your perception adapts, though, you realize that in actuality you are now reacting and moving slightly faster.";
				...
				{
					// > 60% Phy:
					if(target.PQ() >= 60) eventBuffer += "The muscles in your arms and [pc.lowerBody] feel like they’re getting softer and more supple as time goes on, and you’re left with the uneasy impression you’re not as physically capable as you once were. What kind of health food was that, anyway?";
					// < 60% Phy:
					else eventBuffer += "You huff for breath and wipe your brow, heart hammering from all the physical exertion you’ve been putting yourself through recently. In irritation you find yourself dreaming up all sorts of contraptions you could design to ferry yourself around, saving yourself all this unnecessary effort...";
				...
				{
					eventBuffer += "You " + target.mf("growl","purr") + " with enjoyment as liquid heat sinks down from your tummy to your groin,";
					if(target.balls > 0)
					{
						eventBuffer += " your [pc.balls] swelling in gratification";
						target.cumQualityRaw += 1;
					}
					if(target.balls > 0 && target.hasVagina()) eventBuffer += " and";
					if(target.hasVagina())
					{
						eventBuffer += " [pc.eachVagina] moistening with anticipatory need";
						target.fertilityRaw += 1;
					}
					eventBuffer += ". An increasingly tigrish and prickly urge to breed is stealing over you.";
				...
				{
					eventBuffer += "Hm, you feel a little more bottom heavy than usual. You squeeze your [pc.hip] only to find it getting taut between your fingers. Giving your [pc.butt] a slap, it rebounds back with an odd sensation. Your body seems to have changed to adapt to its fertile nature, flaring out and becoming womanly, at least for your lower body anyway...";
