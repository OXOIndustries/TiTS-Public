	...
	public class RedPill extends ItemSlotClass
	{
		...
		override public function useFunction(target:Creature, usingCreature:Creature = null):Boolean
		{
			...
			if(target is PlayerCharacter) {
				...
				if(changes < changeLimit)
				{
					var armLegSkinTF:Boolean = (pc.armType != GLOBAL.TYPE_RED_MYR || (pc.legType != GLOBAL.TYPE_RED_MYR && pc.legCount != GLOBAL.TYPE_HUMAN) || pc.legCount != 2 || (pc.hasFur() || pc.hasScales()));
					if(armLegSkinTF)
					{
						//PC isn't a biped: 
						if(pc.legCount != 2 || (pc.legType != GLOBAL.TYPE_RED_MYR && pc.legCount != GLOBAL.TYPE_HUMAN))
						{
							output("\n\nYour [pc.legOrLegs] ");
						...
						//Gain Chitin: 
						if(pc.armType != GLOBAL.TYPE_RED_MYR || pc.legType != GLOBAL.TYPE_RED_MYR)
						{
							var arms:Boolean = (pc.armType != GLOBAL.TYPE_RED_MYR);
							var legs:Boolean = (pc.legType != GLOBAL.TYPE_RED_MYR);
							...
							pc.armType = GLOBAL.TYPE_RED_MYR;
							pc.legType = GLOBAL.TYPE_RED_MYR;
							pc.legCount = 2;
							pc.clearLegFlags();
							pc.addLegFlag(GLOBAL.FLAG_PLANTIGRADE);
				...
				//Black Hair/Red Hair
				if(!InCollection(pc.hairColor,"black","red") && changes < changeLimit)
				{
					if(rand(2) == 0)
					{
				...
				//Human Face
				if(changes < changeLimit && pc.faceType != GLOBAL.TYPE_HUMAN)
				{
				...
				//Elfin Ears
				if(changes < changeLimit && pc.earType != GLOBAL.TYPE_SYLVAN)
				{
				...
				if(pc.hasCock() && changes < changeLimit)
				{
					...
					if(pc.cocks[x].cLength() < 18)
					{
						...
						pc.cocks[x].cLengthRaw += 3 + rand(4);
				...
				//Black, Insectile Eyes
				if(changes < changeLimit && (pc.eyeType != GLOBAL.TYPE_RED_MYR && pc.eyeType != GLOBAL.TYPE_GOLD_MYR))
				{
					...
					pc.eyeType = GLOBAL.TYPE_RED_MYR;
					changes++;
				}
				//Insect Feelers
				//Gain slight vulnerability to Lust (Tease) & Lust (Chemical) damage
				if(pc.antennae == 0 && changes < changeLimit)
				{
					...
					pc.antennae = 2;
					pc.antennaeType = GLOBAL.TYPE_RED_MYR;
					changes++;
				}
				/*Red Myr Venom
				...
				//Lactation Typechange
				//Gain honey milk
				if(pc.milkType != GLOBAL.FLUID_TYPE_HONEY && changes < changeLimit)
				{
				...
				if(pc.thickness > 15 && changes < changeLimit)
				{
				...
				//+Tone
				//Slight Tone bonus
				if(pc.tone < 85 && changes < changeLimit)
				{
				...
				//+Femininity (if PC has cunt)
				if(pc.femininity < 85 && pc.hasVagina() && changes < changeLimit)
				{
					...
					pc.femininity += 4 + rand(3);
					changes++;
				}
				//RARE!
				//Lactation start
				//PC must have C+ cups
				if(changes < changeLimit && pc.hasVagina() && pc.biggestTitSize() >= 3 && !pc.canLactate())
				{
				...
				//Libido Increase
				//Max 60. Gain +30 Lust
				if(changes < changeLimit && pc.libido() < 60)
				{
				...
				if(changes < changeLimit)
				{
					...
					//Check if in size
					if(pc.breastRows[smallestBoobRow].breastRating() < 5)
					{
				...
				//Honey Jizz
				if(changes < changeLimit && pc.cumType != GLOBAL.FLUID_TYPE_HONEY && pc.hasCock())
				{
