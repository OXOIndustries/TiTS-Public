	...
	public class Foxfire extends ItemSlotClass
	{
		...
		private static function tfGenitals(target:Creature):Boolean
		{
				...
				for (var i:int = 0; i < target.cockTotal(); i++) 
				{
						...
						|| target.cocks[i].knotMultiplier <= 1 && target.knotMultiplierUnlocked(i, 1.25)))
					{
						options.push(function():*{
							...
							{
								AddLogEvent("You feel a light tug at the base of your " + target.cockDescript(i) + ", followed by something sliding up and over your shaft. A quick look tells you that an animalistic sheath has formed, bringing your cock to a standard canine form. <b>Your cock now has a canine-sheath!</b>", "passive");
							...
							{
								AddLogEvent("You feel a bulging pressure near the base of your " + target.cockDescript(i) + ", as a vulpine knot begins to form. <b>Your cock is now knotted!</b>", "passive");
							...
							{
								AddLogEvent("Another feeling of pressure at the base of your " + target.cockDescript(i) + " tells you that the bulk of your knot is growing to a respectible bulge. <b>Your cock-knot has grown!</b>", "passive");
								target.cocks[i].knotMultiplier = 1.25;
							}
							else if (!target.cocks[i].hasFlag(GLOBAL.FLAG_TAPERED) && target.cockFlagsUnlocked(i, GLOBAL.FLAG_TAPERED))
							{
								AddLogEvent("Your " + target.cockDescript(i) + " grows sensitive all of a sudden. As you look down to determine the cause, you catch sight of your cock thinning slightly near the tip, and quickly begins tapering towards the end, like that of a typical fox's member. <b>Your cock is now tapered!</b>", "passive");
					...
					else if (target.cocks[i].cType != GLOBAL.TYPE_VULPINE && target.cockTypeUnlocked(i, GLOBAL.TYPE_VULPINE))
					{
						options.push(function():*{
							...
							AddLogEvent("Your " + target.cockDescript(i) + " throbs suddenly ", "passive");
			...
			{ // Ballz down, this is not a tanuki morphs! Though not too radically - just prevent them from dragging the floor.
				options.push(function():*{
					AddLogEvent("A sudden inward pressure erupts inside your " + target.sackDescript() + ", drawing a gasp from you. It is painless, but certainly discomforting. A quick check reveals that <b>your " + target.ballsDescript(false, true, false, false) + " " + (target.balls == 1 ? "has" : "have") + " shrunk!</b>", "passive");
			...
			if (target.biggestTitSize(true) > maxSizeBreasts) // breasts down to C-cups, if multirow other rows to b/a/flat
			{
				for (var b:int = 0; b < target.breastRows.length; b++)
				{
					...
					{
						options.push(function():*{
							AddLogEvent("You feel as if like a weight has been lifted off your chest. Literally! You discover that <b>your " + target.allBreastsDescript() + " have shrunk", "passive");
			...
			{ // if female or shemale, grow another row of nipples up to 4 total
				options.push(function():*{
					if (minSizeBreasts == 0)
					{ // normal case - flats
						AddLogEvent("A sudden sensation erupts on your chest, as if your nipples had just been flicked during an intense bout of lovemaking. As you reflexively raise your hands to them, you notice that the feeling in fact came from below your nipples. Taking a quick peek", "passive");
					...
					{ // lactating - A-cups to avoid parser weirdshit
						AddLogEvent("Suddenly, you feel as if the area on your " + (target.bRows() < 2 ? "chest" : "belly") + " below your breasts is growing softer and more sensitive, which is followed by a feeling of being filled with something. As you suspected, " + num2Text(target.breastRows[target.breastRows.length - 1].breasts) + " soft orbs of flesh soon begin pushing their way out from your " + (target.bRows() < 2 ? "chest" : "belly") + ", forming a new " + (target.breastRows[target.breastRows.length - 1].breasts == 2 ? "pair" : "set") + " of A-cup breasts below the original ones!", "passive");
					}
			...
			{
				options.push(function():*{
					AddLogEvent(" You feel a strange tingling around your chest area. After some examination you notice the color of your " + target.nipplesDescript() + " has changed. <b>You now have black nipples!</b>", "passive");
		...
		private static function tfStage1(target:Creature):Boolean
		{
			...
			{ // Fixing tail furry flag, if it somehow missing.
				AddLogEvent("The fur on your foxtail" + (target.tailCount > 1 ? "s" : "") + " has regrown!", "passive");
			...
			{ // Fixing tail long flag, if it somehow missing.
				if (target.tailCount == 1) AddLogEvent("Your stubby fox tail suddenly pulls at the base of your spine, growing longer until it reaches the length of a regular fox's tail.", "passive");
				else AddLogEvent("Your stubby fox tails suddenly pull at the base of your spine, growing longer until they all reach the length of a regular fox's tail.", "passive");
			...
			if (target.hasTail(GLOBAL.TYPE_VULPINE) && target.hasTailFlag(GLOBAL.FLAG_FURRED) && !target.hasTailFlag(GLOBAL.FLAG_FLUFFY) && target.tailFlagsUnlocked(GLOBAL.FLAG_FLUFFY)) // Checking tail fluffy flag.
			{
				...
				{
					AddLogEvent("Your " + target.tailsDescript() + " quivers briefly as a strange pressure builds within; a moment later, a new batch of fur pushes its way outwards, doubling the thickness of the fur already there, and keeps growing longer and longer. Within seconds, the fur on your tail has grown out to nearly two feet from the center on all sides. <b>The fur on your foxtail has grown extra fluffy!</b>", "passive");
				}
				else
				{
					AddLogEvent("Your " + target.tailsDescript() + " quiver briefly as a strange pressure builds within; a moment later, a new batch of fur pushes its way outwards, doubling the thickness of the fur already there, and keeps growing longer and longer. Within seconds, the fur on your tails has grown out to nearly two feet from the center on all sides. <b>The fur on your foxtails has grown extra fluffy!</b>", "passive");
				}
			...
			if ((target.earType == GLOBAL.TYPE_VULPINE || place == 1) && !target.hasTail(GLOBAL.TYPE_VULPINE))
			{
				if (target.tailTypeUnlocked(GLOBAL.TYPE_VULPINE) || target.hasParasiteTail())
				{
					...
					if (target.hasTail()) 
					{
						...
						if (target.tailCount == 1)
						{
							AddLogEvent("Your " + target.tailsDescript() + " feels strange somehow. Before you can figure out exactly why, it suddenly twitches and begins to morph in front of your eyes, puffing up into a fluffy vulpine tail. <b>You are now the proud owner of a fox tail!</b>", "passive");
						}
						else
						{
							AddLogEvent("Your " + target.tailsDescript() + " feels strange somehow. Before you can figure out exactly why, your multiple tails suddenly start to twitch and morph in front of your eyes, puffing up into fluffy vulpine tails. <b>Your now have " + num2Text(target.tailCount) + " vulpine tails!</b>", "passive");
						}
					}
					else
					{
			...
			if (kGAMECLASS.attachedMimbranes() > 0) // get rid of all non-tail parasites, since foxfire is technically a parasite itself and won't get along with contenders!
			{
				...
				{
					AddLogEvent("Your mimbrane parasite turns a sickly pale as it suddenly releases your " + target.skin() + ", slumping to the ground lifelessly. You almost feel sorry for the poor creature as it shrivels up and crumbles to dust before your eyes. <b>Your mimbrane is gone!</b>", "passive");
				}
				else
				{
					AddLogEvent("Your mimbrane parasites turn a sickly pale as they suddenly let go of your " + target.skin() + ", slumping to the ground lifelessly. You almost feel sorry for the poor creatures as they shrivel up and crumble to dust before your eyes. <b>Your mimbranes are gone!</b>", "passive");
				}
			...
			if (target.hairType != GLOBAL.HAIR_TYPE_REGULAR && target.hairTypeUnlocked(GLOBAL.HAIR_TYPE_REGULAR)) // normalizing hair
			{
				...
				{
					AddLogEvent("A feather falls down in front of you, followed by several more as you feel something pushing underneath your scalp. Your head-feathers cascade down you as they leave their resting places, and are quickly replaced with tufts of normal hair. <b>You now have normal hair!</b>", "passive");
					target.hairLength = 1;
					target.hairStyle = "null";
				}
				else if (target.hairType == GLOBAL.HAIR_TYPE_TRANSPARENT)
				{
					AddLogEvent("A few stray strands of hair become visible before your eyes, followed by a few more, and a few more. Within moments, <b>your hair loses its transparency</b> and settling into a normal opaque state. <b>You now have normal hair!</b>", "passive");
					// keep both length and style here
				}
				else if (target.hairType == GLOBAL.HAIR_TYPE_GOO)
				{
					AddLogEvent("You notice that you haven't seen any drips falling from your head-goo for a few minutes. Running a hair over your head, you feel distinct strands of something stringy, rather than the gelatinous mass you're used to feeling. The strands thicken and take form as tufts of normal hair, replacing your goo-hair. The new hair stays in place nicely instead of poofing everywhere, as your natural goo seems to be acting as a sort of hair styling gel. <b>You now have normal hair!</b>", "passive");
					target.hairLength = 1;
					target.hairStyle = "null";
				}
				else if (target.hairType == GLOBAL.HAIR_TYPE_TENTACLES)
				{
					AddLogEvent("An alien, sucking sensation erupts at the top of your head, and you feel your head-tentacles pulling down into your scalp. The patches between the tentacles begin sprouting bristles of hair, and soon enough, <b>your head-tentacles are fully replaced by normal hair!</b>", "passive");
					target.hairLength = 1;
					target.hairStyle = "null";
				}
				else if (target.hairType == GLOBAL.HAIR_TYPE_QUILLS)
				{
					AddLogEvent("You feel a strange tingling on your scalp as the skin around your quills loosen and grow slack. Within moments, the your head-quills begin falling down your back and shoulders, and soft tufts of normal hair grow to take their place. <b>You now have normal hair!</b>", "passive");
					target.hairLength = 1;
					target.hairStyle = "null";
				}
				else
				{
					AddLogEvent("Your " + target.hairNoun() + " melts into a gooey-mass on top of your head. You wipe it off only to discover regular human-like hair beneath. <b>You now have normal hair!</b>", "passive");
					target.hairLength = 1;
					target.hairStyle = "null";
				}
			...
			{  // setting hair ablaze!
				...
				AddLogEvent("Wait, is it merely your imagination or is your hair on fire?! You reach up to feel it, but find it to be cool to the touch. Strange... <b>You now have " + fireColor + " hair!</b>", "passive");
				return false;
			}
			...
			{  // hair growth - a bit off for animalistic TF, but since it is a hair dye...
				if (!target.hasHair())
				{
					if (target.hasFur())
					{
						AddLogEvent("Your head-fur begins to itch, and before long it starts to grow out wildly at a rapid pace, blossoming into long hair which reaches all the way down your neck. <b>You now have long hair!</b> You are going to need a brush...", "passive");
					}
					else
					{
						AddLogEvent("Your " + target.hairDescript() + " starts to tingle, and you feel an sudden urge to scratch it. Just as you reach for it your " + target.fingers() + " are met with a large burst of wild growth, not stopping until it reaches down all the way to your neck. <b>You now have long hair!</b> You're going to need a brush...", "passive");
					}
				}
				else
				{
					AddLogEvent("You brush away a few strands of hair from your vision, and feel something brush against your neck. It seems <b>your " + target.hairNoun() + " has grown longer!</b>", "passive");
				}
			...
			{ // setting vanae markings ablaze!
				...
				AddLogEvent("You experience a burning sensation around your skin-markings, as they change tone to match your new form, becoming fiery like that of living-embers. Your markings are now " + accentColor + "!", "passive");
			...
			{
				AddLogEvent("Your " + target.earsDescript() + " shift to become large and pointy like that of a fox’s, allowing you to pick up even the faintest of sound as they twitch atop your head. <b>You now have pointy fox ears!</b>", "passive");
			...
			if (target.eyeType != GLOBAL.TYPE_VULPINE && target.eyeTypeUnlocked(GLOBAL.TYPE_VULPINE)) // fox eyes are more like feline than dog-like, but usually yellowish toned and rarely blue
			{
				...
				AddLogEvent("Your eyes water and strain for a moment. As you blink, you notice a greater depth of contrast in the nearby lights and shadows. <b>You have slitted, " + target.eyeColor + " fox-like eyes now!</b>", "passive");
				return false;
			}
		...
		private static function tfStage2(target:Creature):Boolean
		{
			...
			{
				AddLogEvent("Your foxtail" + (target.tailCount > 1 ? "s" : "") + " has now grown especially long, and appear" + (target.tailCount > 1 ? "" : "s") + " to be... prehensile as well? What the hell.", "passive");
				target.addTailFlag(GLOBAL.FLAG_PREHENSILE);
			...
			if (target.hasTail(GLOBAL.TYPE_VULPINE) && target.tailCount < 9 && target.tailCountUnlocked(target.tailCount + 1)) // kitsunezations
			{
				...
				switch (target.tailCount)
				{
					...
					case 5:
						...
						break;
					case 6:
						AddLogEvent("Here we go, <b>a sixth foxtail now joins the rest.</b>", "passive");
						break;
					case 7:
						AddLogEvent("<b>A seventh foxtail pops out behind you.</b>", "passive");
						if (kGAMECLASS.flags["FOXFIRE_MAX_TAILS"] < target.tailCount) ExtendLogEvent(" Not even surprising, at this point.");
						break;
					case 8:
						AddLogEvent("<b>An eighth foxtail joins the party.</b>", "passive");
						if (kGAMECLASS.flags["FOXFIRE_MAX_TAILS"] < target.tailCount) ExtendLogEvent(" Will this ever stop?");
						break;
					case 9:
						AddLogEvent("<b>A ninth foxtail sprouts next to the rest.</b>", "passive");
						...
						break;
					default:
						AddLogEvent("+1 foxtail!, NOW something gone really wrong!", "passive"); // failsafe here, not supposed to actually happen
				}
		...
		private static function tfStage3(target:Creature):Boolean
		{
			...
			{
				target.furColor = fireColor;
				AddLogEvent("Your fur begins to glow in an eerie-green - before it slowly shifts and settles into a " + fireColor + " tone, retaining its luminosity. <b>You now have " + fireColor + " fur!</b>", "passive");
			...
			if (frost && target.hasFur() && !target.hasSkinFlag(GLOBAL.FLAG_FLUFFY) && target.skinFlagsUnlocked(GLOBAL.FLAG_FLUFFY)) // Checking skin fluff for Frostfire.
			{
				...
				AddLogEvent("You feel a sudden tingling at the base of your coat of fur, and you begin to feel slightly warmer. The reason quickly becomes obvious: your fur is growing slightly longer, and more strands are pushing their way out in between the existing ones, growing thicker and denser with each passing second. Your fur is now thick!", "passive");
			...
			if (target.legType != GLOBAL.TYPE_VULPINE && target.legTypeUnlocked(GLOBAL.TYPE_VULPINE)) // wrong legs type - changing
			{
				if (target.legCount != 2 && target.legCount != 4 && target.legCountUnlocked(target.isTaur() ? 4 : 2)) // naga / leithan, changing to biped / taur
				{
					if (target.isNaga())
					{
						AddLogEvent("Your serpentine lower half shivers for a moment, growing tense and less flexible. Your lower length shortens, and painlessly splits at the tail, then runs the length of your mass until it reaches your hips. The two short tails quickly become rigid and firm, and can now only bend one way at the middle section. Within moments, <b>you have knees again, as well as feet and legs resembling that of a fox's</b>.", "passive");
						target.legCount = 2;
						target.genitalSpot = 0;
					}
					else if (target.isGoo())
					{
						AddLogEvent("Suddenly, the lower part of your gooey mass presses inwards and becomes much more solid. You feel bones and muscles reforming underneath you, and im a moment the mass of goo you were previously using to move yourself about is now a <b>pair of sturdy fox-like legs</b>.", "passive");
						target.legCount = 2;
						target.genitalSpot = 0;
					}
					else if (target.isTaur() || target.isDrider())
					{
						AddLogEvent("Your " + target.legs() + " quiver in a moment of sudden weakness; the muscles within quickly press inwards and tighten, growing taut and high-strung. Your legs' overall thickness follows, becoming slightly leaner within a few seconds, now resembling that of a wild fox. <b>You now have fox-legs!</b>", "passive");
					...
					{
						AddLogEvent("You watch helplessly as your " + target.lowerBody() + " suddenly dissolves underneath you into a pile of goo! Thankfully your worries are short-lived as the goo begin to morph and take shape, reforming into a <b>pair of fully functional fox-legs!</b>", "passive");
						target.legCount = 2;
						target.genitalSpot = 0;
					}
				...
				else if (target.legCount == 2 || target.legCount == 4) // already fitting biped / quadripedal taur, just changing type
				{
					...
					if (!someChange)
					{
						AddLogEvent("Your legs quiver in a moment of sudden weakness; the muscles within quickly press inwards and tighten, growing taut and high-strung. Your legs' overall thickness follows, becoming slightly leaner within a few seconds. <b>You now have fox-legs!</b>", "passive");
					}
					else
					{
			...
			if (target.armType != GLOBAL.TYPE_VULPINE && target.armTypeUnlocked(GLOBAL.TYPE_VULPINE)) // wrong arms type - changing
			{
				...
				if (!wasFurryArms)
				{
					AddLogEvent("A slight tickle crawls across your arms as thin, short strands of fur begin poking out from your skin. Within seconds, a full <b>coat of vulpine fur covers the entirety of the skin on your arms</b> down from the shoulders.", "passive");
					someChange = true;
				}
				
				if (!wasPaddedArms)
				{
					AddLogEvent("A feeling of sudden pressure begins to build on the upper inner span of your hands, and on the bottom of your fingertips. Tough, yet sensitive <b>pads like that of a canine push their way out from your hands</b>, designed to allow grip and traction despite a coat of fur.", "passive");
					someChange = true;
				}
			...
			else if (target.armType == GLOBAL.TYPE_VULPINE) // proper arms type - checking flags
			{
				if (!target.hasArmFlag(GLOBAL.FLAG_FURRED)) // Checking fur on arms. No pre-requisites.
				{
					...
					if (!target.hasFur()) // Only show output if have no skin type fur - it would just force keeping fur on hands should you change skin type.
						AddLogEvent("A slight tickle crawls across your arms as short, thin strands of fur begin poking out from your skin. Within seconds, a <b>full coat of vulpine fur covers the entirety of the skin on your arms</b> down from the shoulders.", "passive");
			...
			else if (target.faceType == GLOBAL.TYPE_VULPINE) // proper face type - checking flags
			{
				if (!target.hasFaceFlag(GLOBAL.FLAG_MUZZLED))
				{
					...
					AddLogEvent("Your face begins to tingle, and is followed by a tugging sensation between your eyes. Your nose pushes outwards, widening slightly as it grows, until it rests upon your face as the signature thin <b>muzzle of a fox</b>.", "passive");
			...
			if (target.beardLength > 0 && target.faceType == GLOBAL.TYPE_VULPINE && target.hasFaceFlag(GLOBAL.FLAG_MUZZLED) && target.hasFur()) // No beards for foxes!
			{
				...
				{
					AddLogEvent("You scratch at your " + target.beard() + ", only to discover it has merged into your fur... Your beard is gone!", "passive");
					target.removeBeard();
				}
				// or, if you have enough beard, something more original?
				else if (target.beardLength >= 1 && target.beardStyleUnlocked(11))
				{
					AddLogEvent("Your " + target.beard() + " starts to suddenly grow out wildly as it merges with your fur, only to pull up to the sides of your " + target.face() + " in thick tufts, which flare back and outwards at the back of your jaw. You now have animalistic side-burns!", "passive");
			...
			if (!InCollection(target.lipColor, "onyx", "black") && !colorsMatching(target))
			{
				AddLogEvent("You catch the sight of your lips darkening, as they fade to a shiny black", "passive");
