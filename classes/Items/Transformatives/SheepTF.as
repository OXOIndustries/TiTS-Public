package classes.Items.Transformatives
{
	...
	public class SheepTF extends ItemSlotClass
	{
		...
		public function SheepTF()
		{
			...
			this.description = "a nondescript white pill with the word “Ovinium” stamped into the side, wrapped in Xenogen Biotech packaging";
			//Displayed on tooltips during mouseovers
			this.tooltip = "Not to be confused with the similarly named Bovinium, this is a transformative pill designed by Xenogen Biotech, designed to make the user take on characteristics similar to that of a Terran sheep. Effects are recorded to range between giving the user ram horns (though almost exclusively this occurs in men) and causing them to start growing a thick layer of fluffy, insulating wool. The pill itself looks bland, but the wrapper with the stylised sheep printed on it is rather fetching.";
		...
		override public function useFunction(target:Creature, usingCreature:Creature = null):Boolean
		{
			...
			if(target is PlayerCharacter) {
				...
				if(pc.horns == 0 && pc.mf("m","") == "m" && pc.hasCock() && changes < changeLimit)
				{
					output("\n\nYou feel a sudden pressure from behind your temples and reach up a hand to investigate. Feeling a pair of bumps from where the ache emanates, you realise - as they seem to harden and push outward - that you’re growing horns! The pain in your head starts to grow and you pull your hands away as the horns rapidly push and grow outwards. By the time it fades, you’ve pulled out your codex to have a look at your recent additions. <b>You’ve grown curved, impressive rams horns!</b>");
					pc.horns = 2;
					pc.hornType = GLOBAL.TYPE_SHEEP;
					changes++;
				}
				...
				if(pc.isNaga() && changes < changeLimit)
				{
				...
				if(pc.legCount == 2 && pc.legType != GLOBAL.TYPE_SHEEP && rand(4) == 0 && changes < changeLimit)
				{
					output("\n\nYour [pc.legs] suddenly start shaking and quickly give out under you, leaving you sprawled out along the ground. Looking back, you realise that your legs and feet are reshaping themselves. It takes a while, but as you watch,<b> they transform into hooved, sheeplike legs.</b>");
					pc.clearLegFlags();
				...
				if(pc.hairType == GLOBAL.HAIR_TYPE_HAIR && pc.hairColor != "white" && pc.hairColor != "black" && rand(3) == 0 && changes < changeLimit)
				{
					output("\n\nA tickling sensation passes through your scalp before fading almost as soon as it appeared. Confused, you pull out your codex to have a look and see a new colour racing down your ");
					if(pc.hairLength < 3) output("short");
					else if(pc.hairLength < 10) output("mid-length");
					else output("long");
					output(" hair. By the time it’s done,<b> you’re left with ");
					if(rand(3) >= 1) pc.hairColor = "white";
					else pc.hairColor = "black";
					output(pc.hairColor + " hair!</b> Well, you guess you can always have it dyed if you’re not a fan.");
					changes++;
				}
				...
				if((pc.hairColor == "white" || pc.hairColor == "black") && pc.hairType == GLOBAL.HAIR_TYPE_HAIR && !pc.hasFur() && rand(2) == 0 && changes < changeLimit)
				{
					output("\n\nYou scratch at your arm, feeling something itching just beneath the surface. Strange. Looking down for a closer look, you see that any hair on your arms is growing rapidly more pronounced - in fact, it’s coming in on your hands and face too! After a long and slightly nerve wracking wait, you realise it’s not going to stop at all - the covering is actually better described as fur now. <b>Well, looks like you’ve got a thin layer of coarse " + pc.hairColor + " fur!</b>");
				...
				//Turn nipples black: 
				if(pc.nippleColor != "black" && rand(4) > 0 && changes < changeLimit)
				{
				...
				//Turn lips black: 
				if(pc.lipColor != "black" && pc.skinTone == "black" && rand(4) > 0 && changes < changeLimit)
				{
