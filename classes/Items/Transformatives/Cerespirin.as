		...
		public static function plantMutations(target:Creature, numProcs:int, deltaT:uint, effect:StorageClass):void
		{
			...
			{	
				...
				else if(select == 5)
				{
					...
					{
						msg += "You blink slowly. It’s difficult to tell if the feeling of increased clarity you’re experiencing is all in your head, or... you get your codex out and activate its mirror function. The color of your eyes has changed - the irises are now " + newEyeColor + ".";
				...
				else if(select == 6)
				{
					if(target.hornType != GLOBAL.TYPE_DRYAD)
					{
						...
						if(target.hornTypeUnlocked(GLOBAL.TYPE_DRYAD))
						{
							if(target.hasHorns())
							{
								msg += ParseText("Your " + (target.horns == 1 ? "[pc.hornNoun] feels" : "[pc.hornsNoun] feel") + " a bit odd - sick, even. You give " + (target.horns == 1 ? "it" : "them") + " a fondle, and are shocked when " + (target.horns == 1 ? "it comes" : "they come") + " off in your hand, as if " + (target.horns == 1 ? "it was" : "they were") + " made of rotten wood. Out with the old, and...");
								msg += "\n\n";
								target.removeHorns();
							}
							msg += ParseText("You grunt as two new something-or-others sprout into existence above your brow; harder and more definitive than your [pc.hair]. Horns? You touch the thin spikes pushing their way out of your [pc.skin] prospectively. It isn’t keratin, exactly... more bark-like.");
				...
				else if(select == 14)
				{
					...
					{
						msg += ParseText("You feel flushed and worked up, unusual in your experience of Ceresperin. Despite your best efforts, [pc.eachVagina] won’t let you forget about " + (target.totalVaginas() == 1 ? "its" : "their") + " presence, first beading and then soaking your " + ((!removePants && target.isCrotchExposed()) ? "[pc.thigh]" : "[pc.lowerGarment]") + " in unrequited arousal");
				...
				else if(select == 17)
				{
					...
					msg += "You sneeze. The air around you is thick, warm and yellow-tinged.";
					// Populated area:
					...
					if(!InShipInterior() && InCollection(pName, ["PLANET: TARKUS", "PLANET: MYRELLION", "PLANET: PHAEDRA"]))
					{
						...
						msg += ParseText(", [pc.eachVagina] to slicken and widen eagerly");
					}
