	...
	public class AusarTreats extends ItemSlotClass
	{
		...
		override public function useFunction(target:Creature):Boolean
		{
			...
			if(target is PlayerCharacter) {
				...
				if(changes < changeLimit && x >= 0)
				{
					output("\n\nYour [pc.cock " + x + "] throbs suddenly ");
					if(pc.isCrotchGarbed()) output("beneath your [pc.lowerGarments]");
					else output("between your [pc.legs]");
					output(", swelling to its full size as a strange heat radiates through your body. You grab your simmering penis as the alien concoction takes effect, rewriting your most sensitive places. You double over in a sick twist of pain and pleasure");
					if(pc.cocks[x].cockColor != "bright red")
					{
						output("as the flesh of your cock shifts color to a bright red");
					}
					if(!pc.hasCockFlag(GLOBAL.TAPERED)) output(", skin melding strangely into a long, pointed shape");
					output(".");
					//if cock was less than 6 inches: 
					if(pc.cocks[x].cLength() < 6) {
						output(" Your prick grows, too, expanding several inches from your groin.");
						pc.cocks[x].cLength(2+rand(3));
					}
					if(!pc.hasCockFlag(GLOBAL.TAPERED)) output(" The [pc.cockHead " + x + "] of your dick twists into a pointed tip");
					else output(" The [pc.cockHead " + x + "] jumps");
					output(", drooling a sticky [pc.cumColor] river from its engorged slit.");
					if(pc.balls > 0) output(" Just over your [pc.balls]");
					else output(" Just ahead of the base");
					output(", your member's flesh engorges exponentially, swelling into a thick, cum-engorged knot. With a cry, you fall to the ground as the new doggy-cock discharges its heady load, shooting a thick rope of [pc.cum] through the swollen knot and onto the ground.");
				...
				if(changes < changeLimit && x >= 0 && target.cockLengthUnlocked(x,pc.cocks[x].cLength() + 1.5))
				{
					...
					output(Math.round(pc.cocks[x].cLength()*10)/10 + " inches. You take a moment to marvel at your new, larger member, ready for action.");
					changes++;
					pc.lust(10+rand(5));
				}
				//Knot size+
				...
				if(changes < changeLimit && x >= 0)
				{
					...
					if(!pc.isCrotchGarbed()) output("strip");
					else output("spread your [pc.legs]");
					output(" and grab your knotty cock. Your bitch-breaking knot is swelling even larger, engorging with cum and blood as your own arousal peaks. You feverishly grab your wang, stroking it as quick as you can as your knot grows larger than it ever has before!");
					changes++;
					pc.lust(20+rand(20));
					pc.libido(.5);
				...
				//Misc. Effects
				// +Intelligence (They made the damn Gates)
				// +Libido (see: Syri)
				// +Reflexes
				// -Physique

				//Change arm type to furred
				if(pc.armType != GLOBAL.CANINE && changes < changeLimit)
				{
				...
				if(pc.legType != GLOBAL.CANINE && changes < changeLimit && pc.legCount == 2)
				{
				...
				if(pc.earType != GLOBAL.CANINE && changes < changeLimit)
				{
					...
					output("\n\n<b>You now have an ausar's wolf-ears!</b>");
					changes++;
				}
				...
				else
				{
					...
					else if(target.legType != GLOBAL.PANDA && target.legTypeUnlocked(GLOBAL.PANDA) && changes < changeLimit && target.tailType == GLOBAL.PANDA)
					{
						...
						target.legCount = 2;
						target.legType = GLOBAL.PANDA;
						target.clearLegFlags();
						target.addLegFlag(GLOBAL.DIGITIGRADE);
						target.addLegFlag(GLOBAL.FURRED);
						target.addLegFlag(GLOBAL.PAWS);
						changes++;
					}
					...
					else if(target.skinType != GLOBAL.FUR && target.skinTypeUnlocked(GLOBAL.FUR) && changes < changeLimit && target.armType == GLOBAL.PANDA)
					{
						...
						target.addSkinFlag(GLOBAL.FLUFFY);
						target.addSkinFlag(GLOBAL.FURRED);
						changes++;
					}
