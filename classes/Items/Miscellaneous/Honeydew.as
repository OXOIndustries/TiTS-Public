	...
	public class Honeydew extends ItemSlotClass
	{
		...
		override public function useFunction(target:Creature, usingCreature:Creature = null):Boolean
		{
			...
			if(target is PlayerCharacter) {
				...
				if(target.canLactate() && target.milkType != GLOBAL.HONEY)
				{
				...
				else if(target.milkType != GLOBAL.HONEY && !target.canLactate() && target.biggestTitSize() >= 4 && rand(2) == 0)
				{
				...
				else
				{
					...
					if(target.milkType != GLOBAL.HONEY || !target.canLactate()) kGAMECLASS.output("milkless ");
					...
					if(target.milkType == GLOBAL.HONEY) kGAMECLASS.output("already have the full effect of the drug</b>.");
