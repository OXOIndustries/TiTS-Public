		...
		override public function useFunction(target:Creature):Boolean
		{
			...
			if(target is PlayerCharacter) {
				...
				if(target.hasCock()) {
					...
					//Grow wings once!
					if(target.wingType != GLOBAL.BEE && rand(3) == 0 && changes < changeLimit) {
						if(!target.hasWings()) {
				...
				if(target.hasVagina()) {
					...
					//Grow zilbutt!
					if(target.tailType != GLOBAL.BEE && changes < changeLimit && rand(3) == 0) {
						//Has tails!
				...
				//Gender Neutral TFs
				//Human-style face
				if(target.faceType != GLOBAL.HUMAN && changes < changeLimit && rand(4) == 0) {
					//Has muzzle
				...
				if(target.legType != GLOBAL.BEE && changes < changeLimit && rand(3) == 0) {
